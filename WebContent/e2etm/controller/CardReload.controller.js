...
sap.ui.controller("sap.ui.project.e2etm.controller.CardReload", {
	...
	//// ucd1kor fema doc fetch
	onPressFEMA:function(evt){
		var path = evt.getSource().getParent().getBindingContextPath();
		var data = evt.getSource().getParent().getModel().getProperty(path);
		//try{window.open(data.EvUrl, "_blank");}catch(err){}
	},
			...
			if(sPath=="TravelPlan"||sPath=="EmpNo"){
				...
				if (key == "MONTH") {
				   filterString = "MRWorklistSet?$filter=LoginRole eq '"+role+"' and "+sPath+" eq '"+evt.getParameter("item").getValue()+"' and Tab eq '" + tab + "'";
					...
					var skipCount = 0;
					var top = 40;
					oController.clicks = 0;
					filterString = "ForexWorklistSet?$filter=LoginRole eq '"+role+"' and "+sPath+" eq '"+evt.getParameter("item").getValue()+"' and Tab eq '" + tab + "'"+"&$skip="+skipCount+"&$top="+top;
					   //end of code added by uml6kor_9/9/2020 for paging
	...
	setFieldDashboard : function(key) {
		...
		if (sap.ui.getCore().getModel("profile").getData().currentRole != "FINA") {
			...
			if (key == "CARD") {
				...
				pepayroll.femavisible = true; // added by ucd1kor 24/05/2021
				// bar.setVisible(false);
			} else if (key == "APPROVE") {
				oController.getView().byId("appIcon").getContent()[1].setMode("MultiSelect");
				properties = {
				...
				pepayroll.auvisible = false;
				pepayroll.femavisible = false; // added by ucd1kor 24/05/2021
				// bar.setVisible(true);
				// approve.setVisible(false);
...
//		if (role == "FINA") {
		...
		if (tab && table){
			if(key == "MONTH"){
				...
				//start of code for paging 10/9/2020 uml6kor
				var skipCount = 0;
				var top = 40;
				oController.clicks = 0;
				filterString = "ForexWorklistSet?$filter=LoginRole eq '"+role+"' and Tab eq '" + tab + "'"+"&$skip="+skipCount+"&$top="+top;
//				 filterString = "ForexWorklistSet?$filter=LoginRole eq '"+role+"' and Tab eq '" + tab + "'";
				//end of code for paging 10/9/2020 uml6kor
			}
			this.getSelectedTabValues(filterString,table);
		}
//		}
	...
	getSelectedTabValues : function(filterString,table) {
		...
		if(oController.clicks<=0 && profileRole == "FINA"){
		try{	
			sap.ui.getCore().getModel("global").getData().prevbtn = false;	
			sap.ui.getCore().getModel("global").refresh();
		}
		...
		jQuery.proxy(function(oData, response) {
			...
			try{	
				...
				oController.initialCount = oData.results.length;
				}	//end of code added by uml6kor_9/9/2020 for paging
...
//			 this.fetchDetails(); "commented by uml6kor function not used  18/1/2021 cggs upload RBEIITAPPPIPELINE_197
		},this));
	...
	getTemplateColumns:function(){
		...
		, {
			...
			template : {
				content : {
					path:"ZZ_HOUSING_NET",//  ZzHousingNet
					type: 'sap.ui.model.type.Float',
	                formatOptions: {
					    maxFractionDigits: 2
	                }
		...
		},
		{
			...
			template : {
				content : {
					path:"ZZ_SOCIAL_SEC", //  ZzSocialSec
					type: 'sap.ui.model.type.Float',
	                formatOptions: {
					    maxFractionDigits: 2
	                }
	...
	//*********************************start of code for changes paging 9/9/2020 uml6kor***********************************
...
//		if (skipCount <= this.initialCount - 1) {
//			sap.ui.getCore().getModel("global").getData().nxtbtn = true;
//		}
		sap.ui.getCore().getModel("global").refresh();
		var tab = this.getTab();
		var role = sap.ui.getCore().getModel("profile").getData().currentRole;
		var filterString = "ForexWorklistSet?$filter=LoginRole eq '"+role+"' and Tab eq '" + tab + "'"+"&$skip="+skipCount+"&$top="+top;	
		var table = evt.getSource().getParent().getParent().mAggregations.content[1];
		this.getSelectedTabValues(filterString,table);
	},
...
//*********************************end of code for changes paging 9/9/2020 uml6kor***********************************
