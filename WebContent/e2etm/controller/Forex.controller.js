...
sap.ui.controller("sap.ui.project.e2etm.controller.Forex", {
	 ...
	 */
	onInit : function() {
		//start of code added by uml6kor_9/9/2020 for paging
		//disabling the previous button on pagination
		sap.ui.getCore().getModel("global").getData().prevbtn = false;
		sap.ui.getCore().getModel("global").getData().nxtbtn = true;
		sap.ui.getCore().getModel("global").refresh();
		this.clicks = 0; //global variable for pagination control
		//end of code added by uml6kor_9/9/2020 for paging
	...
	onRouteMatchedDetail : function(event) {
		if ((event.getParameter("name") == "forex"&&this.getView().sViewName==event.getParameter("view").sViewName) || 
		   (event.getParameter("name") == "modifyclose"&&this.getView().sViewName==event.getParameter("view").sViewName)) {
			...
			//oForex.setFixedAttachments(selctedItem); UCD1KOR commentd here and added in success of batch call
			// Batch
			// get Currencies
			...
			//////////////########################## UCD1KOR 12 Apr 2021 #########################################///////////
			///////////// INS, Passport, TCK attachments added
			var passportUrl = oDataModel.createBatchOperation("DmsDocsSet?$filter=DepReq+eq+'999999999'+and+EmpNo+eq+'" + selctedItem.EmpNo + "'+and+DocType+eq+'PS'", "GET");
			var InsUrl = "DmsDocsSet?$filter=DepReq+eq+'{0}'+and+EmpNo+eq+'{1}'+and+DocType+eq+'{2}'";
			InsUrl = InsUrl.replace("{0}", selctedItem.TravelPlan);
			InsUrl = InsUrl.replace("{1}", selctedItem.EmpNo);
			InsUrl = InsUrl.replace("{2}", 'INS');
			var InsUrl = oDataModel.createBatchOperation(InsUrl, "GET");
			if (selctedItem.whichtab == "FSURR") {
				...
				oDataModel.addBatchReadOperations([ getCurr, getCards, getCash, getPassport, getAttachments, getAttachments1, getComments, getF4, getCardsAll, getCardSur, getCashSur, getInrSur, getTotal, getTrstComments, getTckAttachments,passportUrl,InsUrl ]);
			} else {
				oDataModel.addBatchReadOperations([ getCurr, getCards, getCash, getPassport, getAttachments, getAttachments1, getComments, getF4, getCardsAll, getTckAttachments,passportUrl,InsUrl ]);
			}
			oDataModel.submitBatch(function(oResult) {
				try {
					...
					if (selctedItem.whichtab == "FSURR") {
						...
						commentsList.setModel(commentModel);
						// Attachments
						///// UCD1KOR Possport copy added 12 Apr 2021
						var passportCopy = oResult.__batchResponses[15].data.results;
						if(passportCopy.length !== 0){
							filesAll.unshift(passportCopy[passportCopy.length-1]);
						}
						filesAll = filesAll.concat(oResult.__batchResponses[4].data.results, oResult.__batchResponses[5].data.results, oResult.__batchResponses[14].data.results,oResult.__batchResponses[16].data.results);
						var uploadModel = oForex.getView().getModel("new");
						uploadModel.setData({
							Files : filesAll
						});
						...
						// Attachments
						///// UCD1KOR Possport copy added  12 Apr 2021
						var passportCopy = oResult.__batchResponses[10].data.results;
						if(passportCopy.length !== 0){
							filesAll.unshift(passportCopy[passportCopy.length-1]);
						}
						filesAll = filesAll.concat(oResult.__batchResponses[4].data.results, oResult.__batchResponses[5].data.results, oResult.__batchResponses[9].data.results, oResult.__batchResponses[11].data.results);
						var uploadModel = oForex.getView().getModel("new");
						uploadModel.setData({
							Files : filesAll
						});
...
//						}
	...
	/////// UCD1KOR 20/05/2021 here we have to add one more service for FEMA for Forex and card reload tabs
		if(selctedItem.Tab == "CARD" || selctedItem.Tab == "FINF"){
			var passportNo = this.getView().getModel("passport").getData().PassportNo;
			var service = "FEMADeclarPdf?Empno='" + selctedItem.EmpNo + "'&CardNo=''&Curr=''&CashAmt=''&ToCntry=''&PsprtNo=''&Remarks=''&TpNo='"+tpno+"'&Version='"+selctedItem.Version+"'&Module=''&DisEdit=''&$format=json";
			//var service = "FEMADeclarPdf?Empno='" + selctedItem.EmpNo + "'&CardNo=''&Curr=''&Amt=''&ToCntry=''&PsprtNo=''&Remarks=''&Version='2/FREX/08'&TpNo='"+tpno+"' "
			oDataModel.read(service, null, null, true, function(oData, response) {
				var data = JSON.parse(response.body);
				fileList = oForex.getView().getModel("fixedAttachments").getData();
				if(data.d.FEMADeclarPdf.DocPath !== ""){//FEMA_TPNo_EMPNo
					var fema = {"FileName":"FEMA_"+tpno+"_"+selctedItem.EmpNo+".pdf","FileUrl":data.d.FEMADeclarPdf.DocPath};
					fileList.push(fema);
				}
				oForex.getView().getModel("fixedAttachments").setData(fileList);
			}, function(error) {
				sap.m.MessageBox.error("FEMA Attachments are not available..!");
			});
		}
		
	},
	//////// UCD1KOR 22/05/2021 FEMA Creation
	CreateFEMA:function(selctedItem){
		var tpno = "00" + selctedItem.TravelPlan;
		var carddetails = selctedItem.ForexCardSet;
		var cashdetails = selctedItem.ForexCashSet;
		var cardNo = "";
		var curr = "";
		var amount = "";
		var Genaral= this.getView().getModel("general").getData();
		//var version =  selctedItem.Version !== undefined ?  selctedItem.Version.trim(): "";
		for(var i=0;i<carddetails.length;i++){
			cardNo = cardNo + ",card/"+carddetails[i].CardNo.trim();
			curr = curr +",card/"+carddetails[i].Currency.trim(); 
			amount = amount+",card/"+ carddetails[i].Amount.toString().trim();
		}
		for(var i=0;i<cashdetails.length;i++){
			amount = amount+",cash/"+ cashdetails[i].Cash;
			curr = curr +",cash/"+cashdetails[i].Currency.trim(); 
			cardNo = cardNo + ",cash/X";
		}
		...
		//FEMADeclarPdf?Empno='30249712'&CardNo='card/lasdjsk,card/lasdjsk,card/lasdjsk,card/lasdjsk,card/lasdjsk'&Curr='card/INR,cash/12.00,card/INR,card/INR,card/INR'&CashAmt='card/11.00,card/11.00,card/11.00,card/11.00,card/11.00'&ToCntry='Germany'&PsprtNo='K3415385'&Remarks=''&TpNo='0010109794'&Version=''&Module='FREX'&DisEdit='X' 
		var service = "FEMADeclarPdf?Empno='" + selctedItem.EmpNo + "'&CardNo='"+cardNo+"'&Curr='"+curr+"'&CashAmt='"+amount+"'&ToCntry='"+Genaral.ToCountryText+"'&PsprtNo='"+PassportNo+"'&Remarks='"+comments+"'&TpNo='"+tpno+"'&Version='"+Genaral.Version+"'&Module='"+selctedItem.Module+"'&DisEdit='X'&$format=json";
		var oDataModel = new sap.ui.model.odata.ODataModel(sServiceUrl);
		oDataModel.read(service, null, null, true, function(oData, response) {
			var data = JSON.parse(response.body);
			sap.m.MessageToast.show("FEMA created ..!!");	
		}, function(error) {
			sap.m.MessageBox.error("FEMA Creation Failed ..!!");
		});
	},
	...
	setFieldProperties : function(key) {
		...
		//oForex.getView().byId("btnAddNewCard").setVisible(false); commented by ucd1kor
		...
		if (role == "FEX") {
			if (key == "ISSUE") {
				...
				//oForex.getView().byId("btnAddNewCard").setVisible(true); commented by ucd1kor
			} else if (key == "CLOSE") {
				properties = {
					savvisible : false,
					editable : false,
	...
	onItemPress : function(evt) {
		...
		if (evt.getParameter("listItem").getAggregation("cells")) {
			...
			if (key == "NEW" || key == "NGREE") {
				...
				////################################ UCD1KOR Changes For Forex Surrender Amount 21 Jan 2020 ##############///////
				that.promise1 = jQuery.Deferred();
				var model = new sap.ui.model.json.JSONModel();
				model.setData(selctedItem);
				sap.ui.getCore().setModel(model, "forexitem");
				if(subkey === "NEW"){
					this.ForexSurrendarDataModelSetup();
					this.fetchDetails(selctedItem);
					$.when(that.promise1).done(
							function () {
								that.putServicecall();
					});
	...
	//###################### UCD1KOR Changes For Forex Surrender Amount 21 Jan 2020 ##############///////
	ForexSurrendarDataModelSetup:function(){
		var model = new sap.ui.model.json.JSONModel();
		var data = {};
		data.travel = {
			expenses : [],
		...
		data.currtotal = [];
		model.setData(data);
		this.getView().setModel(model, "travelsettlement");
	},
	fetchDetails : function(selctedItem) {
		var oThis = this;
		sap.ui.project.e2etm.util.StaticUtility.setBusy(true, oForex);
		var currentRole = sap.ui.getCore().getModel("profile").getData().currentRole;
		var global = sap.ui.getCore().getModel("global").getData();
		try{
			var version = selctedItem.Version.trim();
		}catch(err){
			var version = ""
		}
		/*if (selctedItem.TravelType == "SECO" && (currentRole != "TRST" && currentRole != "TRSD")) {
			selctedItem.TravelPlan = selctedItem.ZZ_DEP_REQ;
		}*/
		var getExpenses = oDataModel.createBatchOperation("TrvStdetailsSet?$filter=EmpNo+eq+'" + selctedItem.EmpNo + "'+and+TravelPlan+eq+'" + selctedItem.TravelPlan + "'+and+Version+eq+'" + version + "'+and+TravelType+eq+'" + selctedItem.TravelType + "'+and+LoginRole+eq+'" + currentRole + "'+and+Module+eq+'TRST'+and+Item+eq+'1'", "GET");
		var getTotal = oDataModel.createBatchOperation("TrvStTotalSet?$filter=EmpNo+eq+'" + selctedItem.EmpNo + "'+and+TravelPlan+eq+'" + selctedItem.TravelPlan + "'+and+TravelType+eq+'" + selctedItem.TravelType + "'+and+LoginRole+eq+'TRST'", "GET");
		var general = oDataModel.createBatchOperation("TrvStGenDataSet(EmpNo='" + selctedItem.EmpNo + "',TravelPlan='" + selctedItem.TravelPlan + "',Version='" + version + "',TravelType='" + selctedItem.TravelType + "',LoginRole='" + currentRole + "',Module='TRST',Item='1')?$expand=TRV_COST_ASGNSet", "GET");
		var getCurr = oDataModel.createBatchOperation("GetF4Help?Srch_help='F4_TCURC_ISOCD'&$format=json", "GET");
		oDataModel.addBatchReadOperations([getExpenses, getTotal,general,getCurr]);
		oDataModel.submitBatch(function(oResult) {
			try {
				oThis.setExpensesValues(oResult.__batchResponses[0].data.results);
				var data = oThis.getView().getModel("travelsettlement").getData();		
				data.traveldetails = oResult.__batchResponses[2].data;
				// Currency
				var curModel = new sap.ui.model.json.JSONModel();
				curModel.setSizeLimit(oResult.__batchResponses[0].data.results.length);
				curModel.setData(oResult.__batchResponses[3].data.results);
				oThis.getView().setModel(curModel, "curModel");
				
				var totalmodel = new sap.ui.model.json.JSONModel();
				totalmodel.setData(oResult.__batchResponses[1].data.results);
				oThis.getView().setModel(totalmodel, "totalModel");
				oThis.setFooter();
				//sap.ui.project.e2etm.util.StaticUtility.setBusy(false, oForex);
				oThis.promise1.resolve();
			} catch (err) {
				sap.ui.project.e2etm.util.StaticUtility.setBusy(false, oForex);
			}
		});
	...
	setExpensesValues:function(results){
		...
		this.getView().getModel("travelsettlement").setData(data);
	},
	setFooter:function(){
		var oThis = this;
		var data = oThis.getView().getModel("travelsettlement").getData();
...
//	}
	...
	putServicecall:function(){
		...
		try {
			for ( var prop in data) {
				...
				switch (prop) {
				case "travel":
					oData.TrvStForexDetailSet = oThis.appendRows(data[prop], data.traveldetails, oData.TrvStForexDetailSet, "TRVL", btype, prop);
					break;
				case "lodging":
					oData.TrvStForexDetailSet = oThis.appendRows(data[prop], data.traveldetails, oData.TrvStForexDetailSet, "LODG", btype, prop);
					break;
				case "boarding":
					oData.TrvStForexDetailSet = oThis.appendRows(data[prop], data.traveldetails, oData.TrvStForexDetailSet, "BORD", btype, prop);
					break;
				case "conv":
					oData.TrvStForexDetailSet = oThis.appendRows(data[prop], data.traveldetails, oData.TrvStForexDetailSet, "CONV", btype, prop);
					break;
				case "entr":
					oData.TrvStForexDetailSet = oThis.appendRows(data[prop], data.traveldetails, oData.TrvStForexDetailSet, "ENTA", btype, prop);
					break;
				case "cargo":
					oData.TrvStForexDetailSet = oThis.appendRows(data[prop], data.traveldetails, oData.TrvStForexDetailSet, "CARG", btype, prop);
					break;
				case "other":
					oData.TrvStForexDetailSet = oThis.appendRows(data[prop], data.traveldetails, oData.TrvStForexDetailSet, "OTHC", btype, prop);
					break;
				case "advance":
					
					oData.TrvStForexDetailSet = oThis.appendRows(data[prop], data.traveldetails, oData.TrvStForexDetailSet, "ADVC", btype, prop);
					break;
				case "credit":
					oData.TrvStForexDetailSet = oThis.appendRows(data[prop], data.traveldetails, oData.TrvStForexDetailSet, "CRED", btype, prop);
					break;
				}
			}
			oData.TrvStForexTotalSet = [];
			for ( var i = 0; i < data.currtotal.length; i++) {
				oData.TrvStForexTotalSet.push({
			...
			if (oData.TrvStForexTotalSet.length == 0) {
				oData.TrvStForexTotalSet.push({});
			}
			 /*//oDataModel1.setUseBatch(false); 
			 oDataModel1.setHeaders({
				 'DataServiceVersion': "2.0"
			 }); */
			oDataModel1.create("TrvstGenForexAdvanceSet", oData, null, function(oData, response) {
				sap.ui.project.e2etm.util.StaticUtility.setBusy(false, oForex);
				sap.m.MessageToast.show("Forex Surrender Amount Updated ..!");
				sap.ui.core.routing.Router.getRouter("MyRouter").navTo("forex");
			}, function(error) {
				sap.ui.project.e2etm.util.StaticUtility.setBusy(false, oForex);
				sap.m.MessageToast.show("Forex Surrender Amount not Updated..!");
			}, true);
		} catch (exc) {
			sap.ui.project.e2etm.util.StaticUtility.setBusy(false, oForex);
			sap.m.MessageToast.show("Forex Surrender Amount not Updated..!");
	...
	//////////####### UCD1KOR Changes Done - Update Forex Surrender Amount ##################///////////////////
	...
	onIconTabBarAdminSelect : function(evt) {
		...
		var role = sap.ui.getCore().getModel("profile").getData().currentRole;
		oForex.getView().getModel("pepayroll").getData().role = role; //added by uml6kor_9/9/2020 for paging
...
//			this.getWorklistDetails("NEW", evt.getParameter("selectedItem").getContent()[0]);
			this.getWorklistDetails("NEW", evt.getParameter("selectedItem").getContent()[0],'0','40');//added top,skip , code added by uml6kor_9/9/2020 for paging
			break;
		case "NGREE":
			this.getWorklistDetails("NOTG", evt.getParameter("selectedItem").getContent()[0],'0','40');//added top,skip , code added by uml6kor_9/9/2020 for paging
			break;
		case "CARD":
			oForex.getView().getModel("pepayroll").getData().bankvisible = true;
			this.getWorklistDetails("CARD", evt.getParameter("selectedItem").getContent()[0],'0','40');//added top,skip , code added by uml6kor_9/9/2020 for paging
			break;
		case "ISSUE":
			this.getWorklistDetails("ISSF", evt.getParameter("selectedItem").getContent()[0],'0','40');//added top,skip , code added by uml6kor_9/9/2020 for paging
			break;
		case "SBFT":
			this.getWorklistDetails("FISB", evt.getParameter("selectedItem").getContent()[0],'0','40');//added top,skip , code added by uml6kor_9/9/2020 for paging
			break;
		case "CLOSE":
			oForex.getView().getModel("pepayroll").getData().role = "close"; ////added by uml6kor_9/9/2020 for paging
			...
			break;
		case "CANC":
			this.getWorklistDetails("CANC", evt.getParameter("selectedItem").getContent()[0],'0','40');
			break;
		case "FSURR":
			// sap.ui.project.e2etm.util.StaticUtility.setBusy(true, oForex);
	...
	onSurrTabSelect : function(evt) {
		...
		switch (key) {
		case "NEW":
			this.getWorklistDetails("SRNW", evt.getParameter("selectedItem").getContent()[0],'0','40');//added top,skip ,code added by uml6kor_9/9/2020 for paging
			break;
		case "SUR":
			this.getWorklistDetails("SRSR", evt.getParameter("selectedItem").getContent()[0],'0','40');//added top,skip ,code added by uml6kor_9/9/2020 for paging
			break;
		case "PSUR":
			this.getWorklistDetails("SRPR", evt.getParameter("selectedItem").getContent()[0],'0','40');//added top,skip ,code added by uml6kor_9/9/2020 for paging
			break;
		case "SEBT":
//			this.getWorklistDetails("SRSB", evt.getParameter("selectedItem").getContent()[0]);
			this.getWorklistDetails("SRSB", evt.getParameter("selectedItem").getContent()[0],'0','40');//added top,skip , code added by uml6kor_9/9/2020 for paging
			break;
		}
	},
	getWorklistDetails : function(tab, table, skip, top) { //added top,skip ,code added by uml6kor_9/9/2020 for paging
//		getWorklistDetails : function(tab, table) {
		...
		var oPagingParams = "$skip="+skip+"&$top="+top;
		oDataModel.read("ForexWorklistSet?$filter=LoginRole eq 'FEX' and Tab eq '" + tab + "'", null, oPagingParams, true,
         // success
		function(oData, response) {
			var results = oForex.getTabWorklist(oData.results, tab);
			var worklistModel = new sap.ui.model.json.JSONModel();
			//start of code added by uml6kor_9/9/2020 for paging
			if( results.length == 40 && oForex.clicks == 0){
				sap.ui.getCore().getModel("global").getData().nxtbtn = true;
					sap.ui.getCore().getModel("global").getData().prevbtn = false;	
					sap.ui.getCore().getModel("global").refresh();	
			...
			if(( results.length < 40 || results.length == 0 ) && oForex.clicks == 0 ){
				sap.ui.getCore().getModel("global").getData().nxtbtn = false;
				sap.ui.getCore().getModel("global").getData().prevbtn = false;	
				sap.ui.getCore().getModel("global").refresh();
			}else if(( results.length < 40 || results.length == 0 ) && oForex.clicks > 0 ){
				sap.ui.getCore().getModel("global").getData().nxtbtn = false;
				sap.ui.getCore().getModel("global").getData().prevbtn = true;	
				sap.ui.getCore().getModel("global").refresh();
			}else if( results.length >= 40  && oForex.clicks > 0 ){
				sap.ui.getCore().getModel("global").getData().nxtbtn = true;
				sap.ui.getCore().getModel("global").getData().prevbtn = true;	
				sap.ui.getCore().getModel("global").refresh();	
			}
...
//		this.onExport();
	...
	//############ UCD1KOR Jan 05,2021 Start################### ////
	onExistingCards:function(){
		var generaldata = oForex.getView().getModel("general").getData();
		oForex.fetchCardsAll(generaldata.EmpNo);
		var selectedTab = sap.ui.getCore().getModel("forexitem").getData().whichtab;
		var oView = this.getView();
		/*if(selectedTab == "CARD"){
			oView.byId("idUpdateCardButton").setVisible(true);
		}*/
		var onExistingCardsDialog = oView.byId("idExistingCards");		
		if (!onExistingCardsDialog) {		
			onExistingCardsDialog = sap.ui.xmlfragment(oView.getId(),"sap.ui.project.e2etm.fragment.forex.ExistingCards", this);		
			oView.addDependent(onExistingCardsDialog);
		}
		onExistingCardsDialog.open();
		
	},
	onCloseExistingCards:function(){
		this.getView().byId("idExistingCards").close();	
	},
	onDelimit : function(evt) {
		var that = this;
		var carddata = [];
		var oData = {};
		var rows = [];
		...
		if (data.whichtab != "FSURR") {
			...
			table = oForex.getView().byId("idExistingCardsTable");
			//carddata = table.getModel().getData();
			//rows = table.getSelectedIndices();
			carddata = oForex.getView().getModel("forexmodel").getData().cardsAll;
			rows = evt.getSource().getParent().getParent().getParent().getContent()[0].getSelectedContextPaths();
			...
			if (rows.length > 1 || rows.length == 0) {
				...
				sap.ca.ui.dialog.confirmation.open({
					...
					if (oResult.isConfirmed) {
						...
						oData.IssuedBanker = carddata[parseInt(rows[0].split("/")[2])].IssuedBanker;
						oData.CardNo = carddata[parseInt(rows[0].split("/")[2])].CardNo;
						oData.CardValidUpto = carddata[parseInt(rows[0].split("/")[2])].CardValidUpto;
						oData.Currency = carddata[parseInt(rows[0].split("/")[2])].Currency;
						oDataModel.create("ForexCardsAllSet", oData, null, function(oData, response) {
							...
							if (data.whichtab != "FSURR") {
								...
								/*table.getModel().setData(carddata); commented by UCD1KOR
								table.setVisibleRowCount(carddata.length);*/
								////////########## UCD1KOR added below lines
								oForex.getView().getModel("forexmodel").getData().cardsAll = carddata;
								oForex.getView().getModel("forexmodel").refresh();
							...
							oForex.getView().byId("idExistingCardsTable").removeSelections()
							//that.getView().byId("idExistingCards").close();	
							oForex.fetchCardsAll(generaldata.EmpNo);
						}, function(error) {
							sap.ui.project.e2etm.util.StaticUtility.setBusy(false, oForex);
	...
	fetchCardsAll : function(empno) {
		oDataModel.read("ForexCardsAllSet?$filter=EmpNo eq '" + empno + "'", null, null, true, function(oData, response) {
			...
			/////################# UCD1KOR Jan 5th 21 #################################////
			/// Added below lines  start
			oForex.getView().byId("idExistingCardsTable").setModel();
			oForex.getView().getModel("fieldproperties").getData().existingcards = false;
			oForex.getView().getModel("fieldproperties").refresh();
			//// End
			sap.ui.project.e2etm.util.StaticUtility.setBusy(false, oForex);
	...
	saveForexDetails : function(action, role) {
		...
		try {
				...
				if(results.ForexCardSet.length==0)
					results["ForexCardSet"].push({});
				this.FemaData = results;
		
			oDataModel.create("ForexTransSet", results, null, function(oData, response) {
				// oForex.uploadFiles(gdata.TravelPlan);
				// UCD1KOR 22 May 2021
				if(action == "14" && role == "08"){
					oForex.CreateFEMA(that.FemaData );
				}
				sap.ui.project.e2etm.util.StaticUtility.setBusy(false, oForex);
				window.history.go(-1);
				...
				sap.ui.project.e2etm.util.StaticUtility.setBusy(false, oForex); 
				sap.m.MessageToast.show("Internal Server Error");
			},  false); ////async:true modified to false uml6kor_upgrade 4/12/2019
		} catch (exc) {
			sap.ui.project.e2etm.util.StaticUtility.setBusy(false, oForex);
	...
	onDelimitCard:function(evt){
		oForex.onDelimit(evt);
	},
	onUpdateCard:function(evt){
		///############# UCD1KOR Changes###############////////////
		//var table = oForex.getView().byId("cardtable");
		//rows = table.getSelectedIndices();
		var table = oForex.getView().byId("idExistingCardsTable");
...
//		
		...
		oDataModel.update("ForexCardsAllSet(EmpNo='"+generaldata.EmpNo+"')", oData, null, jQuery.proxy(function(oData, response) {
			...
			//oForex.updateCardTable(); commented by UCD1KOR
	...
	onMassFileUpload:function(){
		...
		//oData[nav] = [];
		 ...
		 reader.onload = function(e) {
			...
			},this),async:false}); //async:true modified to false uml6kor_upgrade 4/12/2019
...
//*********************************start of code for paging 9/9/2020 uml6kor***********************************
	onPrevious : function(evt){
		...
		var skipCount = this.clicks * 40;
//		if (skipCount <= this.initialCount - 40) {
//			sap.ui.getCore().getModel("global").getData().nxtbtn = true;
//		}
		sap.ui.getCore().getModel("global").refresh();
		var tab = this.getTabKeyValue();
		this.getWorklistDetails(tab, evt.getSource().getParent().getParent().mAggregations.content[0],skipCount,top);
	},
	onFirst : function(evt){
		sap.ui.getCore().getModel("global").getData().prevbtn = false;
		sap.ui.getCore().getModel("global").getData().nxtbtn = true;
		sap.ui.getCore().getModel("global").refresh();
		var skipCount = 0;
...
//*********************************end of code for paging 9/9/2020 uml6kor***********************************
