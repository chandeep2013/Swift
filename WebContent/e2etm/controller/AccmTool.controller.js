...
*/
		  ...
		  this.onTabSelect();
			...
			case "reservation":
				if(icontab.getSelectedKey()!="ADVR")
					...
					//Thanh 14.05.2021 set value if navigate from travel plan
					this._fromTravelPlan = oArgs["?query"] ? oArgs["?query"].travelPlan : null;
				break;
			case "manualtp":
	...
	tabsVisibleBasedOnRole:function(){
		...
		//Thanh 14.05.2021 if navigating is from travel plan
		var isVisibleTabs = this._fromTravelPlan === "true" ? false : true;
		var role = sap.ui.getCore().getModel("profile").getData().currentRole; /// ucd1kor added below condition
		if(role =="EMP"){
			isVisibleTabs = false;
			this.getView().byId("tabAdmin").setText("Accomodation");
		}
		else{
			this.getView().byId("tabAdmin").setText("Administrator");
		}
		var oModelAccom = new sap.ui.model.json.JSONModel({
			isNotFromTravelPlan: isVisibleTabs
		});
		this.getView().setModel(oModelAccom, "AccomodationStatus");
		this._fromTravelPlan = null;
	},
