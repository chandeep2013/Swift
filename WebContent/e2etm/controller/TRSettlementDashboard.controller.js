...
sap.ui.controller("sap.ui.project.e2etm.controller.TRSettlementDashboard", {
	...
	onItemPress : function(event) {
		if (event.getParameter("listItem").getAggregation("cells")) {
			...
			if(bindingItem[index].Digitalize =="X" || bindingItem[index].Digitalize == "x"){
				global.DigiFlag = "X";
			}
			else{
				global.DigiFlag = "";//ucd1kor 24/3/2021 issue fix
			}
	...
	onIconTabSelectAdmin : function(evt) {
		...
		this.clicks = 0;//start of code for paging 9/9/2020 uml6kor
		var content;
		if (key == "DOME") {
			this.intltotalvisble = false;
...
//				oController.getDetails(table, "TRST", tab);
				oController.getDetails(table, "TRST", tab, '0', '40');//start of code for paging 9/9/2020 uml6kor
			}
		} else if (key == "INTL") {
			this.intltotalvisble = true;
			...
			if (tab) {
				var table = this.getTable(content, content.getSelectedKey());
//				oController.getDetails(table, "TRST", tab);//start of code for paging 9/9/2020 uml6kor
				oController.getDetails(table, "TRST", tab,'0', '40');
				}
		} else if (key == "PYVR") {
			...
			content.getItems()[7].setVisible(false);
			content.getItems()[8].setVisible(false);
			content.getItems()[9].setVisible(true); //UCD1KOR 08/06/2021 Accounting section enable
			content.getItems()[10].setVisible(true);
			var tab = this.getTabValue(content.getSelectedKey(), key);
			if (tab) {
				var table = this.getTable(content, content.getSelectedKey());
//				oController.getDetails(table, "PYVR", tab);//start of code for paging 9/9/2020 uml6kor
				oController.getDetails(table, "PYVR", tab,'0', '40');
			...
			if (tab) {
				var table = this.getTable(content, content.getSelectedKey());
//				oController.getDetails(table, "INRA", tab);//start of code for paging 9/9/2020 uml6kor
				oController.getDetails(table, "INRA", tab, '0', '40');
				
			}
		}
		/*############### UCD1KOR 27 Jan 2020 Replace text Received Date to Approved Date  Start####################*/
		var selecttab = this.getView().byId("icontabbaradmin");
		selecttab.getItems()[0].getContent()[0].getItems()[0].getContent()[0].getColumns()[4].mAggregations.header.mAggregations.items[1].mProperties.text = "Approved Date";
		selecttab.getItems()[1].getContent()[0].getItems()[0].getContent()[0].getColumns()[4].mAggregations.header.mAggregations.items[1].mProperties.text = "Approved Date";
		
		selecttab.getItems()[2].getContent()[0].getItems()[0].getContent()[0].getColumns()[5].mProperties.visible = false
		selecttab.getItems()[3].getContent()[0].getItems()[0].getContent()[0].getColumns()[5].mProperties.visible = false
		
		/*############### UCD1KOR 27 Jan 2020 Replace text Received Date to Approved Date  End#####################*/
		selecttab.getItems()[1].getContent()[0].getItems()[0].getContent()[0].getColumns()[6].mProperties.visible = false;
		selecttab.getItems()[2].getContent()[0].getItems()[0].getContent()[0].getColumns()[6].mProperties.visible = false;
		selecttab.getItems()[3].getContent()[0].getItems()[0].getContent()[0].getColumns()[6].mProperties.visible = false;

		this.hideAcknowledment();
	},
	hideAcknowledment:function(){
		/*############### UCD1KOR 27 Jan 2020 Hide Acknowledgement Column  Start####################*/
		var selecttab = this.getView().byId("icontabbaradmin");
		var iconTabLen = selecttab.getItems();
		for(var i=0;i<4;i++){
			var itemLen = iconTabLen[i].getContent()[0].getItems();
			for(var j=1; j<itemLen.length;j++){
				try{
					itemLen[j].getContent()[0].getColumns()[5].mProperties.visible = false;
					itemLen[j].getContent()[0].getColumns()[6].mProperties.visible = false;
				}catch(err){}
				
			}
		}
		/*############### UCD1KOR 27 Jan 2020 Hide Acknowledgement Column  End#####################*/
	},
	onMonthTabSelect : function(evt) {
		...
		if (childKey != 'CLSR') {
			...
			oController.getDetails(table, module, tab, '0', '40');//start of code for paging 9/9/2020 uml6kor
			...
			if(parentKey == "INRA" && childKey =="APP"){
				this.getView().byId("btnAccount").setVisible(false);  //// ucd1kor 24/06/2021 hide account button for INRA-Approve
				table.setMode("None"); //// ucd1kor 24/06/2021 change mode MultiSelect to None
			}
		}
		this.hideAcknowledment();
	...
	getTabValue : function(key, parentKey) {
		...
		if (parentKey == "DOME") {
			...
			case "APP":
				tab = "PVA";
				break;
			case "ACCT":  //// UCD1KOR added below section for Payment Voucher Accounting - need to check
				tab = "PAA";
				break;
			case "PAYB":
				tab = "PVP";
	...
	getDetails : function(table, module, tab, skip, top) {
		sap.ui.project.e2etm.util.StaticUtility.setBusy(true, oController);
		var role = sap.ui.getCore().getModel("profile").getData().currentRole;
		var oPagingParams = "$skip="+skip+"&$top="+top;
		oDataModel.read("TrvStWorklistSet?$filter=LoginRole eq '" + role + "' and Module eq '" + module + "' and Tab eq '" + tab + "'", null, oPagingParams, false, 
			function(oData, response) {
			//start of code added by uml6kor_9/9/2020 for paging
			if( oData.results.length == 40 && oController.clicks == 0){
				sap.ui.getCore().getModel("global").getData().nxtbtn = true;
					sap.ui.getCore().getModel("global").getData().prevbtn = false;	
					sap.ui.getCore().getModel("global").refresh();	
				}
			if(( oData.results.length < 40 || oData.results.length == 0 ) && oController.clicks == 0 ){
				sap.ui.getCore().getModel("global").getData().nxtbtn = false;
				sap.ui.getCore().getModel("global").getData().prevbtn = false;	
				sap.ui.getCore().getModel("global").refresh();
			}else if(( oData.results.length < 40 || oData.results.length == 0 ) && oController.clicks > 0 ){
				sap.ui.getCore().getModel("global").getData().nxtbtn = false;
				sap.ui.getCore().getModel("global").getData().prevbtn = true;	
				sap.ui.getCore().getModel("global").refresh();
			}else if( oData.results.length >= 40  && oController.clicks > 0 ){
				sap.ui.getCore().getModel("global").getData().nxtbtn = true;
				sap.ui.getCore().getModel("global").getData().prevbtn = true;	
				sap.ui.getCore().getModel("global").refresh();	
			}
	...
	getTravelFilter : function(key, parentKey) {
		...
		if (parentKey == "DOME") {
			...
			case "APP":
				...
				aFilters.push(oFilter);
				break; 
			case "ACCT":  //// UCD1KOR 08/06/2021 Added below Code for accounting
				oFilter = new sap.ui.model.Filter("Tab", "EQ", "PAA");
				aFilters.push(oFilter);
				break;	
			case "PAYB":
	...
	onSearchWorklist : function(evt) {
		...
		case "recdate":
			...
			break;
			///UCD1KOR Added <!-- added  change RBEI-197 filter with date range travel settlement 8/9/2020 ucd1kor -->
		case "ackdate":
			oFilter = new sap.ui.model.Filter("Ackndt", "Contains", evt.oSource.getValue());
			aFilters.push(oFilter);
			travelreq = sap.ui.getCore().byId(bid[0] + "--" + bid[1] + "--" + bid[2] + "--trvlreq").getValue();
			empname = sap.ui.getCore().byId(bid[0] + "--" + bid[1] + "--" + bid[2] + "--empname").getValue();
			empno = sap.ui.getCore().byId(bid[0] + "--" + bid[1] + "--" + bid[2] + "--empno").getValue();
	...
	/////############ UCD1KOR Live change for FilerDates ###################//////////<!-- added change RBEI-197 filter with date range travel settlement 8/9/2020 ucd1kor -->
	onLiveChange:function(evt){
		var that= this;
		this.globalFilterId = evt.getSource().getId();
		var bid = this.globalFilterId.split("--");
		this.filterBid = bid;
		var table = evt.getSource().getParent().getParent().getParent();
		this.globalTable = table;
		sap.ui.getCore().getModel("global").getData().FilterStartDate = "";
		sap.ui.getCore().getModel("global").getData().FilterEndDate = "";
		sap.ui.getCore().getModel("global").refresh();
		if (!that._FilterWithDatesDialog) {
			that._FilterWithDatesDialog = sap.ui.xmlfragment("sap.ui.project.e2etm.fragment.travelsettlement.FilterDate", that);
			that.getView().addDependent(that._AdvanceoDialog);
		}
		that._FilterWithDatesDialog.open();
	},
	onClosePopup:function(){
		this._FilterWithDatesDialog.close();
	},
	////############## UCD1KOR FilterWith Date Range ##############//////
	onPressProceed:function(){
		var FStart = sap.ui.getCore().getModel("global").getData().FilterStartDate;
		var FEnd = sap.ui.getCore().getModel("global").getData().FilterEndDate ;
		if(FStart == "" || FEnd == ""){
			var aFilters = [];
			var bindItems = this.globalTable.getBinding("items");
			bindItems.filter(aFilters);
			sap.ui.getCore().byId(this.globalFilterId).setValue("");
			this._FilterWithDatesDialog.close();
			//sap.m.MessageBox.error("Enter Start date and End date");
			sap.m.MessageToast.show("No filter Applied..!!");
		}
		else if(FStart > FEnd){
			sap.m.MessageBox.error("Start date should be less than End date");
		}
		else{
			sap.ui.getCore().byId(this.globalFilterId).setValue(FStart.substring(6,8)+"/"+FStart.substring(4,6)+"/"+FStart.substring(0,4) +"-"+ FEnd.substring(6,8)+"/"+FEnd.substring(4,6)+"/"+FEnd.substring(0,4));
			sap.ui.getCore().byId(this.globalFilterId).setTooltip(FStart.substring(6,8)+"/"+FStart.substring(4,6)+"/"+FStart.substring(0,4) +"-"+ FEnd.substring(6,8)+"/"+FEnd.substring(4,6)+"/"+FEnd.substring(0,4));
			
			var aFilters = [];
			if(this.filterBid[3] == "recdate"){
				var oFilter = new sap.ui.model.Filter("ReceivedDt", "BT", FStart,FEnd);
			}
			else if(this.filterBid[3] == "ackdate"){
				var oFilter = new sap.ui.model.Filter("Ackndt", "BT", FStart,FEnd);
			}
			
			aFilters.push(oFilter);
			var travelreq = sap.ui.getCore().byId(this.filterBid[0] + "--" + this.filterBid[1] + "--" + this.filterBid[2] + "--trvlreq").getValue();
			var empname = sap.ui.getCore().byId(this.filterBid[0] + "--" + this.filterBid[1] + "--" + this.filterBid[2] + "--empname").getValue();
			var empno = sap.ui.getCore().byId(this.filterBid[0] + "--" + this.filterBid[1] + "--" + this.filterBid[2] + "--empno").getValue();
			var trvltype = sap.ui.getCore().byId(this.filterBid[0] + "--" + this.filterBid[1] + "--" + this.filterBid[2] + "--trvltype").getValue();
			if (travelreq != "") {
				aFilters = oController.addFilter("TravelPlan", travelreq, aFilters);
			}
			if (empname != "") {
...
//			this.getListAdmin(sRole, searchString, "REIM", sAction, this, "NEW");//commented code for paging 9/9/2020 uml6kor
			this.getListAdmin(sRole, searchString, "REIM", sAction, this, "NEW", top ,skip);//added code for paging 9/9/2020 uml6kor
			break;
		// CLOSED
		case "PAYMENT":
			this.getView().byId("btnSendMail").setVisible(true);
			...
			sap.ui.project.e2etm.util.StaticUtility.setBusy(true, this);
			//this.getView().byId("idColTravelReq").setText("Travel Request");
			this.getListAdmin(sRole, searchString, "REIM", sAction, this, "PAYMENT", top ,skip);//added code for paging 9/9/2020 uml6kor
			this.getListAdmin(sRole, searchString, "REIM", sAction, this, "PAYMENT");///commented code for paging 9/9/2020 uml6kor
			break;
		case "CLOSED":
			var sRole = '11';
			var sAction = '00';
			...
			if (searchString == "") {
				...
				this.getListAdmin(sRole, searchString, "REIM", sAction, this, "CLOSED", top ,skip);//added code for paging 9/9/2020 uml6kor
//				this.getListAdmin(sRole, searchString, "REIM", sAction, this, "CLOSED"); //commented code for paging 9/9/2020 uml6kor
			}
			break;
		}// end switch
	},
	// get list of Reimbursement request
//	getListAdmin : function(sRole, ZZ_OWNER, sModid, sAction, oThis, sStatus) { //commented code for paging 9/9/2020 uml6kor
	getListAdmin : function(sRole, ZZ_OWNER, sModid, sAction, oThis, sStatus, top, skip){
		// ZZ_TRV_REQ =ROLE
		// ZZ_OWNER = OWNER
		// ZZ_TRV_KEY = SMODID
		// ZZ_VERSION = ACTION
		// WAERS = type of admin
		var oPagingParams = "&$skip="+skip+"&$top="+top; //added code for paging 9/9/2020 uml6kor

		var sAccUrl = "/ZE2E_REI_HDRSet?$filter=ZZ_TRV_REQ+eq+'{0}'+and+ZZ_OWNER+eq+'{1}'+and+ZZ_TRV_KEY+eq+'{2}'+and+ZZ_VERSION+eq+'{3}'+and+WAERS+eq+'{4}'+and+ZZ_VISA_PLAN+eq+'{5}'&$expand=TRV_HDR,ZE2E_REI_DETAIL"+ oPagingParams;
//		var sAccUrl = "/ZE2E_REI_HDRSet?$filter=ZZ_TRV_REQ+eq+'{0}'+and+ZZ_OWNER+eq+'{1}'+and+ZZ_TRV_KEY+eq+'{2}'+and+ZZ_VERSION+eq+'{3}'+and+WAERS+eq+'{4}'+and+ZZ_VISA_PLAN+eq+'{5}'&$expand=TRV_HDR,ZE2E_REI_DETAIL";//commented code for paging 9/9/2020 uml6kor
		// var sAccUrl =
	...
	onAccounting : function(evt) {
		...
		if (key == "DOME")
			...
			table = this.getView().byId(sap.ui.core.Fragment.createId("payment", sap.ui.core.Fragment.createId("acct", "accWorklist")));
		else if (key == "INRA")
			table = this.getView().byId(sap.ui.core.Fragment.createId("inra", sap.ui.core.Fragment.createId("acct", "accWorklist")));
		...
		/////////################## UCD1KOR 15 June 2021 Added below code ##################/////////
		if(key == "DOME"||key == "INTL"){
			var modid = "TRST";
		}
		else{
	...
	///################# UCD1KOR 08/06/2021 actual post
	...
	callFunctionImport:function(odata,reqtype,btnLog,dialog){
		...
		this["ipData"] = odata;
		///################# UCD1KOR 08/06/2021 actual post for PYVR,TRST
		/*if(odata.module == "PYVR" || odata.module == "TRST"){
			odata.Action = "14";
			odata.Role ="11";
			odata.NextAction = "";
		}*/
		oComponent.getModel().callFunction("PostTravelAmounts", "GET", odata, null, jQuery.proxy(function(oData, response) {
			...
			if(reqtype == "POST"){
			   // dialog.close();
				/*UCD1KOR 18 Dec 2019 dialog close*/
...
//			var btnLog = source.getParent().getItems()[1];
	...
	openPostDialog : function(results, odata,btnLog) {
		...
		var dialog = new sap.m.Dialog("idConfirmDialog",{
			title : 'Confirm',
			showHeader : true,
			stretch : true,
			type : 'Standard',
	...
	calculateTotal : function(data) {
		...
		/////##################### UCD1KOR 23 July 2020 ########################### //////
		////#################### Currency calculation ############################///////

		/*var cur = []
		for (i = 0; i < accData.length; i++) {
		    cur.push(accData[i].waers)
		}

		function onlyUnique(value, index, self) {
		    return self.indexOf(value) === index;
		}
		var itemms = cur.filter(onlyUnique);
		var currecy = []
		gross_debit = 0
		gross_credit = 0
		curr = ""
		for (i = 0; i < itemms.length; i++) {
		    for (j = 0; j < accData.length; j++) {
		        if (itemms[i] == accData[j].waers) {
		            gross_debit = parseFloat(gross_debit) + parseFloat(accData[j].gross_debit)
		            gross_credit = parseFloat(gross_credit) + parseFloat(accData[j].gross_credit)
		            curr = accData[j].waers;

		        }
		    }


		    if (gross_debit != 0 && gross_credit != 0 && curr != "") {

		        currecy.push({
		        	 "gross_debit": parseFloat(gross_debit).toFixed(2),
			          "gross_credit": parseFloat(gross_credit).toFixed(2),
			          "curr": curr
		        })
		    }
		    gross_debit = 0;
	...
	onViewRequest:function(){
		...
		if (tpno != "") {
			...
			sap.ui.project.e2etm.util.StaticUtility.setBusy(false, oController);
			sap.m.MessageToast.show("Enter Travel plan number to view PDF");
		}
	},
	//######## 16 March 2020 UCD1KOR #########// Start
	onViewStatus:function(){
		var that= this
		 var EmployeeNo = this.getView().byId("srchEmp").getValue();
        var TravelPlanNo = this.getView().byId("srchTp").getValue();
        that.getView().byId("idStatusTable").setVisible(false);
		if(EmployeeNo == "" && TravelPlanNo == ""){
			sap.m.MessageToast.show("Please enter any one value");
		}
		else{
			var filterURL = "CheckTrvlSettlementstatus?&ZZ_PERNR='"+EmployeeNo +"'&ZZ_REINR='"+TravelPlanNo +"'&$format=json";
        	var get = $.ajax({
				cache: false,
				url: sServiceUrl + filterURL, 
				type: "GET",
				async: false
			});
			var reinr_arr = '';
			get.done(function(oData) {
				sap.ui.core.BusyIndicator.hide();
                that.getView().byId("idStatusTable").setVisible(true);
				var stausModel = new sap.ui.model.json.JSONModel();
    			stausModel.setData(oData.d.results);
    			stausModel.setSizeLimit(oData.d.results.length); // Model size limit
    			that.getView().setModel(stausModel,"staus");
...
//		}
		...
		if(key != "REIM" || key != "CLOSED"){
			try{
				if (key == "DOME") {
					var content = this.getView().byId("icontabbaradmin").getItems()[0].getContent()[0];
				}else if (key == "INTL") {
					var content = this.getView().byId("icontabbaradmin").getItems()[1].getContent()[0];				
				}else if (key == "PYVR") {
					var content = this.getView().byId("icontabbaradmin").getItems()[2].getContent()[0];				
				} else if (key == "INRA") {
					var content = this.getView().byId("icontabbaradmin").getItems()[3].getContent()[0];				
				}
		//evt.getSource().getParent().getParent().getProperty("key");
				var tab = this.getTabValue(content.getSelectedKey(), key);
				if (tab) {
					var table = this.getTable(content, content.getSelectedKey());
				}			
				this.getDetails(table, "TRST", tab,skipCount,top);
			}catch(err){}
		}
		if(key == "REIM"){
			var skey = evt.getSource().getParent().getParent().getProperty("selectedKey");
			switch(skey){
			...
			case "NEW":
				...
				sap.ui.project.e2etm.util.StaticUtility.setBusy(true, this);
				this.getView().byId("idColTravelReq").setText("Travel Request");
				this.getListAdmin(sRole, "", "REIM", sAction, this, "NEW",top,skipCount);
				break;
			...
			case "PAYMENT":
				...
				sap.ui.project.e2etm.util.StaticUtility.setBusy(true, this);
			//this.getView().byId("idColTravelReq").setText("Travel Request");
				this.getListAdmin(sRole, "", "REIM", sAction, this, "PAYMENT",top,skipCount);
				break;
			case "CLOSED":
				var sRole = '11';
				var sAction = '00';
				...
				if (searchString == "") {
					...
					this.getListAdmin(sRole, "", "REIM", sAction, this, "CLOSED", top ,skipCount);//added code for paging 9/9/2020 uml6kor
				}
				break;
			}
