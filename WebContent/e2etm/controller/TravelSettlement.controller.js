...
sap.ui.controller("sap.ui.project.e2etm.controller.TravelSettlement", {
	...
	onInit : function() {
		...
		data.advance = {
			expenses : [],
			//############### UCD1KOR 28/09/2020 ###########////
			// Modifiedexpenses array added
			Modifiedexpenses : [],
			footer : [],
			apfooter : []
		};
		//added by uik1kor on 12/05/2021
		data.recv = {
				expenses : [],
				Modifiedexpenses : [],
				footer : [],
	...
	onRouteMatched : function(evt) {
		if (evt.getParameter("name") == "trsettle") {
			...
			if(global.DigiFlag == "X" || global.DigiFlag == "x"){
				this.getView().byId("trvlCondiions0").setVisible(true);
				this.getView().byId("trvlCondiions").setVisible(true);
			}else	{
					this.getView().byId("trvlCondiions0").setVisible(false);
					this.getView().byId("trvlCondiions").setVisible(false);
	...
	setFieldProperties : function(details) {
		...
		var NextAction = details.NextAction;
		var EmpNo = details.EmpNo;
		...
		oThis.getView().byId("panelpayback").setVisible(false);//added by uik1kor
		//Added by uik1kor on 17/05/2021 for hiding recovery amount for all other than travel settlement team
		if(loginrole == "TRST")
			{
		oThis.getView().byId("panelpayback").setVisible(true);
			}
		//Change by UIK1KOR on 09/09/2020 to hide fields with no value
		if(loginrole == "EMP" && action != "00" && action !="")
			{
			if((action == "02" && role == "02") | (action == "02" && role == "11")){}
			else{
				if(data.lodging.footer.length > 0)
				{
				if(data.lodging.footer[0].Paemp <= 0 && data.lodging.footer[0].Pacmp <= 0)
					oThis.getView().byId("panellodging").setVisible(false);
				}
				else
					{
					oThis.getView().byId("panellodging").setVisible(false);
				...
				if(data.boarding.footer.length > 0)
				{
					if(data.boarding.footer[0].Paemp <= 0 && data.boarding.footer[0].Pacmp <= 0)
						oThis.getView().byId("panelboarding").setVisible(false);
				}
				else
				{
					oThis.getView().byId("panelboarding").setVisible(false);
				}
				if(data.conv.footer.length > 0)
				{
					if(data.conv.footer[0].Paemp <= 0 && data.conv.footer[0].Pacmp <= 0)
						oThis.getView().byId("panelconv").setVisible(false);
				}
				else
				{
					oThis.getView().byId("panelconv").setVisible(false);
				}
				if(data.cargo.footer.length > 0)
				{
					if(data.cargo.footer[0].Paemp <= 0 && data.cargo.footer[0].Pacmp <= 0)
						oThis.getView().byId("panelcargo").setVisible(false);
				}
				else
				{
					oThis.getView().byId("panelcargo").setVisible(false);
				}
				if(data.entr.footer.length > 0)
				{
					if(data.entr.footer[0].Paemp <= 0 && data.entr.footer[0].Pacmp <= 0)
						oThis.getView().byId("panelentr").setVisible(false);
				}
				else
				{
					oThis.getView().byId("panelentr").setVisible(false);
				}
				if(data.other.footer.length > 0)
				{
					if(data.other.footer[0].Paemp <= 0 && data.other.footer[0].Pacmp <= 0)
						oThis.getView().byId("panelother").setVisible(false);
				}
				else
				{
					oThis.getView().byId("panelother").setVisible(false);
				}
				if(data.credit.footer.length > 0)
				{
					if(data.credit.footer[0].Paemp <= 0 && data.credit.footer[0].Pacmp <= 0)
						oThis.getView().byId("panelcredit").setVisible(false);
				}
				else
				{
					oThis.getView().byId("panelcredit").setVisible(false);
		...
		//End of change on 09/09/2020
		//UEA6KOR_22.1.2019 adding CTG fields
		for(var i=0;i<data.accass.length;i++){
			if ((data.accass[i].ZZ_GEBER == "F03") && (data.accass[i].ZZ_MCR == "X")){
	            oThis.getView().byId("formCTGMCR").setVisible(true);
	            oThis.getView().byId("formfundF03").setVisible(false);
			}else if((data.accass[i].ZZ_GEBER == "F03") && (data.accass[i].ZZ_MCR == "")){
	          
	            oThis.getView().byId("formfundF03").setVisible(true);
	            
	            oThis.getView().byId("formCTGMCR").setVisible(false);
			}else{
	            oThis.getView().byId("formCTGMCR").setVisible(false);
	            oThis.getView().byId("formfundF03").setVisible(false);
			}
...
//			};
		...
		if (loginrole == "EMP") {
			...
			if ((role == "01" && action == "00") || (role == "" && action == "") || (role == "02" && action == "02") || (role == "12" && action == "02") || (role == "11" && action == "02")) {
				...
				for(var i=0;i<data.accass.length;i++){
					if ((data.accass[i].ZZ_GEBER == "F03") && (data.accass[i].ZZ_MCR == "X")){
						oThis.getView().byId("idtask").setEditable(true);
						oThis.getView().byId("idResGrp").setEditable(true);
					//	oThis.getView().byId("idResTyp").setEditable(true);
					}}
				for(var i=0;i<data.accass.length;i++){
					if((data.accass[i].ZZ_GEBER == "F03") && (data.accass[i].ZZ_MCR == "")){
			          
						oThis.getView().byId("idCustomerCoordinatorName").setEditable(true);
						oThis.getView().byId("idCustCoordinatorDept").setEditable(true);
						oThis.getView().byId("idCustCoordPoNumber").setEditable(true);
						oThis.getView().byId("idCustCostCenter").setEditable(true);
						oThis.getView().byId("idCustCmpnyName").setEditable(true);
					}}
					//UEA6KOR_22.1.2019 adding CTG fields
				...
				for(var i=0;i<data.accass.length;i++){
					if ((data.accass[i].ZZ_GEBER == "F03") && (data.accass[i].ZZ_MCR == "X")){
						oThis.getView().byId("idtask").setEditable(false);
						oThis.getView().byId("idResGrp").setEditable(false);
					//	oThis.getView().byId("ResourceTyp").setEditable(false);
					}}
				for(var i=0;i<data.accass.length;i++){
					if((data.accass[i].ZZ_GEBER == "F03") && (data.accass[i].ZZ_MCR == "")){
			          
						oThis.getView().byId("idCustomerCoordinatorName").setEditable(false);
						oThis.getView().byId("idCustCoordinatorDept").setEditable(false);
						oThis.getView().byId("idCustCoordPoNumber").setEditable(false);
						oThis.getView().byId("idCustCostCenter").setEditable(false);
						oThis.getView().byId("idCustCmpnyName").setEditable(false);
					}}
				//UEA6KOR_22.1.2019 adding CTG fields
			}
	...
	setIntlFieldProperties : function(global) {
		...
		if (global.whichtab == "NEW") {
			...
			/*
			 * commented by uml6kor 4/2/2021 RBEIITAPPPIPELINE-197_12
			 * var txClsFlg = this.setCloseButtonVisible();
			*/
			/*
			 * start of code by uml6kor 4/2/2021 RBEIITAPPPIPELINE-197_12
			 */
			var txClsFlg = false;
			if(global.parentKey == "INTL"){
				txClsFlg = true;
			}
			// end of code 4/2/2021
			if(txClsFlg){
...
//			general = oDataModel.createBatchOperation("TrvStGenDataSet(EmpNo='" + global.ZZ_DEP_PERNR + "',TravelPlan='" + global.ZZ_TRV_REQ + "',Version='" + version + "',TravelType='" + global.ZZ_REQ_TYP + "',LoginRole='" + currentRole + "',Module='TRST',Item='1')", "GET");
		...
		var getComments = oDataModel.createBatchOperation("CardReloadComSet?$filter=Pernr+eq+'" + global.ZZ_DEP_PERNR + "'+and+Reinr+eq+'" + global.ZZ_TRV_REQ + "'+and+Trvky+eq+'" + global.ZZ_REQ_TYP + "'+and+Modid+eq+'TRST-" + currentRole + "'", "GET");
		...
		var custDetails = oDataModel.createBatchOperation("CUSTOMER_DETAILS_RFC?PERNR='" + global.ZZ_DEP_PERNR + "'&$format=json", "GET");
		var mcrdata = oDataModel.createBatchOperation("MCRData?&$format=json", "GET");
		var rcvdata = oDataModel.createBatchOperation("RCVData/?ZZ_TRV_REQ='"+global.ZZ_TRV_REQ+",TRST'&$format=json", "GET");//added by uik1kor on 11/05/2021
		//UEA6KOR_24.1.2020 CTG field addition
		var getTotal;
		...
		if (currentRole != "TRST")
			oDataModel.addBatchReadOperations([ getCurr, general, getModes, getDocs, getComments, getExpenses, getConvModes, getTotal, custDetails, mcrdata ]); //UEA6KOR_24.1.2020 CTG field addition
		else
			oDataModel.addBatchReadOperations([ getCurr, general, getModes, getDocs, getComments, getExpenses, getConvModes, getTotal, getCardSur, getCashSur, getInrSur, getF4,rcvdata	]);
		oDataModel.submitBatch(function(oResult) {
			try {
				...
				// UCD1KOR 5 Jun 2020 
				data.empdetails = sap.ui.getCore().getModel("profile").getData().employeeDetail;
			
				// UEA6KOR_24.1.2020 CTG field addition
				try{
					oThis.getView().byId("idCustomerCoordinatorName").setValue(data.accass[0].ZZ_CCNAME);
					oThis.getView().byId("idCustCoordinatorDept").setValue(data.accass[0].ZZ_CCDEPT);
					oThis.getView().byId("idCustCoordPoNumber").setValue(data.accass[0].ZZ_PONO);
					oThis.getView().byId("idCustCostCenter").setValue(data.accass[0].ZZ_CCOST);
					oThis.getView().byId("idCustCmpnyName").setValue(data.accass[0].ZZ_CLENTY);
					oThis.getView().byId("idtask").setValue(data.accass[0].ZZ_TASKID);
					oThis.getView().byId("idResGrp").setValue(data.accass[0].ZZ_RESOURCEID);
					oThis.getView().byId("idResTyp").setValue(data.accass[0].ZZ_RESOURCETYP);
				}catch(err){}
				//  UEA6KOR_24.1.2020 CTG field addition
...
//				oThis.setFieldProperties(data.traveldetails);
				...
				//UEA6KOR_29.1.2019 MCR Data
				var mcrModel = new sap.ui.model.json.JSONModel();
				mcrModel.setSizeLimit(oResult.__batchResponses[9].data.results.length);
				mcrModel.setData({
					typeMode : oResult.__batchResponses[9].data.results
				});
				oThis.getView().setModel(mcrModel, "mcrModel");
				//UEA6KOR_29.1.2019MCR Data
				
				//Added by uik1kor on 11/05/2021
				///######### UCD1KOR 16 July 2021 Prod bug fix ############///////
				if(currentRole === "TRST"){
					var rcvModel = new sap.ui.model.json.JSONModel();
					rcvModel.setData({
						typeMode : oResult.__batchResponses[12].data.results
					});
					oThis.getView().setModel(rcvModel, "rcvModel");
				}
				//UEA6KOR_29.1.2019Cutomer Details
				var custModel = new sap.ui.model.json.JSONModel();
				custModel.setSizeLimit(oResult.__batchResponses[8].data.results.length);
				custModel.setData({
					typeMode : oResult.__batchResponses[8].data.results
				});
				oThis.getView().setModel(custModel, "custModel");
	...
	setExpensesValues : function(results) {
		...
		///////////############ UCD1KOR 28/09/2020 ###############/////
		data.advance.Modifiedexpenses =[];
		data.recv.expenses = [];// added by uik1kor
		for ( var i = 0; i < results.length; i++) {
			...
			case "ADVC":
				...
				//data.advance.Modifiedexpenses.push(results[i]);
				break;
			case "CRED":
				data.credit.expenses.push(results[i]);
				break;
			case "RECV":
				data.recv.expenses.push(results[i]);
				break;	
			}
		}
		oThis.getView().getModel("travelsettlement").setData(data);
	...
	AddDefaultRow:function(){
		var data = oThis.getView().getModel("travelsettlement").getData();
		var currency = data.traveldetails.DefaultCurr;
		if(data.advance.Modifiedexpenses.length === 0){
			data.advance.Modifiedexpenses.push({
				Waers : currency,
				Rmrks : "Advance/Allowance received"
			});
		}
	...
	setFooter : function() {
		...
		for ( var prop in data) {
			...
			case "other":
				...
				break;
			case "advance":
				var advInitial = "check";
				data[prop] = oThis.calculateSubTotals(data[prop],advInitial);
				break;
			//added by uik1kor on 12/05/2021
			case "recv":
				var advInitial = "check";
		...
		for ( var i = 0; i < totalvalues.length; i++) {
			if (i < data.currtotal.length) {
				...
				data.currtotal[i].Total = (data.currtotal[i].receivable + data.currtotal[i].payable);//added by uik1kor
				data.currtotal[i].Total = data.currtotal[i].Total * data.currtotal[i].Rate;
			}
		}
		oThis.getView().getModel("travelsettlement").setData(data);
	},
	calculateSubTotals : function(exptype,advInitial) {
		exptype.footer = [];
		exptype.apfooter = [];
		var data = oThis.getView().getModel("travelsettlement").getData();
		var flag = false;
		...
		for ( var i = 0; i < currencyList.length; i++) {
			...
			if (flag) {
				if (totalemp != 0 || totalcmp != 0) {
					totalcmp = parseFloat(parseFloat(totalcmp).toFixed(2));
					exptype.footer.push({
						Paemp : totalemp,
						Pacmp : totalcmp,
						// Apamt : totalapp,
					...
					if(exptype.expenses[0].EType ==="ADVC" && advInitial === "check"){
						totalcmp = parseFloat(parseFloat(totalcmp).toFixed(2));
						exptype.Modifiedexpenses.push({
							Rmrks:"Advance/Allowance received",
							Paemp : totalemp,
							Pacmp : totalcmp,
							Flag : "X",
	...
	onCalculate1 : function(evt) {
		...
		if (evt.getSource().getBindingInfo("value").type && (isNaN(parseFloat(evt.getSource().getValue())) || evt.getSource().getValue() == "")) {
		   ...
		   //added by uik1kor on 12/05/2021
			case "paybackexp":
				data.recv.expenses[index].Apamt = data.recv.expenses[index].Pacmp;
				data.recv.expenses[index].Apcur = data.recv.expenses[index].Waers;
				data.recv.expenses[index].Cmode = data.recv.expenses[index].Otype;
				data.recv = oThis.calculateSubTotals(data.recv);
				break;
			case "creditexp":
				// data.advance.expenses[index].Apamt =
				// data.advance.expenses[index].Pacmp;
	...
	calculateTotalValues : function() {
		...
		var amtData = [];
		for ( var prop in data) {
			if (prop != "advance" && prop != "credit" && prop != 'recv') {
			...
			else if (prop == "recv") { //added by uik1kor in 12/05/2021
				var content = data[prop];
				for ( var i = 0; i < content.footer.length; i++) {
					amtData.push({
						Apcur : content.footer[i].Waers,
						Paemp : 0,
						receivable : content.footer[i].Pacmp,
					});
				}
			}
		}
			...
			for ( var j = 0; j < result.length; j++) {
				if (result[j].Apcur != "") {
					...
					if (scurrency == oCurrency) {
						...
						receivable = receivable + oThis.formatAmountValue(result[j].receivable)
						flag = true;
					}
				}
			}
			if (flag) {
				if (totalamt != 0 || totalemp != 0 || receivable != 0) { // receivable != 0 added by uik1kor for recovary amount
					var tvalue = totalamt + receivable;
					if( tvalue > totalemp )
						{
						receivable = tvalue - totalemp;
						payable = 0
						}
					else
						{
						payable = totalemp- tvalue;
						receivable = 0;
						}
					data.currtotal.push({
						emppaid : totalemp,
						advance : totalamt,
						receivable : receivable,//((totalamt - totalemp)+ receivable) > 0 ? ((totalamt - totalemp)+receivable) : 0,  //(totalamt - totalemp) > 0 ? (totalamt - totalemp) : 0,
						Rate : 1,
						Ratecur : oCurrency,
						currency : oCurrency,
						payable : payable,//(totalamt - totalemp) < 0 ? (totalamt - totalemp) : 0, //commented by uik1kor
		...
		for ( var i = 0; i < data.currtotal.length; i++) {
			...
			if (value <= 0) {
				value = value * -1;
				data.currtotal[i].payable = value //commented by uik1kor
			} else {
	...
	onDel : function(evt) {
		...
		if (index.length == 0) {
			...
			case "advanceDel":
				...
				////################## UCD1KOR 28/09/2020 ################//////////////////
				/// Remove data from Modifiedexpenses array
				data.advance.Modifiedexpenses = oThis.deleteRows(parent, index, data.advance.Modifiedexpenses);
				data.advance = oThis.calculateSubTotals(data.advance);
				...
				//added by UIK1KOR on 11/05/2021
			case "paybackDel":
				data.recv.expenses = oThis.deleteRows(parent, index, data.recv.expenses);
				data.recv = oThis.calculateSubTotals(data.recv);
				break;
			case "creditDel":
	...
	deleteRows : function(table, index, expense) {
		var act = oThis.getView().getModel("travelsettlement").getData().traveldetails.Action;//added by uik1kor for recovery 
		for ( var i = index.length - 1; i >= 0; i--) {
			var tableIndex = table.indexOfItem(index[i]);
			if (expense[tableIndex].Flag != "X") {
				expense.splice(tableIndex, 1);
			}
			else if (expense[tableIndex].EType == "RECV" && expense[tableIndex].Module == "TRST" && act == "00") {//added by uik1kor for recovery 
				expense.splice(tableIndex, 1);
			}
		}
		return expense;
	...
	onSubmit : function() {
		// UCD1KOR 05 Jun 2020 
		//Paid By validatio check for submit
		var data = oThis.getView().getModel("travelsettlement").getData();
		var exp = data.lodging.expenses;
		var column = this.getView().byId("idPaidByColumn").getVisible();
		if(exp.length >=1 ){
			for(var i=0;i<exp.length;i++){
				if(column == true && parseInt(exp[i].Paemp) > 0 && exp[i].BookedBy == ""){
					sap.m.MessageBox.information("Please select Booked by field in Lodging ");
					sap.ui.project.e2etm.util.StaticUtility.setBusy(false, oThis);
					return;
				}
			}
		...
		//var mgr = sap.ui.getCore().getModel("profile").getData().employeeDetail.ZZ_DEP_GRM;//added by uik1kor on 21/09/2020
		//Start of change by uik1kor on 08/10/2020 to check the manager data on submit
		var mgr = "";
    	var epernr = sap.ui.getCore().getModel("profile").getData().employeeDetail.ZZ_DEP_PERNR;
		...
		//End of change by uik1kor 
		if(globalData.DigiFlag == "X" || globalData.DigiFlag == "x"){
			sap.m.MessageBox.confirm("Please confirm that you have attached softcopies of all relevant bills", function(oAction) {
				if (oAction == "OK") {
					// 01,01
					sap.ui.project.e2etm.util.StaticUtility.setBusy(true, oThis);
		...
		// Start of change by uik1kor on 21/09/2020 to check the manager data on submit
		else if(mgr === "")
			{
	 ...
	 handleLinkPress : function() // added by uik1kor
	    {
	...
	onClose : function() {
		...
		jQuery.sap.delayedCall(500, oThis, function() {
			...
			if(global.parentKey == "INTL" && global.whichtab == "APP"){
				var txClsFlg = true; 
				//uml6kor 4/2/2021 RBEIITAPPPIPELINE-197_12 modified " oThis.setCloseButtonVisible(); " to true to enable taxables always
				if(txClsFlg){
	...
	saveDetails : function(role, action, btype) {
		...
		var loginrole = sap.ui.getCore().getModel("profile").getData().currentRole;
		//Checking for correct data in recovary by uik1kor on 25/05/2021
		if(data.recv.expenses.length > 0)
			{
			for(var i = 0; i < data.recv.expenses.length ; i++ )
				{
				if(data.recv.expenses[i].Waers == "")
					{
					sap.m.MessageBox.error("Please check Category/Currency in Recovery. ");
					sap.ui.project.e2etm.util.StaticUtility.setBusy(false, oThis);
					return;
					}
				}
		...
		/// Changes for Advance
		try{
			for(var i=0;i<data.advance.Modifiedexpenses.length;i++){
			    if(data.advance.Modifiedexpenses[i].onExpenseAdd === true){
			    	delete data.advance.Modifiedexpenses[i].onExpenseAdd;
			        data.advance.expenses.push(data.advance.Modifiedexpenses[i]);
			    }
			}
		}catch(err){}
		...
		if (!declaration && btype != "SAV" && loginrole == "EMP") {
			...
			oData = {
				...
				ZZ_CCNAME : oThis.getView().byId("idCustomerCoordinatorName").getValue(),//traveldetails.TRV_COST_ASGNSet.results[0].ZZ_CCDEPT, //uea6kor_24.1.2020 CTG field addition
				ZZ_PONO : oThis.getView().byId("idCustCoordPoNumber").getValue(),//traveldetails.TRV_COST_ASGNSet.results[0].ZZ_PONO, //UEA6KOR_24.1.2020 CTG field addition
				ZZ_CCDEPT : oThis.getView().byId("idCustCoordinatorDept").getValue(),//traveldetails.TRV_COST_ASGNSet.results[0].ZZ_CCDEPT, //UEA6KOR_24.1.2020 CTG field addition
				ZZ_CLENTY : oThis.getView().byId("idCustCmpnyName").getValue(),//traveldetails.TRV_COST_ASGNSet.results[0].ZZ_CLENTY, //UEA6KOR_24.1.2020 CTG field addition
				ZZ_CCOST : oThis.getView().byId("idCustCostCenter").getValue(),//traveldetails.TRV_COST_ASGNSet.results[0].ZZ_CCOST, //UEA6KOR_24.1.2020 CTG field addition
				ZZ_TASKID : oThis.getView().byId("idtask").getValue(),//traveldetails.TRV_COST_ASGNSet.results[0].ZZ_TASKID, //UEA6KOR_24.1.2020 CTG field addition
				ZZ_RESOURCEID : oThis.getView().byId("idResGrp").getValue(),//traveldetails.TRV_COST_ASGNSet.results[0].ZZ_RESOURCEID, //UEA6KOR_24.1.2020 CTG field addition
				ZZ_RESOURCETYP : oThis.getView().byId("ResourceTyp").getValue(),//traveldetails.TRV_COST_ASGNSet.results[0].ZZ_RESOURCETYP, //UEA6KOR_24.1.2020 CTG field addition
			}
			...
			try {
				for ( var prop in data) {
					...
					case "credit":
						...
						break;
					case "recv":// added by uik1kor on 12/05/2021	
						oData.TrvStDetailsSet = oThis.appendRows(data[prop], data.traveldetails, oData.TrvStDetailsSet, "RECV", btype, prop);
						break;
					}
				}
				// if(loginrole=="TRST"){
				...
				for ( var i = 0; i < data.currtotal.length; i++) {
					...
					oData.TrvStTotalSet.push({
						...
						PayBack : data.currtotal[i].receivable,//added by uik1kor on 17/05/2021
	...
	setFocus : function(property, exc) {
		...
		case "recv":
			if (exc == "Currency") {
				sap.m.MessageToast.show("Please enter currency in Recovery");
			} else if (exc == "Valid Currency") {
				sap.m.MessageToast.show("Please enter valid currency in Recovery Expense");
			}
			oThis.getView().byId("paybackexp").focus();
			break;
		}
	},
		...
		for ( var i = 0; i < exptype.expenses.length; i++) {
           if (exptype.expenses[i].Apcur != "" && exptype.expenses[i].Apcur != undefined) {
			...
			if ((exptype.expenses[i].Apamt != 0 && exptype.expenses[i].Apamt != undefined)&& exptype.expenses[i].Apcur == "") {
				throw "Currency";
			} //exptype.expenses[i].EType != 'RECV' && added by uik1kor for recovery amount 
			...
			trvdetset.push({
				...
				BookedBy : exptype.expenses[i].BookedBy,
...
onValueHelpRequest : function(evt) {
	...
	if(evt.getSource().getEditable() == true){
	...
	var oValueHelpDialog = new sap.ui.comp.valuehelpdialog.ValueHelpDialog({
		...
		ok : function(oControlEvent) {
			...
			oValueHelpDialog.close(); 
			var mcrData = oThis.getView().getModel("mcrModel").getData().typeMode;
			
			if(oControl.getId().indexOf("idResGrp") != -1){
				//oThis.getView().getModel().getData().ResoucesDesc = sText;
				//oThis.getView().getModel("travelsettlement").getData()
				for(var i=0;i<mcrData.length;i++){
					if(sKey == mcrData[i].ResourceId){
					oThis.getView().byId("ResourceTyp").setValue(mcrData[i].ResourceTyp);
					oThis.getView().byId("ResourceTyp").setEditable(false);
					}
			...
			if (oControl.getId().indexOf("idCustCmpnyName") != -1) {
				
				oThis.getView().getModel().getData().ZZ_CLENTY = sKey;
	...
	if (oControl.getId().indexOf("idCustCmpnyName") != -1) { // Customer search
		// help
		oThis.setCustomerF4(oRowsModel, oValueHelpDialog);
		oValueHelpDialog.open();
	}
	
	if (oControl.getId().indexOf("idtask") != -1) { // task
		oThis.setTaskID(oRowsModel, oValueHelpDialog);
		oValueHelpDialog.open();
		}
	if (oControl.getId().indexOf("idResGrp") != -1) { // resource group
		oThis.setResourceID(oRowsModel, oValueHelpDialog);
		oValueHelpDialog.open();
	}
...
//	var oData = oThis.getView().getModel("travelsettlement").getData();
...
setCustomerF4 : function(oRowsModel, oValueHelpDialog) {
	...
	try{
		oRowsModel.setData(oThis.getView().getModel("custModel").getData().typeMode);
	}catch(err){
...
setTaskID:function(oRowsModel, oValueHelpDialog) {
	...
	try{oRowsModel.setData(oThis.getView().getModel("mcrModel").getData().typeMode);}catch(err){}
...
onChangeBookedBy:function(oEvent){
	var selectedKey = oEvent.getSource().getSelectedItem().getKey();
	if(selectedKey == "HOTE"){
	sap.m.MessageBox.information("Please submit the original invoice along with the GST number.");
	}
},

onPressAdvanceDetails:function(){
	if (!this._oDialogonAdvanceDetails) {
		this._oDialogonAdvanceDetails = sap.ui.xmlfragment("sap.ui.project.e2etm.fragment.travelsettlement.AdvanceDetails", this);
		this.getView().addDependent(this._oDialogonAdvanceDetails);
	}
	this._oDialogonAdvanceDetails.open();
},
onCloseonAdvanceDetails:function(){
	this._oDialogonAdvanceDetails.close();
},
