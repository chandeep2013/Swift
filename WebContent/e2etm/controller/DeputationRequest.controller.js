...
/*------------------------CONTROLLER EVENT AREA BEGIN------------------------*/
	...
	onInit: function() {
		if (sap.ui.getCore().getModel("global") != null) {
			...
			// Bind the whole table to model and bind detail to first row in a
			// table
			...
			oDeputationThis.locFromEntered = $.Deferred(); // Check if location
															// From is selected
															// or not
			oDeputationThis.locToEntered = $.Deferred(); // Check if location
															// to is selected or
															// not
			oDeputationThis.afterRenderDeffered = $.Deferred(); // Check if
																// afterRendered
																// is done or
																// not
			oDeputationThis.stageDeferred = $.Deferred(); // Check if
															// populateStages is
															// done or not
			oDeputationThis.readDepDeferred = $.Deferred(); // Check if
															// readRequest is
															// done or not
			oDeputationThis.resetFormDeffered = $.Deferred(); // Check if
																// resetForm is
																// done or not
			oDeputationThis.initDeffered = $.Deferred(); // Check if init is
															// done or not
			oDeputationThis.travelDeffered = $.Deferred(); // Check if Travel
															// Plan controller
															// is initiated or
															// not
	...
	onPressMore:function(){
		sap.ui.getCore().byId("idInsrLine3").setVisible(true);
		sap.ui.getCore().byId("idInsrLine4").setVisible(true);
		sap.ui.getCore().byId("idInsrLine5").setVisible(true);
		sap.ui.getCore().byId("idInsrLine2").setVisible(false);
	},
	onPressLess:function(){
		sap.ui.getCore().byId("idInsrLine3").setVisible(false);
		sap.ui.getCore().byId("idInsrLine4").setVisible(false);
		sap.ui.getCore().byId("idInsrLine5").setVisible(false);
		sap.ui.getCore().byId("idInsrLine2").setVisible(true);
	},
	...
	onRouteMatched : function(oEvent) {
		try {
			...
			if (routeName == "deputation") {
				...
				oDeputationThis.locFromEntered = $.Deferred(); // Check if
																// location From
																// is selected
																// or not
				oDeputationThis.locToEntered = $.Deferred(); // Check if
																// location to
																// is selected
																// or not
				oDeputationThis.travelDeffered = $.Deferred(); // Check if
																// Travel Plan
																// controller is
																// initiated or
																// not
...
/* Start-CGGS Changes */
				...
				// uea6kor-uml6kor 26/8/2020 RBEIITAPPPIPELINE-217 date change
				// extension
			try{		
				if(globalData.cggsdata.Asgtyp == ""){
				globalData.cggsdata.Asgtyp = globalData.ZZ_ASG_TYP;
				}
				}catch(err){}
				// uea6kor-uml6kor 26/8/2020 RBEIITAPPPIPELINE-217 date change
				// extension
...
/* End-CGGS Changes */ 				
			...
			///////////////////////////////////////// UCD1KOR Nov 15,2020 
			var InsrModel = new sap.ui.model.json.JSONModel();
			var InsrData = {};
			var Data = sap.ui.getCore().getModel("global").getData().screenData;
			if(Data === undefined){
				 Data =sap.ui.getCore().getModel("global").getData();
			}
			

			if(Data.ZZ_DEP_TYPE === "INTL" && Data.ZZ_TRV_CAT === "TRFR" && Data.ZZ_REQ_TYP === "DEPU"){
				InsrData = {insr: true};
			}
			else{
				InsrData = {insr: false};
			}
			InsrModel.setData(InsrData);
			oDeputationThis.getView().setModel(InsrModel,"InsrModel");
...
/*------------------------CUSTOM FUNCTION AREA BEGIN------------------------*/
	...
	reLoadAllData : function() {
		...
		// Visible appropriate elements in the page which are dynamically set
		// based on stages
		$.when(oDeputationThis.stageDeferred).then(function() {
			// Display default screen for user if the request is submitted by
			// employee
	...
	// This method is used to set initial value to the details screen data
	// (view.getModel().getData()).
	// Initial value can be taken from the CREATE NEW REQUEST DIALOG or from the
	// selected request
	// in the Dash-Board. All those initial values are mainly stored in the
	// global model data
	// and then are re-assigned to the details screen data
	// (view.getModel().getData())
	resetForm: function() {
		try {
			var globalData = sap.ui.getCore().getModel("global").getData();
		...
		////################ UCD1KOR 12 July 2021 changes BUSR Copy###############//////////
			if(globalData.isCopy){
				sDeputationNo = globalData.ZZ_DEP_REQ;
			}
			...
			aData.screenData.ZZ_VREASON_EX = ""; // uea6kor-uml6kor 26/8/2020
			...
			aData.screenData.onRecOrgEnable = true;   // recieving org add by
														// UEA6KOR_30.01.2019
...
/* End-CGGS Changes */	        
			// If new deputation request is created, fetch value from popup
			// screen
			if (aData.screenData.ZZ_DEP_REQ == "") {
				...
				if (globalData.screenData != null) {
					...
					if (aData.screenData.ZZ_DEP_TYPE == "INTL") {
						...
						aData.RecOrgId = globalData.RecOrgId;
					
					// Start of changes_dye5kor_phase-3,second set of release
						
						/*
						 * if(aData.screenData.ZZ_TRV_REQ != "" &&
						 * aData.screenData.ZZ_TRV_REQ != " " ){
						 * 
						 * view.byId("lblTrvlRequestType").setVisible(true);
						 * view.byId("lblTrvlRequestNumber").setVisible(true);
						 * view.byId("lblTrvlRequestType").setText("Travel
						 * request No");
					...
					// After assigning the value from the popup, delete that
					// node from the global data
				} else {    // Navigated from existing request in home page
					// This block can only happen if a INFO/SECO/EMER/HOME is
					// about to be created
					if (globalData != null) {
				...
				// This block can only happen if viewing the INFO/SECO/HOME/EMER
				// request
				if (globalData != null) {
					aData.screenData.ZZ_STAT_FLAG = globalData.ZZ_STAT_FLAG;
					aData.screenData.ZZ_TRV_CAT = globalData.ZZ_TRV_CAT;
		...
		if (sDeputationNo != '') {
			...
			if (sap.ui.getCore().getModel("profile").getData().currentRole == "EMP") {
				...
				if (!globalData.isChange) {
					...
					if ((aData.screenData.ZZ_STAT_FLAG == "AA000" && oDeputationThis.currentStage == "1") || 
						...
						if ( aData.screenData.ZZ_NEXT_APPROVER == profileData.employeeDetail.ZZ_DEP_PERNR) {
							aData.screenData.editable = true;
							aData.screenData.onRecOrgEnable = true;   // recieving
																		// org
																		// add
																		// by
																		// UEA6KOR_30.01.2019
						...
						// aData.screenData.onRecOrgEnable = false; //recieving
						// org add by UEA6KOR_30.01.2019
							aData.screenData.onRecOrgEnable = true;   // recieving
																		// org
																		// changes
																		// by
						...
						aData.screenData.editable = false;
					// aData.screenData.onRecOrgEnable = false; //recieving org
					// add by UEA6KOR_30.01.2019
						aData.screenData.onRecOrgEnable = true;   // recieving
																	// org
																	// changes
																	// by
					...
					// Set other flags to current editable version if process is
					// not change process
				...
				// changes to familly 24/08/2016
					...
					aData.screenData.isDependentChangeBeforeDeputation = aData.screenData.editable; // used
																									// in
																									// radios
																									// button
					// this group is used in dependent box
					aData.screenData.changeDeputationReturnDate = aData.screenData.editable; // return
																								// date
					aData.screenData.changeDeputationStartDate = aData.screenData.editable; // start
																							// date
					aData.screenData.changeDeputationDepeCheck = aData.screenData.editable; // dependent
																							// checkbox
					aData.screenData.changeDeputationDepeType = aData.screenData.editable; // dependent
																							// type
					aData.screenData.changeDeputationDepeVisaExist = aData.screenData.editable; // dependent
																								// visa
																								// exist
																								// checkbox

					aData.screenData.changeDeputationSelDepeCheck = aData.screenData.editable; // selected
																								// dependent
																								// checkbox
					aData.screenData.changeDeputationSelDepeType = aData.screenData.editable; // selected
																								// dependent
																								// type
					aData.screenData.changeDeputationSelDepeVisaExist = aData.screenData.editable; // selected
																									// dependent
																									// Visa
																									// checkbox
					aData.screenData.changeDeputationSelStartDate = aData.screenData.editable; // selected
																								// dependent
																								// start
																								// date
					aData.screenData.changeDeputationSelReturnDate = aData.screenData.editable; // selected
																								// dependent
																								// return
																								// date
					...
					aData.screenData.editable = false;
			// aData.screenData.onRecOrgEnable = false; //recieving org add by
			// UEA6KOR_30.01.2019
					aData.screenData.onRecOrgEnable = true;   // recieving org
																// changes by
																// UEA6KOR_05.03.2019
					...
					if (globalData.changeType == "DA") {
						...
						aData.screenData.isSTAandDE = false;
						// fix enabled family
						aData.screenData.isDependentChangeBeforeDeputation = false;  // used
																						// in
																						// radios
																						// button

						aData.screenData.changeDeputationReturnDate = false; // return
																				// date
						aData.screenData.changeDeputationStartDate = false; // start
																			// date
// aData.screenData.changeDeputationReturnDate = true; // return date // added
// by VAB6KOR on 09.04.2018
// aData.screenData.changeDeputationStartDate = true; // start date // added by
// VAB6KOR on 09.04.2018
						aData.screenData.changeDeputationDepeCheck = false; // dependent
																			// checkbox
						aData.screenData.changeDeputationDepeType = false; // dependent
																			// type
						aData.screenData.changeDeputationDepeVisaExist = false; // dependent
																				// visa
																				// exist
																				// checkbox

						aData.screenData.changeDeputationSelDepeCheck = false; // selected
																				// dependent
																				// checkbox
						aData.screenData.changeDeputationSelDepeType = false; // selected
																				// dependent
																				// type
						aData.screenData.changeDeputationSelDepeVisaExist = false; // selected
																					// dependent
																					// Visa
																					// checkbox
						aData.screenData.changeDeputationSelStartDate = true; // selected
																				// dependent
																				// start
																				// date
						aData.screenData.changeDeputationSelReturnDate = true; // selected
																				// dependent
																				// return
																				// date
						...
						try{
							if( globalData.cggsdata.Vreason_ex == "EX")
								{
								aData.screenData.ZZ_VREASON_EX = "EX";
								}
						}
						catch(err){}
						// fix enabled family
					...
					// aData.screenData.changeDeputationReturnDate = false; //
					// return date
						aData.screenData.changeDeputationReturnDate = true; // return
																			// date
																			// //
																			// added
																			// by
						...
						aData.screenData.changeDeputationSelDepeVisaExist = false; // selected
																					...
																					// Visa
																					// checkbox
						aData.screenData.changeDeputationSelStartDate = false; // selected
																				// dependent
																				// start
																				// date
						aData.screenData.changeDeputationSelReturnDate = false; // selected
																				// dependent
																				// return
																				// date
						...
						aData.screenData.startDateChange = false;
						aData.screenData.endDateisChange = false;
// if (aData.screenData.ZZ_TRV_CAT == "TRFR" && aData.screenData.ZZ_DEP_TYPE ==
// "DOME") {
// aData.screenData.endDateisChange = false;
// } else {
// aData.screenData.endDateisChange = true;
// }
						...
						aData.screenData.isSTAandDE = false;
						// fix enabled family
						aData.screenData.isDependentChangeBeforeDeputation = true;  // used
																					// in
																					// radios
																					// button
						...
						aData.screenData.changeDeputationReturnDate = true; // return
																			// date
						aData.screenData.changeDeputationStartDate = true; // start
																			// date
						aData.screenData.changeDeputationDepeCheck = true; // dependent
																			// checkbox
						aData.screenData.changeDeputationDepeType = true; // dependent
																			// type
						aData.screenData.changeDeputationDepeVisaExist = true; // dependent
																				// visa
																				// exist
																				// checkbox

						aData.screenData.changeDeputationSelDepeCheck = true; // selected
																				// dependent
																				// checkbox
						aData.screenData.changeDeputationSelDepeType = true; // selected
																				// dependent
																				// type
						aData.screenData.changeDeputationSelDepeVisaExist = true; // selected
																					// dependent
																					// Visa
																					// checkbox
						aData.screenData.changeDeputationSelStartDate = true; // selected
	...
	showDuration : function() {
		...
		if( aData.screenData.ZZ_DEP_STDATE != null && aData.screenData.ZZ_DEP_ENDATE != null ){
			...
			// Setting additional information to screen data (sponsor by
			// company)
			if (aData.screenData.ZZ_DEP_TYPE == "INTL") {
	...
	displayDocuments : function() {
		try {
			if( oDeputationThis.getView().getModel().getData().screenData.ZZ_DEP_TYPE == "INTL" &&
				...
				// //############# UCD1KOR 19th Aug 2020 Extension Documents
				// Added ################////////
			// modified start code uea6kor-uml6kor 24/8/2020
			// RBEIITAPPPIPELINE-217 date change extension
		
				if ( aData.screenData!= undefined ){
				if ( aData.screenData.ZZ_VREASON_EX == "EX" && aData.screenData.ZZ_VREASON == "DB"){
					sap.ui.getCore().getModel("global").getData().extDoc = true;
					sap.ui.getCore().getModel("global").refresh();
				// //modified endcode uea6kor-uml6kor 24/8/2020
				// RBEIITAPPPIPELINE-217 date change extension
					sap.ui.getCore().byId("extdoclistdepuDetail--panelExtensionDocs").setVisible(true);
					
						oDeputationThis.onExtListId = "extdoclistdepuDetail--List_ExtDocument_Id";
						oDeputationThis.onExtRepId  = "extdoclistdepuDetail--ExtViewRepeatId";
						oDeputationThis.currentViewId  = "extdoclistdepuDetail";
					sap.ui.project.e2etm.util.StaticUtility.getExtDocument(
						aData.screenData.ZZ_DEP_TOCNTRY, oDeputationThis.getView().getModel().getData().screenData.ZZ_TRV_CAT, oDeputationThis, 
						aData.screenData.ZZ_DEP_REQ, aData.screenData.ZZ_DEP_PERNR);
				if(sap.ui.getCore().getModel("profile").getData().currentRole === "DEPU"){
					sap.ui.getCore().byId("extdoclistdepuDetail--idListEXT_DocFlexBox").setVisible(true);
					sap.ui.getCore().byId("extdoclistdepuDetail--idActionEXT_DocFlexBox").setWidth("100%");
				}
				// modified start code uea6kor-uml6kor 24/8/2020
				// RBEIITAPPPIPELINE-217 date change extension
				sap.ui.getCore().byId("extdoclistdepuDetail--ConfirmExtUpload").setVisible(false);
				}else
				{
						sap.ui.getCore().byId("extdoclistdepuDetail--panelExtensionDocs").setVisible(false);
			...
			}else
			{
				...
				sap.ui.getCore().getModel("global").getData().extDoc = false; 
				sap.ui.getCore().getModel("global").refresh();
			}		// //modified end code uea6kor-uml6kor 24/8/2020
					// RBEIITAPPPIPELINE-217 date change extension
//				
//				
	}
...
// sap.ui.getCore().byId("linkTemplate").setHref(url + "/" +
// view.getModel().getData().screenData.ZZ_DEP_TOCNTRY + "/Stage " +
// globalData.currentStage);
	...
	displayBreadcrumbText : function() {
		...
		if(sap.ui.getCore().getModel("global").getData().isCopy){
			view.byId("lblRequestType").setText("BUSR");
			view.byId("lblRequestType").setTooltip("BUSINESS REQUEST");
		}else{
		...
		// Set help documents
		// sidd code start
		var url = "http://sgpmis02.apac.bosch.com/Helpdesk/ESS/swifthelpdoc/deputation/"; // change
																							// in
																							// url
																							// _uml6kor_21_5_2019
		...
		if (aData.screenData.ZZ_REQ_TYP == "DEPU") {
			...
			if(aData.screenData.ZZ_DEP_TYPE == "INTL"){
				...
				}else if (a == "4") {  // added uml6kor 21_5_2019
					
					url = "http://sgpmis02.apac.bosch.com/Helpdesk/ESS/swifthelpdoc/";
		...
		////################ UCD1KOR 14 July 2021 Changes BUSR COPY #############/////// 
		if(sDeputationNo != "" && aData.screenData.ZZ_REQ_TYP == "DEPU" && sap.ui.getCore().getModel("global").getData().isCopy == false){
	...
	// End of change_dye5kor_TP_SWAP_Changes_Phase-3
		...
		if (globalData.isChange) {
			...
			if (sDeputationNo == "") {
				if (aData.screenData.ZZ_REQ_TYP == "DEPU") {
					...
					/////################ UCD1KOR 20 July 2021 BUSR-TRFR Message change #################//////
					if(sap.ui.getCore().getModel("global").getData().TrfrFlag == "X"){
						view.byId("lblRequestNumber").setText("YOU ARE CREATING A NEW REQUEST FOR DOMESTIC TRANSFER");
					}
					else{
				...
				////############ UCD1KOR 12 July 2021 Changes BUSR COPY ###################///////
				 if(globalData.isCopy){
					view.byId("lblRequestNumber").setText("YOU ARE CREATING A NEW BUSINESS REQUEST");
					view.byId("lblRequestNumber").addStyleClass("request_breadcrumb");
				}else if (aData.screenData.ZZ_VREASON == "" || aData.screenData.ZZ_VREASON == null) {
		...
		/* UCD1KOR 29 Nov 2019 Tooltip added for request no */
		view.byId("lblRequestNumber").setWidth("550px");
		var tooltiptext = view.byId("lblRequestNumber").getText();
		view.byId("lblRequestNumber").setTooltip(tooltiptext);

		
	},

	// This method is used to display the default screen based on the current
	// scenario
	// For example: display DEPUTATION DETAILS screen when manager is about to
	// approve
	// a request
	displayDefaultScreen : function() {
		try {
			...
			if (aData.screenData.ZZ_REQ_TYP == "DEPU") {
				if (!globalData.isChange) {       // Considerring non-change
													// deputation request
					...
					// For manager to approve an international deputation
					// request
					...
					// Sent back from Travel approval screen for domestic with
					// travel plan
					if (aData.screenData.ZZ_DEP_TYPE == "DOME" && 
							((aData.screenData.ZZ_TRV_REQ != "0000000000" && aData.screenData.ZZ_TRV_REQ != "") || 
									aData.screenData.ZZ_TRV_REQ == null)) {
...
/* Start-Change Famil Return Dates */
					...
					if(aData.screenData.ZZ_DEP_TYPE == "INTL" && chgFlag){// For
																			// Change
																			// Family
																			// Return
																			// Dates,display
...
/* End-Change Famil Return Dates */					
			...
			} else { // Display default screen for BUSR / SECO / HOME / EMER
						// / INFO request
				if (!globalData.isChange) {
					// For manager to approve a request
				...
				get.done(function(data) {
					if (currentRole == "EMP" || currentRole == "DEPU" || currentRole == "GRM") {
					 ...
					 if(aData.screenData.ZZ_DEP_TOCNTRY == "DE" && aData.screenData.ZZ_ASG_TYP == "STA"&& 
						...
						/*
						 * if(clIndex == 1) salaryList =
						 * data.d.results.splice(0,clIndex); else if(clIndex>0){
						 * salaryList = data.d.results.splice(0,clIndex); }else
						 * if(clIndex == -1){ if(data.d.results.length!=0){
...
// Commented by UEA6KOR_08.03.2019
// aData.screenData.contractList = data.d.results.reverse();
								aData.screenData.contractList = data.d.results; 
								// modify the contract list value
					...
					// Consider documents of migrated request for DEPU in
					// domestic deputation workflow
					if (sap.ui.getCore().getModel("profile").getData().currentRole == "DEPU" &&
							oDeputationThis.getView().getModel().getData().screenData.ZZ_REQ_TYP == "DEPU" &&
							oDeputationThis.getView().getModel().getData().screenData.ZZ_DEP_TYPE == "DOME") {
						// Logic to determine migration request
				...
				get.done(function(data){
					...
					if( view.getModel().getData().screenData.ZZ_STAT_FLAG == "AA007" ||
							...
							(view.getModel().getData().screenData.ZZ_STAT_FLAG == "JJ009" && 
									currentRole != "COC")){ // 11/Jun/2021 Thanh - add condition coc dash board 
		...
		if (aData.screenData.ZZ_DEP_TYPE == "INTL" && aData.screenData.ZZ_REQ_TYP == "DEPU" &&
			...
			try {
				...
				if (aData.screenData.ZZ_DEP_TYPE == "INTL") {
					...
					if (aData.screenData.ZZ_VISA_PLAN != "" && aData.screenData.ZZ_VISA_PLAN != null) { // Visa
																										// header
																										// exists
						var get = $.ajax({
							cache: false,
					...
					// After getting existing visa, control visible or invisible
					// visa copy
					$.when(oDeputationThis.visaData).then(function() {
						...
						if ((globalData.currentStage == "1" && globalData.currentSubSet == "1_2_1" && 
							...
							// If visa not existing and not processed by bosch
							// or visa existing and not process by Bosch
							...
							// If Yes, loop through deputation items to find any
							// existing visa
							if (aData.screenData.ZZ_FAMILY_ACCOMP) {
								if (aData.visaExistingDependent.length == 0) {  
									...
									if (foundExist == false) {
										...
										// Display only the dependent that has
										// existing visa
										for (var i = 0; i < selectedDependents.length; i++) {
											var prop = {
...
/* Start-Date Change */
									if(aData.screenData.ZZ_VREASON == "DF" && aData.screenData.ZZ_REQ_TYP=="DEPU"
										&&aData.screenData.ZZ_DEP_TYPE == "INTL"){// New
																					// born
																					// kid
...
/* End-Date Change Issue */													
						...
						} else if ((globalData.currentStage == "2" &&     // Deputation
																			// team,
																			// no
																			// visa
																			// plan
								...
								(globalData.currentStage == "3" &&      // Employee
																		// upload
																		// visa
																		// for
																		// travel
							...
							if (aData.visaExistingDependent.length == 0) {  // dependent
																			// has
																			// no
																			// visa
																			// exist
								if (selectedDependents.length == 0) {   // No
																		// dependent
																		// in
																		// travel
																		// request
...
/* Start-Date change */
										...
										if ((selectedDependents[i].ZZ_VISA_EX == "X" && 
											...
											selectedDependents[i].ZZ_PROC_BSCH == "")) {// Original:only
																						// selectedDependents[i].ZZ_VISA_EX
																						// ==
																						// ""
											for(var j=aData.visaExistingDependent.length-1;j>=0;j--){
...
/* End-Date change */										
						...
						} else if(globalData.currentStage == "2" &&     // Employee
																		// upload
																		// secondary
																		// document
								globalData.currentSubSet == "1_2_1"){
			...
			if (viewData.screenData.ZZ_DEP_TYPE == "INTL" &&
				...
				if (globalData.currentStage == "1" && globalData.currentSet == "1_1") {
					...
					if (sDeputationNo == "") {
						...
						if( parseInt(viewData.screenData.ZZ_DEP_DAYS) > globalData.sponsorPeriod) { // Disable
																									// sponsor
																									// parseInt(viewData.screenData.sponsor.ZZ_MAX)
...
/* End-Change Return Dates */	
	...
	populateDynamicStages: function(generateTab) {
		...
		if (sDeputationNo != '' && sDeputationNo != null) {
			aData = view.getModel().getData().currentStages; // This is for
																// displaying
																// current
																// request
		} else {
			aData = view.getModel().getData().generalStages; // This is for
																// creating a
																// new
																// deputation
																// request
				...
				stageItem = new sap.ui.project.e2etm.controls.stageItem("_" + aData[i].ZZ_STAGE + "_" + aData[i].ZZ_SET, {
					...
					press    : function(evt) {
						if (evt.getSource().getStatus() == "2") {
							...
							// Display tabs that are processed consider edit
							// mode
							evt.getSource().getParent().removeSetAllIsSelectedProperty();
							evt.getSource().setIsSelected("X");
							oDeputationThis.displaySelectedStageItem(this.mProperties.stage, this.mProperties.item, true);
	...
	displaySelectedStageItem : function(zz_stage, zz_set, userClicked) {
		try {
			...
			var aData = [];
			if (sDeputationNo != '' && sDeputationNo != null) {
				aData = view.getModel().getData().currentStages; // This is
																	// for
																	// displaying
																	// current
																	// request
			} else {
				aData = view.getModel().getData().generalStages; // This is
																	// for
																	// creating
																	// a new
																	// deputation
			...
			// Stage which is in process will have the status of "1"
			// Stage which is completed will have the status of "2"
			// Firstly, Loop through data and set current process to global
			// model
			for (var i = 0; i < aData.length; i++) {
				...
				if (aData[i].ZZ_SET == zz_set && aData[i].ZZ_STAGE == zz_stage) {
							...
							if( viewData.screenData.ZZ_REQ_TYP ==  "BUSR" ||
								...
								if (viewData.screenData.ZZ_DEP_TYPE == "BGEN") {
									fragment = new sap.ui.xmlfragment("sap.ui.project.e2etm.fragment.deputation.BUSR." 
											...
											+ aData[i].ZZ_SUBSET , oDeputationThis); // here
																						// the
																						// Fragment
																						// is
																						// instantiated
					...
					try {
						if (aData[i].ZZ_SUBSUBSET != "0") {
							...
							if(aData[i].ZZ_SETD != "Travel Plan"){// uml6kor_upgrade
																	// 10/12/2019
								oDeputationThis.iconTab.getItems()[0].getContent()[0].getItems()[1].addContent(panel); 		// uml6kor_upgrade
																															// 6/12/2019
							}
				...
				try {
					...
					if( viewData.screenData.ZZ_REQ_TYP ==  "BUSR" ||
						...
						fragment = new sap.ui.xmlfragment("sap.ui.project.e2etm.fragment.deputation.BUSR." 
								...
								+ aData[aData.length-1].ZZ_SUBSET , oDeputationThis); // here
																						// the
																						// Fragment
																						// is
																						// instantiated
			...
			try {
				...
				///UCD1KOR 19/05/2021 Added below code enable feedback link
				if(zz_stage == "2" && zz_set == "1_4"){
					oDeputationThis.feedbackgetservicecall();
				}
			} catch(exc) {}
	...
	// This method is used to read details of a request which can be a
	// deputation request,
	...
	readRequest: function() {
		...
		// Only creation of new deputation, business request will go inside this
		// block
		if (sDeputationNo == '') {
			var stageReq = "";
			...
			// Calling service to get the intial stages of new request based on
		...
		} else {    // This block of code will be used for reading an existing
					// request
			var getDeputation = "";
			if (sap.ui.getCore().getModel("global").getData().ZZ_REQ_TYP == "BUSR" ||
				...
				if (sap.ui.getCore().getModel("global").getData().ZZ_REQ_TYP == "BUSR") {   // Business
																							// travel
			...
			// Getting the details of an existing deputation request including
			// the stage
			var get = $.ajax({
				cache: false,
			...
			get.done(function(data, header, response) {
				...
				view.getController().displayDeputationHeader(data.d); // Display
																		// Deputation
																		// header
																		// data
																		// from
																		...
																		// to
																		// front-end
				view.getController().displayDeputationItem(data.d); // Display
																	// Deputation
																	// item data
																	// from
																	// back-end
				...
				$.when($.ajax({cache: false,url:stagReq}),$.ajax({cache: false,url:sSponsor}),
						$.ajax({cache: false,url:asgModel})).done(function(stagReqData,sSponsorData,asgModelData){
							...
							// Getting the number of days to determine sponsor
							// by company
							...
							// comment
							aData.RecOrgId = sap.ui.getCore().getModel("global").getData().RecOrgId;
	...
	displayDeputationHeader: function(aInput) {
		...
		aData.screenData.ZZ_COMABBR = aInput.ZZ_COMABBR;       // Adding
																// Recieving Org
																// by
																// UEA6KOR_29.01.2019
		...
		aData.screenData.ZZ_VREASON_EX = aInput.ZZ_VREASON_EX; // uea6kor-uml6kor
																// 25/8/2020
																// RBEIITAPPPIPELINE-217
																// date change
																// extension
	...
	// This method is used to display deputation items(dependents) data onto the
	// screen
	...
	// This method is used to validate Deputation before submitting the
	// deputation request
	validateDeputation : function() {
		var that= this;
		var ext_date = this.ext_date;
		...
		// //####### UCD1KOR 06 May 2020 Conditions ########// added
		// removed conditions for duration
		// Covid condiotins Future comment /// Covid-19 changes
		that.endDate = new Date(aData.screenData.ZZ_DEP_ENDATE.substr(0, 4), aData.screenData.ZZ_DEP_ENDATE.substr(4, 2) - 1, 
				aData.screenData.ZZ_DEP_ENDATE.substr(6, 2));
		that.ext_date = new Date(globalData.ext_date.substr(0, 4), globalData.ext_date.substr(4, 2) - 1, 
				globalData.ext_date.substr(6, 2));
		that.startDate = new Date(aData.screenData.ZZ_DEP_STDATE.substr(0, 4), aData.screenData.ZZ_DEP_STDATE.substr(4, 2) - 1, 
				aData.screenData.ZZ_DEP_STDATE.substr(6, 2));
		that.curDate = new Date();
		that.covid_max = that.covid_19COnditionCheck(aData.screenData);
		...
		if(that.durationDays > that.covid_max && globalData.changeType == "DB" && that.curDate > that.startDate  && parseInt(aData.screenData.ZZ_DEP_DAYS) > that.covid_max && that.endDate > that.ext_date){
		...
		// Covid condiotins Future comment /// Covid-19 changes
		if(that.durationDays > that.covid_max && globalData.changeType == "DB" && that.curDate > that.startDate  && parseInt(aData.screenData.ZZ_DEP_DAYS) > that.covid_max){
			// no
		}
		else{
			// start : uml6kor 9/4/2019 coolingoff strtdate > nxtdate.
			if( aData.screenData.ZZ_TRV_CAT == "WRKP" || // aData.screenData.ZZ_TRV_CAT
															// == "TRNG" ||
															// //commented trng
															// by
															// uml6kor14/2/2020
					aData.screenData.ZZ_TRV_CAT == "TRFR" ){// trng added by
															// uml6kor
			var nextdate = "";
			var sMaxDay = 0;
			var trvldur = 0;// added var to get travel duration for DE uml6kor:
							// 12/04/2019
			var visaExists = ""; // started by uml6kor 10/6/2019
			if( aData.screenData.ZZ_VISA_AEXIST == true )
			{
				visaExists = "X";
			}
			else
			...
			"'&ZZ_DEP_DAYS='" + aData.screenData.ZZ_DEP_DAYS + // added by
																// uml6kor
																// 16/1/2020 stx
																// cooling off
			"'&ZZ_TRV_CAT='" + aData.screenData.ZZ_TRV_CAT +  // added by
																// uml6kor
																// 16/1/2020 stx
																// cooling off
			"'&$format=json";
			...
			get.done(function(oCheckCooling) {
				if (aData.screenData.ZZ_DEP_TOCNTRY != "") {
					nextdate = oCheckCooling.d.Check_cooling_ROW.NEXT_DATE ;
					trvldur = oCheckCooling.d.Check_cooling_ROW.TRVL_DURATION;
					// start: uml6kor 12/4/2019
				
				
						  if (sap.ui.getCore().getModel("global").getData().coolingPeriodMaxdays < parseInt(oCheckCooling.d.Check_cooling_ROW.MAX_DURATION))
						  {
						  		sMaxday = sap.ui.getCore().getModel("global").getData().coolingPeriodMaxdays;
						  }
						  else 
						  {
							  sMaxday = oCheckCooling.d.Check_cooling_ROW.MAX_DURATION;
						  }
						}
	...
	// start : uml6kor 9/4/2019 validating whether coolingoff startdate is less
	// than nextdate .

			
			if(aData.screenData.ZZ_DEP_STDATE < nextdate )
			{
				aData.screenData.ZZ_DEP_STDATE_ERROR = "Error";
				if(aData.screenData.ZZ_DEP_TOCNTRY == "GB" && ( aData.screenData.ZZ_TRV_CAT == "WRKP" // commented
																										// by
																										// uml6kor
																										// 14/2/2020
																										// "trng"||
				...
				var text_coolingoff = 
					 ...
					 + 	sap.ui.project.e2etm.util.Formatter.sapDate(nextdate)  + '. Please correct the dates accordingly and continue.' ;
				}
				else if ( aData.screenData.ZZ_TRV_CAT == "TRFR" ){ // added by
																	// uml6kor
																	// 16/1/2020
																	// stx
																	// cooling
																	// off
					var text_coolingoff = 'As per cooling period guidelines, you will not be able to travel to the selected country on STX assignment model only from '
					...
					var text_coolingoff = 
					  'As per proposed cooling period conditions, you are not allowed to travel on the selected date. The probable travel start date can be on or after '
					 + 	sap.ui.project.e2etm.util.Formatter.sapDate(nextdate)  + ' for a maximum duration of "'+ sMaxday + '" days. Please correct the dates accordingly and continue.' ;
					}
		...
		// end : uml6kor 9/4/2019
			// start uml6kor 12/4/2019 "travel duration check _ immigration

			if( trvldur != "" && (parseInt(trvldur) < sap.ui.getCore().getModel("global").getData().coolingPeriodMaxdays) &&
					(parseInt(aData.screenData.ZZ_DEP_DAYS) > trvldur))  {
					var text = 'As per immigration rules, employee is left with ' + trvldur 
					+ ' days to be on deputation to Germany. Please correct the dates accordingly and continue.';
					sap.ca.ui.message.showMessageBox({
						type: sap.ca.ui.message.Type.ERROR,
						message: text,
						details: text
			...
			// ended uml6kor 12/4/2019
				// start 19/6/2019 uml6kor
		
			
		if ( aData.screenData.ZZ_TRV_CAT == "WRKP" || aData.screenData.ZZ_TRV_CAT == "TRFR" ){ // added
																								// by
																								// uml6kor
																								// 19.6.2019
			
	    var trvl_dur = 0;
		var sCheckAssign = sServiceUrl + "AssignmentConversion?ZZ_DEP_REQ='"+aData.screenData.ZZ_DEP_REQ+
										 "'&ZZ_PERNR='" +sap.ui.getCore().getModel("profile").getData().employeeDetail.ZZ_DEP_PERNR +
										 "'&ZZ_DEP_TOCNTR='" + aData.screenData.ZZ_DEP_TOCNTRY +
										 "'&ZZ_DEP_SDATE='" + aData.screenData.ZZ_DEP_STDATE +
										 "'&ZZ_DEP_EDATE='" + aData.screenData.ZZ_DEP_ENDATE +
										 "'&$format=json";
		...
		get.done(function(result){
		 if(result != null){
			  trvl_dur = result.d.AssignmentConversion.TRVL_DURATION;	
		 }
		 });

		  if( trvl_dur != 0 && trvl_dur != '1-' && parseInt(aData.screenData.ZZ_DEP_DAYS) > trvl_dur  )
		  {
			  var text = 'As per converison policy, you can travel for maximum of '+trvl_dur+' days only. Kindly adjust dates accordingly. For further information refer to the travel Policy  ';// ask
																																																	// for
																																																	// text
																																																	// to
																																																	// madhu
		  ...
		  {
			  var text = 'Currently no assignment models are available for the duration in between 367 to 394 days.(Total duration considered is ( previous travel duration + current travel duration ) ). ';
				sap.ca.ui.message.showMessageBox({
					type: sap.ca.ui.message.Type.ERROR,
					message: text,
					details: text
		...
		// end 19/6/2019 uml6kor
		
		// start 20/6/2019 uml6kor new policy display msg when no assignment
		// model available
		// UCD1KOR 06Feb 2020 added ZZ_ASG_TYP condition sSTVA and VA and VN//
		// UCD1KOR 10 OCT 2020  ZZ_DEP_TYPE == "INTL" added
		if(aData.screenData.ZZ_DEP_TYPE == "INTL" && parseInt(aData.screenData.ZZ_DEP_DAYS) > '367' && parseInt(aData.screenData.ZZ_DEP_DAYS) < '395' &&  trvl_dur == 0 && aData.screenData.ZZ_ASG_TYP != "STVA" && trvl_dur == 0 && aData.screenData.ZZ_ASG_TYP != "VA"  && aData.screenData.ZZ_ASG_TYP != "VN"){
			
				  var text = 'Currently no assignment models are available for the duration in between 368 to 394 days. ';
					sap.ca.ui.message.showMessageBox({
						type: sap.ca.ui.message.Type.ERROR,
						message: text,
						details: text
		...
		// end 20/6/2019 uml6kor new policy display msg when no assignment model
		// available
		}// ######## UCD1KOR 06 May 2020 ########// end
...
// aData.screenData.ZZ_DEP_STDATE_ERROR = "Error";
// err = "For transfer request, start date must always be start day of the
// month";
// sap.ca.ui.message.showMessageBox({
// type: sap.ca.ui.message.Type.ERROR,
// message: err,
...
// }
			...
			if (parseInt(aData.screenData.ZZ_DEP_DAYS) > 0) {
				...
				if(aData.screenData.ZZ_TRV_CAT == "WRKP" && aData.screenData.ZZ_DEP_TYPE == "INTL")
				{
					...
					if(oCrDt.getTime() > oDecDt.getTime()){
						// UCD1KOR 7 May 2020 Covid-19 changes && that.endDate
						// >= that.ext_date
						if( that.durationDays > that.covid_max && globalData.changeType == "DB" && that.curDate > that.startDate  && parseInt(aData.screenData.ZZ_DEP_DAYS) > that.covid_max ){
							// no
							err = "";
					...
					// UCD1KOR 7 May 2020 Covid-19 changes
					if(that.durationDays > that.covid_max && globalData.changeType == "DB" && that.curDate > that.startDate  && parseInt(aData.screenData.ZZ_DEP_DAYS) > that.covid_max ){
						// no
						err = "";
					}
		...
		if(that.durationDays > that.covid_max && globalData.changeType == "DB" && that.curDate > that.startDate  && parseInt(aData.screenData.ZZ_DEP_DAYS) > that.covid_max){
			...
			// start of code uml6kor newpolicy_CR_DE 16.9.2019
			var np_date = sap.ui.getCore().getModel("global").getData().npde_date; // added
																					// by
																					// uml6kor
																					// effective
																					// from
																					// date
																					// 25.9.2019
			// start-fix of created date 11/11/2019 uml6kor
			var begin_date = oDeputationThis.getView().getModel().getData().screenData.ZZ_BEGDA;
			if(oDeputationThis.getView().getModel().getData().screenData.ZZ_BEGDA == undefined)
			{
				begin_date = new Date();
			}
			// end-fix 11/11/2019 uml6kor
			
			if(aData.screenData.ZZ_TRV_CAT == "WRKP" && aData.screenData.ZZ_DEP_TOCNTRY == "DE" && begin_date >= np_date)// oDeputationThis.getView().getModel().getData().screenData.ZZ_BEGDA
																															// >=
																															// np_date
																															// )
			{
			...
			if((durStEn) >= 180){
				...
				if((aData.screenData.ZZ_DEP_STDATE.substr(6,8) == aData.screenData.ZZ_DEP_ENDATE.substr(6,8) ) && aData.screenData.ZZ_DEP_ENDATE.substr(6,8) <= "03") // if
																																										// both
																																										// dates
																																										// are
																																										// eq
																																										// 01
				{
					...
					return;
						 ...
						 edate1.getMonth()+1, 0)) && (aData.screenData.ZZ_DEP_STDATE.substr(6,8) <= "03" )) // modified
																											// by
																											// uml6kor
																											// 20/9/2019
				{// //if both start date is 01 and end date has to be end of
					// month(28,29,30,31)
					 if ((aData.screenData.ZZ_DEP_ENDATE.substr(6,8) >= aData.screenData.ZZ_DEP_STDATE.substr(6,8))){// added
																														// by
																														// uml6kor
																														// 24/9/2019
						 sap.ca.ui.message.showMessageBox({
				...
				else if ((aData.screenData.ZZ_DEP_ENDATE.substr(6,8) >= aData.screenData.ZZ_DEP_STDATE.substr(6,8)) && (aData.screenData.ZZ_DEP_STDATE.substr(6,8) > "03"))// &&
																																											// aData.screenData.ZZ_DEP_STDATE.substr(6,8)
																																											// !=
																																											// "01")
	...
	// end of code newpolicy_CR_DE 16.9.2019
		...
		// Start of changes UEA6KOR_29.5.2019
		var oCrDt = new Date(new Date().getFullYear(),new Date().getMonth(),new Date().getDate());
	    if(aData.screenData.ZZ_BEGDA!="" && aData.screenData.ZZ_BEGDA)				    
		   oCrDt = new Date(aData.screenData.ZZ_BEGDA.substr(0, 4), aData.screenData.ZZ_BEGDA.substr(4, 2) - 1, aData.screenData.ZZ_BEGDA.substr(6, 2));
		var oDecDt = new Date(2017, 11, 31);
		
		if(aData.screenData.ZZ_DEP_TYPE == "INTL" && oCrDt.getTime() > oDecDt.getTime()){
			var max = 0;
			var min = 0;
			
			for (var i = 0; i < globalData.DepTravelType.length; i++) {
				if(aData.screenData.ZZ_DEP_TOCNTRY == "DE" ){
					if(aData.screenData.ZZ_DEP_TOCNTRY == globalData.DepTravelType[i].ZzCountry &&
							aData.screenData.ZZ_REQ_TYP ==	globalData.DepTravelType[i].ZzTrvKey &&
							aData.screenData.ZZ_DEP_TYPE ==	globalData.DepTravelType[i].ZzDepSubType &&
							aData.screenData.ZZ_TRV_CAT ==	globalData.DepTravelType[i].ZzTrvCat ){
						
						max = globalData.DepTravelType[i].ZzMaxDur;
						min = globalData.DepTravelType[i].ZzMinDur;
						
						if(parseInt(aData.screenData.ZZ_DEP_DAYS) <= max && parseInt(aData.screenData.ZZ_DEP_DAYS) >= min ){
							// allow to summit for all countries
							/*
							 * if(parseInt(aData.screenData.ZZ_DEP_DAYS) >=
							 * "152" &&
							 * aData.screenData.ZZ_DEP_ENDATE.substr(6,2) < "05" ){
							 * 
							 * aData.screenData.ZZ_DEP_ENDATE_ERROR = "Error";
							 * 
							 * var text = 'Error: Please select the dates from
							 * previous month or after 04th of this month, as
							 * your duration is more than 152 days. This is as
							 * per RBEI travel policy for Germany travels' ;
							 * sap.ca.ui.message.showMessageBox({ type:
							 * sap.ca.ui.message.Type.ERROR, message: text,
							 * details: text }); view.getModel().setData(aData);
							 * return false; break;
							 *  }
							 */ // commented by uml6kor 16/9/2019
						}
						else{
							// not allow to submit and display error for all
							// countries case
							// /########## UCD1KOR 06 May 2020 COnditions
							// removed for duration #####// start
							// / Future uncomment start
							// Covid condiotins Future comment
							// / to check duration for county && that.endDate >
							// that.ext_date
							if(that.durationDays > that.covid_max && globalData.changeType == "DB" && that.curDate > that.startDate && parseInt(aData.screenData.ZZ_DEP_DAYS) > that.covid_max ){
								// no
							}
						...
						// /########## UCD1KOR 06 May 2020 COnditions removed
						// for duration #####// end
					...
					if(     aData.screenData.ZZ_REQ_TYP ==	globalData.DepTravelType[i].ZzTrvKey &&
							aData.screenData.ZZ_DEP_TYPE ==	globalData.DepTravelType[i].ZzDepSubType &&
							aData.screenData.ZZ_TRV_CAT ==	globalData.DepTravelType[i].ZzTrvCat ){
						...
						if(that.durationDays > that.covid_max && globalData.changeType == "DB" && that.curDate > that.startDate && parseInt(aData.screenData.ZZ_DEP_DAYS) > that.covid_max ){
							...
							if(parseInt(aData.screenData.ZZ_DEP_DAYS) > max ){
								// not allow to submit and display error for all
								// countries case
								aData.screenData.ZZ_DEP_ENDATE_ERROR = "Error";
								console.log("testcase22");
								var text =
								  'Error: Maximum allowed travel duration for the selected country is ' + max +' . Please correct the dates accordingly' ;
							sap.ca.ui.message.showMessageBox({
								type: sap.ca.ui.message.Type.ERROR,
...
// End of changes UEA6KOR_29.5.2019
...
// {
// var oCrDt = new Date(aData.screenData.ZZ_BEGDA.substr(0, 4),
// aData.screenData.ZZ_BEGDA.substr(4, 2) - 1,
// aData.screenData.ZZ_BEGDA.substr(6, 2));
// var oDecDt = new Date(2017, 11, 31);
// if(oCrDt.getTime() >= oDecDt.getTime()){
// if(parseInt(aData.screenData.ZZ_DEP_DAYS) > 183){
// err = "Duration should not exceed more than 183 days for STA assignment
// travels";
// aData.screenData.ZZ_DEP_STDATE_ERROR = "Error";
// aData.screenData.ZZ_DEP_ENDATE_ERROR = "Error";
// sap.ca.ui.message.showMessageBox({
...
// }else{
// if(parseInt(aData.screenData.ZZ_DEP_DAYS) > 730){
// err = "Duration should not exceed more than 730 days for STA assignment
// travels";
// aData.screenData.ZZ_DEP_STDATE_ERROR = "Error";
// aData.screenData.ZZ_DEP_ENDATE_ERROR = "Error";
// sap.ca.ui.message.showMessageBox({
...
// }
		...
		if(sap.ui.getCore().getModel("profile").getData().currentRole == "EMP" && 
		   (sap.ui.project.e2etm.util.StaticUtility.checkStvaAsg(aData.screenData.ZZ_ASG_TYP) ||
			...
			// Validate if Recieving Org is entered by UEA6KOR_20.02.2019
			
			if (aData.screenData.ZZ_COMABBR.trim() == "Please Select") {
			// aData.screenData.ZZ_CUST_NAME_ERROR = "Error";
				err = "Please enter Receiving Organization";
				sap.ca.ui.message.showMessageBox({
					type: sap.ca.ui.message.Type.ERROR,
					message: err,
					details: err
			...
			/*
			 * else { aData.screenData.ZZ_CUST_NAME_ERROR = "None"; }
			 */		
	...
	// UCD1KOR Manager Pop Up Added 13 May 2021
	openMngrPopup:function(){
		...
		sap.ui.getCore().getModel("global").getData().depuStatus = "AA003";
	...
	checkPeriod: function(aData){
			...
			if( mgr_name == null || mgr_name == undefined || mgr_name == "" ){
				if (this.oCommonMngrDialog || this.oCommonDialog) {
					try{
						this.oCommonMngrDialog.destroy();
		...
		$.when($.ajax({cache: false,url:sChecPeriod}),$.ajax({cache: false,url:sPastFuture})).done(
				function(oChecPeriod,oPastFuture){
					...
					if( oChecPeriod[0].d.Period_Check.ZZ_COOL_PRD == "" && oChecPeriod[0].d.Period_Check.ZZ_LEAD_TIME == "") { // Validation
																																// is
																																// ok
						oDeputationThis.saveDeputation("AA003");
						//oDeputationThis.openMngrPopup("AA003"); // UCD1KOR Manager Pop up Added 14 May 2021
					} else {  // Show error or warning message
						if( oChecPeriod[0].d.Period_Check.ZZ_COOL_PRD != "" ){    // Show
																					// error
																					// message
						...
						}else if(oChecPeriod[0].d.Period_Check.ZZ_LEAD_TIME != ""){                   // Show
																										// confirmation
...
// "Travel should start from " +
// sap.ui.project.e2etm.util.Formatter.sapDate(oChecPeriod[0].d.Period_Check.ZZ_LEAD_TIME)
// +
...
// });
	...
	// aData.save.ZZ_COMABBR =
	// sap.ui.getCore().byId("RecOrgddId").getSelectedKey(); //Added by UEA6KOR
		aData.save.ZZ_COMABBR = aData.screenData.ZZ_COMABBR;  // Added by
																// UEA6KOR
		
		if(aData.screenData.ZZ_VREASON_EX === "EX"){
			aData.save.ZZ_VREASON_EX = "EX"; 
		}else{
			aData.save.ZZ_VREASON_EX = ""; 
		}
...
/* End-CGGS Changes */		
	...
	onSubmitMgrNTID: function(){
		var that= this;
		var depuStatus = sap.ui.getCore().getModel("global").getData().depuStatus;
//		if(depuStatus == "AA000"){
//			var aData = sap.ui.getCore().getModel("global").getData();
//		}else{
			var aData = oDeputationThis.getView().getModel().getData();
//		}
		...
		sap.ui.getCore().getModel("profile").getData().employeeDetail.ZZ_DEP_GRM = ntidmgr1;
	...
	// Prepare header/details value and call webservice to create new deputation
	// request
	saveDeputation: function(aMode){
		...
		///####################### 29 Jan 2021 UCD1KOR Validation for retirement#################//////
		if(aMode == "AA000" || aMode == "AA003"){
		...
		//############# End Changes for Retirement Date Validation #############///////////
		if (aData.screenData.ZZ_DEP_TYPE == "INTL") {
// Start-New changes
			if (aData.screenData.ZZ_TRV_CAT == "TRFR") {
				
			// if ( aData.screenData.ZZ_DEP_TOCNTRY != "AT") { // Added by
			// UEA6KOR

				if (aData.screenData.ZZ_DEP_STDATE_VALUE > new Date(aData.screenData.ZZ_DEP_STDATE_VALUE.getFullYear(), 
						 aData.screenData.ZZ_DEP_STDATE_VALUE.getMonth(), 1)) { // changes
																				// by
																				// uea6kor
																				// _new
																				// policy_
																				// 18/6/2019
					aData.screenData.ZZ_DEP_STDATE_ERROR = "Error";
					err = "For transfer request, start date must always start date";// between
																					// 01
																					// and
																					// 05";//commented
																					// by
...
/* End-Change Return Dates */						
	...
	managerAction : function(status, statusString) {
		...
		/*aData.ZZ_DEP_ENDATE = data.ZZ_DEP_ENDATE; // Added by ucd1kor
		aData.ZZ_DEP_STDATE = data.ZZ_DEP_STDATE; // Added by ucd1kor
		aData.ZZ_DEP_NAME = data.ZZ_GIVEN_NAME;// Added by ucd1kor
*/
		...
		// uea6kor-uml6kor-uik1kor 21/8/2020 RBEIITAPPPIPELINE-217 for Extension
		// date for Visa
...
/* End-CGGS Changes */
		    		...
		    		var checkFlag = JSON.parse(response.body);
		    			//start of changes uml6kor_18/11/2020 Requirement specification for automation of CL generation for inland deputation RBEIITAPPPIPELINE-681
		    			// Call webservice to approve a request
		    				if(checkFlag.d.getDepuStatus.ZZ_STATUS_DESC == "X" && aData.ZZ_DEP_TYPE == "DOME" && view.getModel().getData().screenData.ZZ_REQ_TYP == "DEPU"){
		    							oDeputationThis.onCLSubmitToEMPButtonClick('');
		    						}
		   					else{
		  							sap.ui.core.routing.Router.getRouter("MyRouter").myNavBack();
	   							}
		    							//end of changes uml6kor_18/11/2020 Requirement specification for automation of CL generation for inland deputation RBEIITAPPPIPELINE-681
		    				}, function(error) {
					...
					if(aData.ZZ_DEP_TOCNTRY == "DE" && aData.ZZ_DEP_TYPE == "INTL"){
						//oDeputationThis.AutomaticGenOfCggsCalSheet(aData, "CGGS Calculation Sheet Generated Successfully");
					}
					},
			});
			post.fail(function(err) {
			...
			sap.ui.project.e2etm.util.StaticUtility.setBusy(false, oHomeThis);
			  var message;
				// start of code exflag uea6kor-uml6kor 24/8/2020
				// RBEIITAPPPIPELINE-217 date change extension
// if(aData.ZZ_VREASON_EX == "EX"){
// message = "Upload new fully approved requisition form for date extension to
// proceed further";
// } //end of code exflag uea6kor-uml6kor 24/8/2020 RBEIITAPPPIPELINE-217 date
// change extension
// else
		...
		var aData = oDeputationThis.getView().getModel().getData().screenData;
		 var stvaFlag = false;
		    if(aData.ZZ_TRV_CAT == "TRFR")
		    	stvaFlag = true;
// //start of code uea6kor-uml6kor 24/8/2020 RBEIITAPPPIPELINE-217 date change
// extension
			var ex_flag = false;
// if(aData.ZZ_VREASON_EX == "EX"){
// ex_flag = true;
// }
// //end of code uea6kor-uml6kor 24/8/2020 RBEIITAPPPIPELINE-217 date change
// extension
///////##################### UCD1KOR 11/09/2020 File Upload #####################?//////////////
			/*stvaFlag = true; UCD1KOR fix*/ 
			sap.ui.project.e2etm.util.StaticUtility.validateUploadCggsFileChange(evt,aData.ZZ_DEP_REQ,aData.ZZ_DEP_PERNR,
				                                                       sap.ui.getCore().byId(id[0]+"--cggsFormsDisplay"),
				                                                       stvaFlag,aData.ZZ_VERSION,ex_flag);
    },
	...
	onDownloadCggsForms:function(evt){
		...
		var stvaFlag = false;
		    if(aData.ZZ_TRV_CAT == "TRFR")
		    	sModule = "SRQ";
// //start of code exflag uea6kor-uml6kor 26/8/2020 RBEIITAPPPIPELINE-217 date
// change extension
// if(aData.ZZ_VREASON_EX == "EX"){
// sModule = "EX";
//			
// }
// ////end of code exflag uea6kor-uml6kor 26/8/2020 RBEIITAPPPIPELINE-217 date
// change extension
...
// "CGF");
	...
	deputationAction : function(status, statusString) {
		...
		if (status.substring(2, 5) == "003" || status.substring(2, 5) == "002") {
			...
			/// UCD1KOR current role
			var currentRole = sap.ui.getCore().getModel("profile").getData().currentRole;
			//start of changes uml6kor_18/11/2020 Requirement specification for automation of CL generation for inland deputation RBEIITAPPPIPELINE-681
				if(aData.ZZ_DEP_TYPE == "DOME" && data.ZZ_REQ_TYP == "DEPU" && currentRole !== "DEPU"){
					aData.ZZ_NEW_COMMENTS = sap.ui.getCore().byId("txtManagerWFComment").getValue() + "";
					status = aData.ZZ_STAT_FLAG = "JJ006";
				}
				else{
		...
		//start of changes uml6kor_18/11/2020 Requirement specification for automation of CL generation for inland deputation RBEIITAPPPIPELINE-681
		if(aData.ZZ_DEP_TYPE == "DOME" && data.ZZ_REQ_TYP == "DEPU" ){
			...
			get.done(function(odata1, header, response) {
				eTag = response.getResponseHeader("etag");
				if (eTag.indexOf("W/")!== -1 || eTag == "W/\"''\"" || eTag == "W/\"'0.0000000%20'\"") {
					eTag = "*";
...
//end of changes uml6kor_18/11/2020 Requirement specification for automation of CL generation for inland deputation RBEIITAPPPIPELINE-681
			...
			var post = $.ajax({
				...
				success: function(result, response, header) {
					...
					if (aData.ZZ_STAT_FLAG == "JJ005" || aData.ZZ_STAT_FLAG == "JJ008" || 
						...
						var ttype = oDeputationThis.getView().getModel().getData().screenData.ZZ_TTYPE;
						if (aData.ZZ_STAT_FLAG == "CLOSE" && ttype == "WTRP") {
							if (header.getResponseHeader('REASON') != "" && 
									header.getResponseHeader('REASON') != null) {
								sap.ca.ui.message.showMessageBox({
									type: sap.ca.ui.message.Type.ERROR,
	...
	visibleEmployeeButtons : function(evt) {
		...
		var profileData = sap.ui.getCore().getModel("profile").getData();
		var viewData = view.getModel().getData();
		// For each button in this list, logic for showing them should be
		// considered
		...
		$.when(oDeputationThis.travelDeffered).then(function(){
			if (sDeputationNo == '') {
				...
				if ((globalData.currentStage == "1" && globalData.currentSubSet == "1_1_1" && // Travel
																								// request
						( viewData.screenData.ZZ_REQ_TYP == "BUSR" || viewData.screenData.ZZ_REQ_TYP == "HOME" || 
								...
								viewData.screenData.ZZ_REQ_TYP == "INFO") ) || 
								(globalData.currentStage == "2" && globalData.currentSubSet == "1_1_1" &&  // Domestic
																											// Deputation
										...
										(globalData.currentStage == "3" && globalData.currentSubSet == "1_1_1" && // International
																													// Deputation
					...
					if (sap.ui.getCore().getModel("profile").getData().currentRole == "EMP" && 
							( action == "000" || action == "008" || 
									...
									sap.ui.getCore().getModel("global").getData().isCreate)) {  // Save,
																								// new
																								// SECO/HOME/EMER,
																								// send
																								// back
	...
	visibleManagerButtons: function(evt) {
		...
		var action = viewData.screenData.ZZ_STAT_FLAG.substring(2, 5);
		if (action == "003") {
			if (prefix == "AA" || prefix == "JJ") { // Submitted in deputation
													// always starts with AA
				// 1. SendBack Button
				...
				// Check if this is the case that a non-employee user submits a
	...
	visibleDeputationButtons: function(evt) {
		var aData = view.getModel().getData();
		var globalData = sap.ui.getCore().getModel("global").getData();
		var currentRole = sap.ui.getCore().getModel("profile").getData().currentRole; // 11/Jun/2021 Thanh - get role
		// For each button in this list, logic for showing them should be
		// considered
		var prefix = aData.screenData.ZZ_STAT_FLAG.substring(0, 2);
		var action = aData.screenData.ZZ_STAT_FLAG.substring(2, 5);
		...
		if (aData.screenData.ZZ_DEP_TYPE == "INTL") {
			...
			// 7. Coc Button
			if (globalData.currentStage == "2" && globalData.currentSubSet == "1_3_1") {
				if (prefix == "JJ" && action == "009" && currentRole === "COC") { // 11/Jun/2021 Thanh - add condition coc dash board 
		...
		if (action != "002") {
			var isVisible = true;
			// 11/Jun/2021 Thanh - COC Dashboard - reject button only show in coc team when status is JJ009.
			if (prefix == "JJ" && action == "009" && currentRole === "DEPU") { 
				isVisible = false;
			}
			view.byId("btnDEPUReject").setVisible(isVisible);
		} else {
	...
	displayFooterButtons: function() {
		...
		if ((aData.screenData.ZZ_REQ_TYP == "DEPU" &&      // Depu check
			...
			if (!globalData.isChange) { // Logic for Forward Process
				// Should not visible any button if it is navigated from my
				// request Tab
				if (globalData.whichTab != "MR") {
					// Set visibility of Middle buttons
					if (currentRole == "EMP") {           // If current role
															// is EMP, visible
															// save button
					...
					} else {                    // If current role is not EMP,
												// invisible save button
						view.byId("btnSave").setVisible(false);
						view.byId("btnSaveTravelPlan").setVisible(false);
					}
					...
					if (currentRole == "EMP") {
						...
						oDeputationThis.inVisibleDeputationButtons();
					} else if (currentRole == "DEPU" || currentRole == "COC") { // 11/Jun/2021 Thanh - add condition coc dash board
						// invisible employee buttons
						oDeputationThis.inVisibleEmployeeButtons();
						// invisible manager buttons
						oDeputationThis.inVisibleManagerButtons();
			...
			if (currentRole == "GRM") {
				if ((aData.screenData.ZZ_REQ_TYP == "DEPU" && aData.screenData.ZZ_STAT_FLAG == "AA001") ||  // DEPU
																											// Check
						(aData.screenData.ZZ_REQ_TYP != "DEPU" &&                         // TRAVEL
																							// Check
	...
	// This method is used for validating the VISA for SELF during uploading
	// visacopy
...
// if(parseInt(aData.selfVisa.ZZ_VISA_EDATE) <
// parseInt(aData.screenData.ZZ_DEP_STDATE)){
// aData.selfVisa.ZZ_VISA_EDATE_ERROR = "Error";
...
// if(parseInt(aData.selfVisa.ZZ_VISA_SDATE) >
// parseInt(aData.screenData.ZZ_DEP_STDATE)){
// aData.selfVisa.ZZ_VISA_SDATE_ERROR = "Error";
...
// }
	...
	// This method is used for validating the VISA for DEPENDENTS during
	// uploading visacopy
...
/* End-CGGS Changes */	
	...
	onViewMainRequest : function(evt) {
		...
		// Getting the details of an existing deputation request including the
		// stage
		var get = $.ajax({
			cache: false,
...
// oDeputationThis.displayFamilyPanel();
				...
				sap.m.MessageBox.confirm(
						...
						function (oAction){
							if(oAction == "OK" ){
								sDeputationNo = '';   
							/////############# UCD1KOR 12 July 2021 changes BUSR Copy #######//////
								globalData.isCopy= false;
								globalData.copyGeneral= false;
								globalData.copyTravelDetails= false;
								globalData.copyCostAssignment= false;
								globalData.copyAdvance= false;
								sap.ui.getCore().getModel("global").refresh();
								sap.ui.core.routing.Router.getRouter("MyRouter").myNavBack();
								return;
							}
	...
	onSavePress : function(evt) {
		...
		//start of code by uml6kor(09/11/2020) for RBEIITAPPPIPELINE-683 Displaying information related to conversion in Deputation dashboard
		var aData = view.getModel().getData();
		/// 09 Apr 2021 UCD1KOR Commented Below if condition
		//if ((aData.screenData.ZZ_TRV_CAT == "TRFR" ||aData.screenData.ZZ_TRV_CAT == "WRKP" ) && aData.screenData.ZZ_DEP_TYPE == "INTL") {
				var mgr_name = sap.ui.getCore().getModel("profile").getData().employeeDetail.ZZ_DEP_GRM;
				var emp_name = sap.ui.getCore().getModel("profile").getData().employeeDetail.ZZ_DEP_NAME;
				if(emp_name == mgr_name){
					mgr_name = null;
				...
				//mgr_name = null;
				if( mgr_name == null || mgr_name == undefined || mgr_name == "" ){
					sap.ui.getCore().getModel("global").getData().depuStatus = "AA000";
...
//end of code by uml6kor(09/11/2020) for RBEIITAPPPIPELINE-683  Displaying information related to conversion in Deputation dashboard
	...
	onEMPSubmitPress : function(evt) {
		if (oDeputationThis.validateDeputation()) {
			var aData = view.getModel().getData();
			if (sDeputationNo == "" || sDeputationNo == null) { // Create a new
																// request
				...
				if (aData.screenData.ZZ_STAT_FLAG != "AA000") {
					if (aData.screenData.ZZ_CONFIRM_FLAG == true){
// oDeputationThis.checkPeriod(aData); 
						/// UCD1KOR Global value addeds
						oDeputationThis.Extension = aData.screenData.ZZ_VREASON_EX;
						oDeputationThis.aData = aData;
						// begin of Changes to familly details
						if( Number(aData.screenData.ZZ_DEP_DAYS) <= 183 && aData.screenData.ZZ_DEP_TOCNTRY == "DE" &&
								aData.screenData.ZZ_TRV_CAT != 'TRFR'){
							// display pop up
							var dialog = new sap.m.Dialog({
								...
								content: [
								          ...
								          beginButton: new sap.m.Button({
								        	  text: 'OK',
								        	  press: function () {
// ////modified start code uea6kor-uml6kor 24/8/2020 RBEIITAPPPIPELINE-217 date
// change extension
								        		  if(sap.ui.getCore().getModel("global").getData().changeType == "DB" && oDeputationThis.Extension == "EX"){
								        			  var aData = view.getModel().getData();
								        				oDeputationThis.onExtListId = "changeEndDateExt--List_ExtDocument_Id";
														oDeputationThis.onExtRepId  = "changeEndDateExt--ExtViewRepeatId";
//													  
														oDeputationThis.currentViewId  = "changeEndDateExt";
											sap.ui.project.e2etm.util.StaticUtility.getExtDocument(
													aData.screenData.ZZ_DEP_TOCNTRY, oDeputationThis.getView().getModel().getData().screenData.ZZ_TRV_CAT, oDeputationThis, 
													aData.screenData.ZZ_DEP_REQ, aData.screenData.ZZ_DEP_PERNR);
											oDeputationThis.contetnDestroyExt();
											if (!oDeputationThis.oCommonDialog) {
												oDeputationThis.oCommonDialog = sap.ui.xmlfragment("sap.ui.project.e2etm.fragment.common.changeEndDateOnDeputation", oDeputationThis);
												oDeputationThis.getView().addDependent(oDeputationThis.oCommonDialog);
											}
											oDeputationThis.oCommonDialog.open();
											sap.ui.getCore().getModel("global").getData().extDoc = true;
											sap.ui.getCore().getModel("global").getData().extDateKey = "01";
											sap.ui.getCore().getModel("global").refresh();
								        		  }else{
								        				sap.ui.getCore().getModel("global").getData().extDoc = true;
								        				
												oDeputationThis.checkPeriod(oDeputationThis.aData);
											    dialog.close();
											}// //modified end code
												// uea6kor-uml6kor 24/8/2020
												// RBEIITAPPPIPELINE-217 date
												// change extension
											  }
								          }),
								          afterClose: function() {
							...
							// RBEIITAPPPIPELINE-217 date change extension
			        		  ...
			        		  if(sap.ui.getCore().getModel("global").getData().changeType == "DB" && aData.screenData.ZZ_VREASON_EX == "EX"){
								var aData = view.getModel().getData();
								oDeputationThis.onExtListId = "changeEndDateExt--List_ExtDocument_Id";
								oDeputationThis.onExtRepId  = "changeEndDateExt--ExtViewRepeatId";
								oDeputationThis.currentViewId  = "changeEndDateExt";
...
// ////modified end code uea6kor-uml6kor 24/8/2020 RBEIITAPPPIPELINE-217 date
// change extension
// oDeputationThis.checkPeriod(aData);
						}
					...
					if (aData.screenData.ZZ_CONFIRM_FLAG == false) {
						...
						aData.screenData.ZZ_CONFIRM_FLAG_ERROR = "None";
						/////////################### UCD1KOR 10/09/2020 Global Variable added ######################//////////
						oDeputationThis.Extention =aData.screenData.ZZ_VREASON_EX ;
						view.getModel().setData(aData);
						// begin of Changes to familly details
						if( Number(aData.screenData.ZZ_DEP_DAYS) <= 183 && aData.screenData.ZZ_DEP_TOCNTRY == "DE" &&
								aData.screenData.ZZ_TRV_CAT != 'TRFR'){
								...
								content: [
								          ...
								          beginButton: new sap.m.Button({
								        	  ...
								        	  press: function () {
													...
													// date change extension
								        		var aData = view.getModel().getData();
								        		if(sap.ui.getCore().getModel("global").getData().changeType == "DB" && oDeputationThis.Extention == "EX"){
								        			oDeputationThis.onExtListId = "changeEndDateExt--List_ExtDocument_Id";
													oDeputationThis.onExtRepId  = "changeEndDateExt--ExtViewRepeatId";
												  	oDeputationThis.currentViewId  = "changeEndDateExt";
													sap.ui.project.e2etm.util.StaticUtility.getExtDocument(
...
// oDeputationThis.checkPeriod(aData);
	...
	onEMPSubmitDocsPress : function(evt) {
		...
		// If none of the visa copy panels is visible, that means we don't need
		// to create a visa plan
	...
	onBeforeEMPSubmitPassportPress : function(){
		...
		var aData = this.getView().getModel().getData().screenData;
			if(aData.ZZ_DEP_TYPE == "INTL"){ //&& aData.ZZ_TRV_CAT =="TRFR"  
				this.handleFeedbackLinkPress();
			}
			else{
				this.onEMPSubmitPassportPress();
			}
	...
	onEMPAcceptCLPress : function(evt) {
		...
		if(sap.ui.project.e2etm.util.StaticUtility.checkStvaAsg(view.getModel().getData().screenData.ZZ_ASG_TYP)){
			...
			///// UCD1KOR 17/06/2021 added below condition 
			if(view.getModel().getData().screenData.ZZ_TRV_CAT == "TRFR" && sap.ui.getCore().getModel("global").getData().changeType !="" ){
		     	 sap.ui.getCore().byId("chkSTVA").setVisible(false);
		     } 
	...
	onAcceptCLWFButtonClick : function(evt) {
		...
		if (comment.trim() == "" || comment.trim() == null) {
			...
			///// UCD1KOR 17/06/2021 included  aData.screenData.ZZ_TRV_CAT !="TRFR" condition 
			 if(aData.screenData.ZZ_ASG_TYP!="NC" && aData.screenData.ZZ_ASG_TYP!="VN" && sap.ui.getCore().byId("chkSTVA").getVisible() == true  && 
...
// }
	...
	onEMPChangePress : function(evt) {
		...
		if (globalData.changeType == "DA") {
			...
			// Get end date of the last home trip and prepare validated start
			// date
			...
			for(var i=0; i<validatedList.length;i++) {
				if ((validatedList[i].ZZ_REQ_TYP == "HOME" || validatedList[i].ZZ_REQ_TYP == "SECO" || 
					...
					// Get the latest day
					if (validatedEndDate != null) { // This will come here for
													// date comparison
						// If current date is less than the current latest date
						// in the list
					...
					} else {  // First loop, it will come here to get the
								// first Date in the list for comparison
						validatedEndDate = validatedList[i].ZZ_DEP_ENDATE;
					}
		...
		if (curDate >= aData.screenData.ZZ_DEP_STDATE_VALUE && curDate <= aData.screenData.ZZ_DEP_ENDATE_VALUE) {
			if (oDeputationThis.validateDeputation()) {
				if (sDeputationNo == "" || sDeputationNo == null) { // Create a
																	// new
																	// request
					if (aData.screenData.ZZ_CONFIRM_FLAG == false) {
						aData.screenData.ZZ_CONFIRM_FLAG_ERROR = "Error";
				...
				if (aData.screenData.ZZ_DEP_TYPE == "INTL") {
					...
					// ############################////////
					// oDeputationThis.oCommonDialog =
					// sap.ui.xmlfragment("sap.ui.project.e2etm.fragment.deputation.COMMON.EMPChangeDialog",
					// oDeputationThis);
					// ///////################################ UCD1KOR Aug 17th
					// Added below Fragment
					// ###########################/////////////
					var aData = view.getModel().getData();
					oDeputationThis.onExtListId = "changeEndDateExt--List_ExtDocument_Id";
					oDeputationThis.onExtRepId  = "changeEndDateExt--ExtViewRepeatId";
					...
					get.done(function(result) {
						...
						if (foundCOC) {
							...
							// /// UCD1KOR Aug 17h Commented below code
					...
					// /// UCD1KOR Aug 17h Commented below code
						/*
						 * oDeputationThis.oCommonDialog.open(); var oModel =
						 * new sap.ui.model.json.JSONModel();
						 * oModel.setData({dummy1: "", dummy2: "", dummy3: "",
						 * dummy4: ""});
...
// }
	...
	contetnDestroyExt:function(){
		try{sap.ui.getCore().byId("ConfirmExtUpload").destroy();}catch(err){} // added
																				// code
																				// uea6kor-uml6kor
																				// 24/8/2020
																				// RBEIITAPPPIPELINE-217
																				...
																				// change
																				// extension
		try{sap.ui.getCore().byId("List_ExtDocument_Id").destroy();}catch(err){}
		try{sap.ui.getCore().byId("ExtViewRepeatId").destroy();}catch(err){}
		try{sap.ui.getCore().byId("FileUploadExtButtonsId").destroy();}catch(err){}
		try{sap.ui.getCore().byId("idListEXT_DocFlexBox").destroy();}catch(err){}
		try{sap.ui.getCore().byId("idActionEXT_DocFlexBox").destroy();}catch(err){}
			  try{
				  sap.ui.getCore().byId("linkTemplate").destroy()
			  }catch(err){}
	},
	onExtDocValidCheck:function(evt){
		var aData = view.getModel().getData();
		var currentViewId = "changeEndDateExt";
		
		var selectedKey = sap.ui.getCore().getModel("global").getData().extDateKey;
		if(selectedKey == "02"){
			oDeputationThis.checkPeriod(aData);
		}
		else{
// var Files = sap.ui.getCore().byId("ExtViewRepeatId").getRows();
			// //modified start code uea6kor-uml6kor 24/8/2020
			// RBEIITAPPPIPELINE-217 date change extension
			if(!sap.ui.getCore().byId(currentViewId+"--checkExtBoxConfirm").getChecked()){
				sap.m.MessageBox.error("Please confirm by selecting the checkbox- 'I have uploaded All the documents'");
				return;	
			}	
			// //modified end code uea6kor-uml6kor 24/8/2020
			// RBEIITAPPPIPELINE-217 date change extension
			
// if(Files.length <3 && aData.screenData.ZZ_DEP_TOCNTRY == "DE"){
// sap.m.MessageBox.error("Please upload three documents");
// return;
// } else if(Files.length <2 && aData.screenData.ZZ_DEP_TOCNTRY != "DE"){
// sap.m.MessageBox.error("Please upload All documents");
// return;
// //add check box
//				
// }
	...
	DateChangeonDep:function(){
		var selectedKey = sap.ui.getCore().getModel("global").getData().extDateKey;
		if(selectedKey =="02"){
			sap.ui.getCore().getModel("global").getData().extDoc = false ;
			sap.ui.getCore().byId("changeEndDateExt--panelExtensionDocs").setVisible(false);
			
		}
		else{
			sap.ui.getCore().getModel("global").getData().extDoc = true ;
			sap.ui.getCore().byId("changeEndDateExt--panelExtensionDocs").setVisible(true);
			}
		sap.ui.getCore().getModel("global").refresh();
	},
	afterClose: function () {
		  if (oDeputationThis.oCommonDialog) {
			  ...
			  oDeputationThis.oCommonDialog.destroy();
			  oDeputationThis.oCommonDialog = null; // make it falsy so that it
													// can be created next time
		  }
		},
	
	onDateSelectionClose:function(){
		oDeputationThis.oCommonDialog.close();
	},
	...
	onUploadVisaCopyPress : function(evt) {
		sap.m.MessageBox.confirm(
				...
				function (oAction){
					if(oAction == "OK" ){
						// Perform an action to create visa plan
						// If none of the visa copy panels is visible, that
						// means we don't need to create a visa plan
						// but we directly update the workflow status
							...
							get.done(function(result, response, header) {
								...
								var post = $.ajax({
									...
									success: function(result) {
										// Update workflow stages
										oDeputationThis.deputationAction("009", "Visa Copy Uploaded");
										sap.ui.core.routing.Router.getRouter("MyRouter").myNavBack(); // 11/Jun/2021 Thanh - COC dashboard - navigate back when confirm visa copy
									},
								});
							});
						}
	...
	onUploadCOCPress : function(evt) {
     ...
     // if (parseInt(aData.screenData.ZZ_COC_STDATE) >
		// parseInt(aData.screenData.ZZ_DEP_STDATE)) {
	 // sap.ca.ui.message.showMessageBox({
	// type: sap.ca.ui.message.Type.ERROR,
	// message: "COC Start Date must be less than or equal to Deputation Start
	// Date",
	// details: "COC Start Date must be less than or equal to Deputation Start
	// Date"
	// });
	// sap.ui.getCore().byId('dateCOCSTDATE').setValueState("Error");
	// return;
	...
	// if (parseInt(aData.screenData.ZZ_COC_EDATE) <
	// parseInt(aData.screenData.ZZ_DEP_ENDATE)) {
	// sap.ca.ui.message.showMessageBox({
	// type: sap.ca.ui.message.Type.ERROR,
	// message: "COC End Date must be greater than or equal to Deputation End
	// Date",
	// details: "COC End Date must be greater than or equal to Deputation End
	// Date"
	// });
	// sap.ui.getCore().byId('dateCOCEDATE').setValueState("Error");
	// return;
	...
	onGeneratePress : function(evt) {
		...
		// for domestic transfer scenario, if no requisition form uploaded,
		// cannot generate contract letter
...
// New Changes-Start
				...
				if (aData.screenData.ZZ_VREASON != "") {  // Set visibility of
															// fields based on
															// Change Process
					sap.ui.getCore().byId("flexBoxMentor").setVisible(false);
					sap.ui.getCore().byId("flexBoxHost").setVisible(false);
					sap.ui.getCore().byId("flexBoxJob").setVisible(false);
					// Start of Changes for CL generation issue
					// UEA6KOR_20.02.2018
					if (( aData.screenData.ZZ_VREASON == "DA" || aData.screenData.ZZ_VREASON == "DR") && (aData.screenData.ZZ_ASG_TYP == "VA" || aData.screenData.ZZ_ASG_TYP == "VN" || aData.screenData.ZZ_ASG_TYP == "STVA" || aData.screenData.ZZ_ASG_TYP == "LTX" || aData.screenData.ZZ_ASG_TYP == "NCX" || aData.screenData.ZZ_ASG_TYP == "STX" )){ 	// uml6kor
																																																																																							// _New
																																																																																							// Policy_
																																																																																							// 18.6.2019
																																																																																							// added
																																																																																							// changes
						// Adding DR Version UEA6KOR_05.03.2019
						sap.ui.getCore().byId("flexBoxMentor").setVisible(true);
						sap.ui.getCore().byId("flexBoxHost").setVisible(true);
						sap.ui.getCore().byId("flexBoxJob").setVisible(true);
			     	...
			     	// End of Changes for CL generation issue UEA6KOR_20.02.2018
				...
				} else {  // Set visibility of fields based on Assigment Model
							// and Deputation type(DOME/INTL)
					if (aData.screenData.ZZ_DEP_TYPE == "DOME") {
						sap.ui.getCore().byId("flexBoxMentor").setVisible(false);
						sap.ui.getCore().byId("flexBoxHost").setVisible(false);
						sap.ui.getCore().byId("flexBoxJob").setVisible(false);
						...
						if (aData.screenData.ZZ_ASG_TYP == "STA" || aData.screenData.ZZ_ASG_TYP == "TRNG" ||
								aData.screenData.ZZ_ASG_TYP == "STAPP") {   // if
																			// assignment
																			// model
																			// is
																			// STA
							...
							sap.ui.getCore().byId("flexBoxHost").setVisible(false);
							sap.ui.getCore().byId("flexBoxJob").setVisible(false);
						} else {  // Otherwise display all fields for
									// international deputation process
							sap.ui.getCore().byId("flexBoxMentor").setVisible(true);
							sap.ui.getCore().byId("flexBoxHost").setVisible(true);
							sap.ui.getCore().byId("flexBoxJob").setVisible(true);
							sap.ui.getCore().byId("txtHost").setValue(aData.screenData.ZZ_CUST_NAME);
	...
	onGenerateCLButtonClick : function(evt) {
		...
		// Start of changes UEA6KOR_20.02.2019
		if (aData.screenData.ZZ_VREASON == "DA" || aData.screenData.ZZ_VREASON == "DR") {   // Adding
																							// DR
																							// Version
																							// UEA6KOR_05.03.2019
			if (aData.screenData.ZZ_DEP_TYPE == "INTL" && aData.screenData.ZZ_ASG_TYP != "STA" &&
		...
		// End of changes UEA6KOR_20.02.2019
	...
	ExtensionServiceIntegrtion : function(){
		var aData = view.getModel().getData().screenData;
		//UCD1KOR 16 Jun 2021 added below error handling
		try{
			var file = sap.ui.getCore().byId("extdoclistdepuDetail--ExtViewRepeatId").getModel().getData().FileUpload;
		}catch(err){}
			...
			for(var i=0;i<file.length;i++){
				if(file[i].Checked === true){
					if(fileName === undefined){
						fileName =  file[i].FileName.concat(file[i].Extension);
	                }else{
	                	fileName = fileName +"*"+ file[i].FileName.concat(file[i].Extension);
	                }
	            }
			}
		}
		if(fileName == undefined){
			...
			allFileNames = fileName.toUpperCase();

		}
		// //modified start code uea6kor-uml6kor 24/8/2020 RBEIITAPPPIPELINE-217
		// date change extension
		if(	allFileNames == ""){
			return false;
			
		}
		// //modified end code uea6kor-uml6kor 24/8/2020 RBEIITAPPPIPELINE-217
		// date change extension
		
		var sExtensionDate = sServiceUrl + "SendExtMailDepu?AsgTyp='" + aData.ZZ_ASG_TYP +
		"'&begda='" + aData.ZZ_BEGDA +
		"'&Deptype='" + aData.ZZ_TRV_CAT +
		"'&DRNo='" + aData.ZZ_DEP_REQ +
		"'&ename='" + aData.ZZ_GIVEN_NAME +
		"'&endda='" + aData.ZZ_DEP_ENDATE + 
		"'&extDate='" + aData.ZZ_DEP_ENDATE +  
		"'&FileName='" + allFileNames + 
		"'&ModPur='" + "" +  
		"'&Pernr='" + aData.ZZ_DEP_PERNR + 
		"'&RecOrg='" + aData.ZZ_COMABBR +  
		"'&Status='" + aData.ZZ_STAT_FLAG +  
		"'&ToCntryTxt='" + aData.ZZ_DEP_TOCNTRY_TXT +  
		"'&ToCountry='" + aData.ZZ_DEP_TOCNTRY +  
		"'&TRNo='" + aData.ZZ_TRV_REQ +  
		"'&$format=json";
		var get = $.ajax({
			cache: false,
			url: sExtensionDate,
	...
	onCLSubmitToEMPButtonClick : function(evt) {
		...
		// Validate if the comment is entered
		//start of changes uml6kor_18/11/2020 Requirement specification for automation of CL generation for inland deputation RBEIITAPPPIPELINE-681
		if(aData.screenData.ZZ_DEP_TYPE == "DOME" && aData.screenData.ZZ_REQ_TYP == "DEPU" && currentRole !== "DEPU" ){//
			var strStaDeMode = "";//sap.ui.getCore().byId("cbClFormat").getSelectedKey();
		...
		if(aData.screenData.ZZ_VREASON_EX == "EX"){
			...
			var fileValue = oDeputationThis.ExtensionServiceIntegrtion();
			
			if(fileValue == false){
				sap.ca.ui.message.showMessageBox({
					type: sap.ca.ui.message.Type.ERROR,
					message: "Please select atleast one file.",
					details: "Please select atleast one file."
				});
			return;
	...
	onSendBackPress : function(evt){
		// Service check UCD1KOR 04 May 2020
		var aData = view.getModel().getData().screenData;
		...
		if (comment.trim() == "" || comment.trim() == null) {
		  ...
		  if(sap.ui.getCore().getModel("profile").getData().currentRole != "EMP" &&
			...
			if (aData.screenData.ZZ_REQ_TYP == "DEPU" && 
			  ...
			  if(sap.ui.getCore().getModel("profile").getData().currentRole == "EMP"){// then
																						// Employee
																						// clicks
																						// on
																						// Send
...
// oThis.getView().byId("txtReject").setVisible(true);
	// sap.ui.getCore().byId("txtReject").setVisible(true); //change by uml6kor
	// 06_05_2019
		if (comment.trim() == "" || comment.trim() == null) {
			sap.ca.ui.message.showMessageBox({
	...
	// /######### UCD1KOR 04 May 2020 ###########// Service call check end
	...
	onApprovePress : function(evt){
		...
		// removed conditions for duration
		// Covid condiotins Future comment
		if(aData.screenData.ZZ_DEP_ENDATE == undefined){
			var endDate = new Date(oData.ZZ_DATB1.substr(0, 4), oData.ZZ_DATB1.substr(4, 2) - 1, 
				oData.ZZ_DATB1.substr(6, 2));
			that.startDate = new Date(oData.ZZ_DATV1.substr(0, 4), oData.ZZ_DATV1.substr(4, 2) - 1, 
		...
		if(aData.screenData.ZZ_TRV_CAT =="WRKP" && aData.screenData.ZZ_DEP_TYPE =="INTL"){
			...
			else if(aData.screenData.ZZ_TRV_CAT =="WRKP" && aData.screenData.ZZ_DEP_TYPE =="DOME"){
			max = 183
			}
			else if(aData.screenData.ZZ_TRV_CAT =="TRNG" ){
			max = 90
			}
			else if(aData.screenData.ZZ_TRV_CAT =="TRFR"  && aData.screenData.ZZ_DEP_TYPE =="INTL" ){
			max = 4380
			}
			...
			else if(aData.screenData.ZZ_TRV_CAT =="HOME" || aData.screenData.ZZ_TRV_CAT =="ERMR" || aData.screenData.ZZ_TRV_CAT =="SECO" ){
			max = 30
			}else if(aData.screenData.ZZ_TRV_CAT =="BUSR" && aData.screenData.ZZ_DEP_TYPE =="INTL"){
			max = 90
			}
			else if(aData.screenData.ZZ_TRV_CAT =="BUSR" && aData.screenData.ZZ_DEP_TYPE =="DOME"){
			max = 89
			}
		if(aData.screenData.ZZ_DEP_DAYS != undefined && that.durationDays > max && parseInt(aData.screenData.ZZ_DEP_DAYS) > max && endDate > ext_date){
			aData.screenData.ZZ_TRV_PUR ="COVI";
		}
		// ####### UCD1KOR 06 May 2020 Conditions ########// end
// start code uea6kor-uml6kor 24/8/2020 RBEIITAPPPIPELINE-217 date change
// extension
		try{
			if(aData.screenData.ZZ_VREASON == "DB" && aData.screenData.ZZ_VREASON_EX == "EX"){
				oDeputationThis.contetnDestroyExt();
				sap.ui.getCore().getModel("global").getData().extDoc = true;
			}
		}catch(err){			sap.ui.getCore().getModel("global").getData().extDoc = false;
		}
// end code uea6kor-uml6kor 24/8/2020 RBEIITAPPPIPELINE-217 date change
// extension
		if (aData.screenData.ZZ_REQ_TYP == "DEPU" && 
				(aData.screenData.ZZ_TRV_REQ == "0000000000" || aData.screenData.ZZ_TRV_REQ == "" ||
						aData.screenData.ZZ_TRV_REQ == null)) {
// if(sap.ui.getCore().getModel("global").getData().changeType == "DB"){
//  
...
/* Commented as a part of change */								
					...
					sap.ui.project.e2etm.util.StaticUtility.fetchCGGSChecklistForms(aData.screenData,sap.ui.getCore().byId("grmDetailedCggsChecklist--cggsFormsDisplay"));
						oDeputationThis.oCommonDialog.open();
					// uea6kor-uml6kor 26/8/2020 RBEIITAPPPIPELINE-217 date
					// change extension
					if(aData.screenData.ZZ_VREASON == "DB" && aData.screenData.ZZ_VREASON_EX == "EX"){
						sap.ui.getCore().getModel("global").getData().extDoc = true;
						sap.ui.getCore().byId("extdoclistgrmDetailed--panelExtensionDocs").setVisible(true);
							oDeputationThis.onExtListId = "extdoclistgrmDetailed--List_ExtDocument_Id";
						oDeputationThis.onExtRepId  = "extdoclistgrmDetailed--ExtViewRepeatId";
						oDeputationThis.currentViewId  = "extdoclistgrmDetailed";
						sap.ui.project.e2etm.util.StaticUtility.getExtDocument(
							aData.screenData.ZZ_DEP_TOCNTRY, oDeputationThis.getView().getModel().getData().screenData.ZZ_TRV_CAT, oDeputationThis, 
							aData.screenData.ZZ_DEP_REQ, aData.screenData.ZZ_DEP_PERNR);
					...
					oDeputationThis.contetnDestroyExt();
						...
						try{sap.ui.getCore().byId("extdoclistgrmDetailed--ConfirmExtUpload").setVisible(false);}
						catch(err){}
						}
					else{
						sap.ui.getCore().byId("extdoclistgrmDetailed--panelExtensionDocs").setVisible(false);
			...
			/////////////////////################################## UCD1KOR 24 Mar 2021 ######################/////////////
			//////// Hide Icon for Fund F08.
			if(aData.screenData.ZZ_GEBER.indexOf("F08") !== -1){
				sap.ui.getCore().getModel("global").getData().hideImgeForF08 = false;
			}
			else{
				sap.ui.getCore().getModel("global").getData().hideImgeForF08 = true;
			}
			sap.ui.getCore().getModel("global").refresh();
			...
			sap.ui.project.e2etm.util.StaticUtility.setBusy(false,oHomeThis);
			sap.ui.getCore().byId("cbSubType").setSelectedKey(aData.screenData.ZZ_DEP_SUB_TYPE);
// if (aData.screenData.ZZ_DEP_SUB_TYPE == "" ||
// aData.screenData.ZZ_DEP_SUB_TYPE == "*") {
...
// }
	...
	onManagerApproveClick : function(evt) {
		...
		if (strAssModel == "" || strAssModel == null || strAssModel == "Please select") {
			...
			// // Covid19 conditions check 07 May 2020 UCD1KOR
				// / Start
				that.covid_max = that.covid_19COnditionCheck(aData.screenData);
				that.endDate = new Date(aData.screenData.ZZ_DEP_ENDATE.substr(0, 4), aData.screenData.ZZ_DEP_ENDATE.substr(4, 2) - 1, 
						aData.screenData.ZZ_DEP_ENDATE.substr(6, 2));
				...
				that.durationDays = Math.round(Math.abs((that.startDate - that.ext_date) / oneDay));
			// // Covid19 conditions check 07 May 2020 UCD1KOR && that.endDate
			// >= that.ext_date
				// / Start
				if(that.durationDays > that.covid_max && parseInt(aData.screenData.ZZ_DEP_DAYS) > that.covid_max ){
					// no
					...
					if ( sap.ui.getCore().getModel("global").getData().CoolingPeriodCheck.EXCEPTION == "") {
						...
						if ((nextDateObj > startDateObj && aData.screenData.ZZ_VREASON == "") || 
							...
							if( sap.ui.getCore().getModel("global").getData().CoolingPeriodCheck.NEXT_DATE > aData.screenData.ZZ_DEP_STDATE  ){
// var text = 'Please check your information. You can only travel to Germany on
// "STA Assignment Model" from '
// + sap.ui.project.e2etm.util.Formatter.sapDate(nextDate) + ' for a maximum of
// ' +
// sap.ui.getCore().getModel("global").getData().CoolingPeriodCheck.MAX_DURATION
// + ' days';
// var text = 'Please check your information. You can only travel to Germany on
// "STA Assignment Model" from '
// + sap.ui.project.e2etm.util.Formatter.sapDate(nextDate);
// var text = 'Associate can travel to Germany on "STA Assignment Model" only
// from '
// + sap.ui.project.e2etm.util.Formatter.sapDate(nextDate)
// // + ' for a deputation of ' +
// sap.ui.getCore().getModel("global").getData().CoolingPeriodCheck.MAX_DURATION
...
// + ' days as per RBEI Cooling period guidelines.';
								 ...
								 * else{ // var text = 'Please check your
								 * information. You can only travel to Germany
								 * on "STA Assignment Model" from ' // +
								 * sap.ui.project.e2etm.util.Formatter.sapDate(nextDate) + ".
								 * Do you want to contunue ?"; var text =
								 * 'Associate can travel to Germany on "STA
								 * Assignment Model" only from ' +
								 * sap.ui.project.e2etm.util.Formatter.sapDate(nextDate) // + '
								 * for a deputation of ' +
								 * sap.ui.getCore().getModel("global").getData().CoolingPeriodCheck.MAX_DURATION + '
								 * for a deputation of ' + sMaxday + ' days as
								 * per RBEI Cooling period guidelines. Do you
								 * want to continue?'; sap.m.MessageBox.confirm(
								 * text, function (oAction){ if (oAction ==
								 * "OK") { oDeputationThis.managerAction('001',
								 * "Successfully Approved"); } });
								 * sap.ui.project.e2etm.util.StaticUtility.setBusy(false,oHomeThis);
								 * return; }
								 */// commented by uml6kor 7_5_2019
								// uml6kor start_7_5_2019
							else if( (sap.ui.getCore().getModel("global").getData().CoolingPeriodCheck.TRVL_DURATION != "" && 
									sap.ui.getCore().getModel("global").getData().CoolingPeriodCheck.TRVL_DURATION != '0') && 
									(sap.ui.getCore().getModel("global").getData().CoolingPeriodCheck.TRVL_DURATION < parseInt(aData.screenData.ZZ_DEP_DAYS)) )
									{
									sMaxday = sap.ui.getCore().getModel("global").getData().CoolingPeriodCheck.TRVL_DURATION;
									var text = 'As per proposed cooling period conditions, associate  for a  maximum duration of ' + sMaxday
										+ ' days. Kindly contact Deputation team.';
								...
								// end_uml6kor_7_5_2019
						}
					}
				}
				
				
			}else if (aData.screenData.ZZ_DEP_FRCNTRY != aData.screenData.ZZ_DEP_TOCNTRY  && ( strAssModel == "STA" || strAssModel == "STAPP" || strAssModel == "STX" )) { // added
																																											// stx
																																											// 16/1/2020
				// // Covid19 conditions check 07 May 2020 UCD1KOR
				// / Start
				...
				that.durationDays = Math.round(Math.abs((that.startDate - that.ext_date) / oneDay));
				// // Covid19 conditions check 07 May 2020 UCD1KOR
				// / end with if conditions && that.endDate >= that.ext_date
				if(that.durationDays > that.covid_max && parseInt(aData.screenData.ZZ_DEP_DAYS) > that.covid_max ){
					// no
					oDeputationThis.managerAction('001', "Successfully Approved");
				}else{
					// start of code uml6kor 26_9_2019
					var nextDate = sap.ui.getCore().getModel("global").getData().CoolingPeriodCheck.NEXT_DATE;
					var sMaxday = 0;
					if(parseInt(sap.ui.getCore().getModel("global").getData().CoolingPeriodCheck.MAX_DURATION) > sap.ui.getCore().getModel("global").getData().coolingPeriodMaxdays){
						sMaxday = sap.ui.getCore().getModel("global").getData().coolingPeriodMaxdays;
					...
					// end of code uml6kor 26_9_2019
					
					if( sap.ui.getCore().getModel("global").getData().CoolingPeriodCheck.NEXT_DATE > aData.screenData.ZZ_DEP_STDATE  ){	
						
							var coolStartDate = sap.ui.getCore().getModel("global").getData().coolingPeriodStartDate;
							if(strAssModel == "STX"){ // added stx for stx
														// cooling off by
														// uml6kor 16/1/2020
...
//		//start of changes uml6kor_18/11/2020 Requirement specification for automation of CL generation for inland deputation RBEIITAPPPIPELINE-681
//		// Call webservice to approve a request
//		if(aData.screenData.ZZ_DEP_TYPE == "DOME" && aData.screenData.ZZ_REQ_TYP == "DEPU"){
//			oDeputationThis.onGeneratePress('');
//		}
//		else{
			oDeputationThis.managerAction('001', "Successfully Approved");
//		}
	...
	onManagerTRApproveClick : function(evt) {
		...
		if(data.screenData.ZZ_GEBER.indexOf("F08") == -1){
			var get = $.ajax({
				cache: false,
			...
			get.done(function(output){
				try {
					...
					if (sap.ui.project.e2etm.util.StaticUtility.noBudgetCheck(data.screenData.ZZ_STAT_FLAG, sHasBudget, 
						...
						that.ManagerApproveServiceCallLatest(data,reqNo);
					} else {
						oDeputationThis.oCommonDialog.close();
						sap.ui.project.e2etm.util.StaticUtility.setBusy(false,oHomeThis);
						sap.ca.ui.message.showMessageBox({
	...
	// sap.ca.ui.message.showMessageBox({
	// type: sap.ca.ui.message.Type.ERROR,
	// message: "Cannot approve this request because budget is not available.
	// Contact respective Budget Center owner and CTG BU-Coordinators.\n\n" +
	...
	// "5. Mr. Nishchay Mallaraj Urs (RBEI/CTG36) / 6757-3123 / Corporate
	...
	// });
			...
			var val = data.screenData.ZZ_VREASON;
			if(val == "DB" || val == "TA"){
				var indicator = "ADJ";
			}
			...
			var get = $.ajax({
				cache: false,
				url: sServiceUrl + "ProjChckBudget?ZZ_DEP_SUB_TYP='" + sap.ui.getCore().byId("cbSubType").getSelectedKey() + 
				"'&ZZ_PERNR='" + data.screenData.ZZ_DEP_PERNR + 
				"'&ZZ_MGR_PERNR='" + sap.ui.getCore().getModel("profile").getData().employeeDetail.ZZ_DEP_PERNR + 
				"'&ZZ_REINR='" + reqNo + "'&ZZ_STAT_FLAG='" + data.screenData.ZZ_STAT_FLAG + 
				"'&ZZ_TTYPE='" + data.screenData.ZZ_REQ_TYP +
				"'&Indicator='"+ indicator +"'&$format=json",
				type: "GET"
			});
			get.done(function(output) {
				try{
	...
	ManagerApproveServiceCallLatest:function(data,reqNo){
	...
	onExtRemovePress: function(evt){
		var ext="ExtFile";
		var aData = view.getModel().getData();
		var currentViewId = evt.getSource().getParent().sId.split("--")[0];
		oDeputationThis.onExtListId = currentViewId+"--List_ExtDocument_Id";
		oDeputationThis.onExtRepId  = currentViewId+"--ExtViewRepeatId";
		oDeputationThis.currentViewId  = currentViewId;
			sap.ui.project.e2etm.util.StaticUtility.removeFile(oDeputationThis, sap.ui.getCore().byId(currentViewId+"--List_ExtDocument_Id").getModel(),ext);
		/*sap.ui.project.e2etm.util.StaticUtility.getExtDocument(
			aData.screenData.ZZ_DEP_TOCNTRY, oDeputationThis.getView().getModel().getData().screenData.ZZ_TRV_CAT, oDeputationThis, 
			aData.screenData.ZZ_DEP_REQ, aData.screenData.ZZ_DEP_PERNR);*/
	},
	...
	onExtFileSelectPress : function(curEvt) {
		...
		var currentViewId = curEvt.getSource().getParent().sId.split("--")[0];
			sap.ui.project.e2etm.util.StaticUtility.openDeputationExtFileDialog(oDeputationThis, 
				sap.ui.getCore().byId(currentViewId+"--List_ExtDocument_Id").getModel(),ext);
	},
	...
	onExtFileDownloadPress : function(curEvt) {
		var currentViewId = curEvt.getSource().getParent().getParent().sId.split("--")[0];
		var ext="ExtFile"+"#"+currentViewId;
		sap.ui.project.e2etm.util.StaticUtility.donwloadSelectedFile(ext);
	},
	...
	onDeputationExtFileDialogClose : function(evt) {
		...
		sap.ui.project.e2etm.util.StaticUtility.closeFileDialog(oDeputationThis, sap.ui.getCore().byId(currentViewId+"--List_ExtDocument_Id").getModel());
	},
	...
	onDeputationExtFileDialogUpload:function(evt){
		var ext="ExtFile";
		var currentViewId = evt.getSource().mEventRegistry.press[0].oListener.currentViewId;
		sap.ui.project.e2etm.util.StaticUtility.uploadDocument(oDeputationThis,sap.ui.getCore().byId(currentViewId+"--List_ExtDocument_Id").getModel(),ext);
	},
...
// }
	...
	///// on press feedback link press from pp collection stage
	///// UCD1KOR/UML6KOR 17 May 2021
	handleFeedbackLinkPress:function(evt){
		var that= this;
		if(oDeputationThis.getView().getModel().getData().screenData.ZZ_STAT_FLAG == "JJ010" && !sap.ui.getCore().byId("checkBoxConfirm1").getChecked()){
			sap.ca.ui.message.showMessageBox({
				type: sap.ca.ui.message.Type.ERROR,
				message: "Please check on 'I have submitted the required documents'",
				details: "Please check on 'I have submitted the required documents'"
...
//		var sDeputationNo = evt.getSource().getText();
		var screenData = this.getView().getModel().getData().screenData;
		var sDeputationNo = screenData.ZZ_DEP_REQ;
		var get = $.ajax({
			cache: false,
			url: sServiceUrl + "DEP_HDR_INFOSet(ZZ_DEP_REQ='" + sDeputationNo + "',ZZ_VERSION='')?$expand=HDR_ITEM&$format=json",
			type: "GET"
		...
		get.done(function(data, header, response) {
				...
				that.feedbackgetservicecall();
				if (!that.oFeebackCommonDialog) {
						that.oFeebackCommonDialog = sap.ui.xmlfragment("sap.ui.project.e2etm.fragment.deputation.COMMON.FeedBackDialog", that);
						that.getView().addDependent(that.oFeebackCommonDialog);
				} 
				that.oFeebackCommonDialog.open();
		});
	},
	onCloseFeedbackDialog:function(){
		this.oFeebackCommonDialog.close();
	},
	onRatingChange:function(){
		this.getView().getModel("QAModel").refresh();
	},
	feedbackgetservicecall:function(){

		var role = sap.ui.getCore().getModel("profile").getData().currentRole;
		var screenData = this.getView().getModel().getData().screenData;
		screenData.ZZ_REQ_TYP = "DEPU";
		/*screenData.ZZ_ASG_TYP = "STX";*/
		var oDataModel = new sap.ui.model.odata.ODataModel(sServiceUrl);
		sap.ui.project.e2etm.util.StaticUtility.setBusy(true,oDeputationThis);
		 var queryURL = "FeedbackFormSet?$filter=Role eq '" +role+"' and TravelType eq '" + screenData.ZZ_REQ_TYP +"' and AssignmentModel eq '"+ screenData.ZZ_ASG_TYP +"' and EmployeeNumber eq '"
		 + screenData.ZZ_DEP_PERNR +"' and DeputationRequest eq'"+ screenData.ZZ_DEP_REQ +"'and Country eq '" +screenData.ZZ_DEP_TOCNTRY+"' &$format=json";
		oDataModel.read(queryURL, null, null,
				true, function(oData, response) {
					...
					for(var i=0;i<data.d.results.length;i++){
						...
						data.d.results[i].Country = screenData.ZZ_DEP_TOCNTRY;
					}
					if(data.d.results[0].Answer != 0 && data.d.results[1].Answer != 0){
						sap.ui.getCore().byId("idFeedbackLink").setVisible(true);
						data.d.editable = false;
						if(oDeputationThis.getView().getModel().getData().screenData.ZZ_STAT_FLAG =="JJ010"){
							data.d.editable = true;
						}
					}
					else{
						sap.ui.getCore().byId("idFeedbackLink").setVisible(false);
						data.d.editable = true;
					}
	...
	/*onSubmitFeedbackDialog: function(){
		if( !sap.ui.project.e2etm.util.StaticUtility.checkRequredDocuments(sap.ui.getCore().byId("List_Document_Id").getModel()) &&
				sap.ui.getCore().byId("List_Document_Id").getItems().length != 0){
			sap.m.MessageBox.confirm(
					"Some documents have not been uploaded, do you want to submit ?",
					function (oAction){
						if(oAction == "OK" ){
							oDeputationThis.onSubmitFeedbackDialogData();
						}}
			);
	},*/
	onSubmitFeedbackDialog:function(){
		var that = this;
		var payload = sap.ui.getCore().getModel("QAModel").getData();
		
		payload.AdditionalFeedback = payload.results[0].AdditionalFeedback;
		payload.RequestHandledby = payload.results[0].RequestHandledby;
		
		for(var i=0;i<payload.results.length;i++){
			if(payload.results[i].Answer == 0 || payload.results[i].Answer == ""){
				sap.m.MessageBox.error("Please give rating for all questions..!!");
				return;
			}
			if((payload.results[i].Answer == 1 || payload.results[i].Answer == 2) && payload.results[i].Suggestions == ""){
				payload.results[i].Suggestions_ERROR = "Error";
				sap.m.MessageBox.error("Please provide Suggestions for the feedback you entered for 1 and 2 star rating...!!");
				sap.ui.getCore().getModel("QAModel").refresh();
				return; 
			}
			else{
				payload.results[i].Suggestions_ERROR = "None";
			}
		}
		if(payload.results[0].AdditionalFeedback == ""){
			sap.m.MessageBox.error("Please enter additional feedback..!!");
			return;
		}
		for ( var i = 0; i < payload.results.length; i++) {
			delete payload.results[i].Suggestions_ERROR;
		}
		payload.FEEDBACKANS = payload.results;
		delete payload.results;
		delete payload.editable;

		sap.ui.project.e2etm.util.StaticUtility.setBusy(true, oDeputationThis);
		var oDataModel = new sap.ui.model.odata.ODataModel(sServiceUrl);
		oDataModel.create("Feedback_HdrSet", payload, {success:jQuery.proxy(function(oData, response) {
			sap.ui.project.e2etm.util.StaticUtility.setBusy(false, oDeputationThis);
			sap.m.MessageToast.show("Feedbackform submitted ..!!");	
			that.oFeebackCommonDialog.close();
			that.onEMPSubmitPassportPress();
			
		},this), error:jQuery.proxy(function(error) {
			sap.ui.project.e2etm.util.StaticUtility.setBusy(false, oDeputationThis);
			sap.m.MessageBox.error("Error on feedback submit");
		},this), async:true});
		
	},
///////////////////////////######################## Feedback end changes ucd1kor/uml6kor ###########################////////////////
