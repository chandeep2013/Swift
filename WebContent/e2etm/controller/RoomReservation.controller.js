...
//	     }
	...
	onRouteMatched:function(evt){
		...
		var role = sap.ui.getCore().getModel("profile").getData().currentRole; /// ucd1kor added below condition
		if(role =="EMP"){
			this.getView().byId("ipHcnty").setEditable(false);
			this.getView().byId("ipHcity").setEditable(false);
		}
		else{
			this.getView().byId("ipHcnty").setEditable(true);
			this.getView().byId("ipHcity").setEditable(true);
		}
...
//				    });
	...
	onSearch:function(evt){
		...
		//Thanh 5.20.2021 wrap to validate function
		var bError = this.validateViewRoomsForm();
		if(bError){
			return;
		}
...
//        	},this));
		...
		//uml6kor_upgrade 12/12/2019 removed HotelToRoomListNav/
		var aFilters = [new sap.ui.model.Filter({path:"Hcnty",operator:"EQ",value1:country}),
         	...
         	new sap.ui.model.Filter({path:"Rsize",operator:"BT",value1:rSizeFrm,value2:rSizeTo}),   
         	new sap.ui.model.Filter({path:"Runit",operator:"EQ",value1:rUnit})
		...
		if(noPers !="")
			aFilters.push(new sap.ui.model.Filter({path:"Nopers",operator:"EQ",value1:noPers}));

		//uml6kor_upgrade 12/12/2019 removed HotelToRoomListNav/		
		var sUri = "/HotelDetailsSet?$filter=Hcnty eq '"+country+"' and Hcity eq '"+city+"' and Begda eq '"+begda+"' and Endda eq '"+endda+"'&$expand=HotelToRoomListNav,HotelToRoomListNav/RoomListToStatusNav";
	...
	prepareData:function(list,vSkip,vTop,type){
		...
		var aFilters = [new sap.ui.model.Filter({path:"Hcnty",operator:"EQ",value1:country}),
                    	new sap.ui.model.Filter({path:"Hcity",operator:"EQ",value1:city}),
                     	new sap.ui.model.Filter({path:"Begda",operator:"EQ",value1:begda}),
                     	new sap.ui.model.Filter({path:"Endda",operator:"EQ",value1:endda})];
		//uml6kor_upgrade 12/12/2019 removed HotelToRoomListNav/ for begda and endda
		...
		/*uml6kor_upgrade 16/12/2019 set data to view
			var oModel =  new sap.ui.model.json.JSONModel();
		    var oData =this.getView().byId("resView").getModel("resStatusModel").getData();
			oModel.setData(oData);
			this.getView().byId("resView").setModel(oModel,"resStatusModel");
//		//uml6kor_upgrade 16/12/2019 set data to view*/
			oCtx.getModel().refresh(); //ucd1kor16/12/2019 refresh model
			list.setBusy(false);
		},this));
...
//		}
		...
		//Thanh 14.05.2021 set visible buttons if navigating from travel plant
		this.getView().byId("rbtnOha").setVisible(true);
		var isNotFromTravelPlan = this.getView().getModel("AccomodationStatus").getProperty("/isNotFromTravelPlan");
		if(!isNotFromTravelPlan) {
			this.getView().byId("btnBook").setVisible(false);
		    this.getView().byId("btnMaint").setVisible(false);
		    this.getView().byId("rbtnOha").setVisible(false);
		}
		/// Ucd1Kor 22/05/2021 accomdation changes for emp
...
//		}, this);
	...
	//Thanh 20.5.2021 Accommodation self booking
	onInformAccommodation: function() {
		var that =this;
		var bError = this.validateViewRoomsForm();
		if(bError){
			return;
		...
		var oView = this.getView();
		var sEmployeeNumber = this["Travel"].Pernr;
		var sTPNumber = this["Travel"].Reinr;
		var sFormCountry = this["Travel"].FromCountryTxt;
		var sFromlocation = this["Travel"].FromLocTxt;
		var sToCountry = oView.byId("ipHcnty").getValue();
		var sToLocation = oView.byId("ipHcity").getValue();
		var sBeginDate = oView.byId("ipBegda").getValue();
		var sEndDate = oView.byId("ipEndda").getValue();
		
		var aFilters = [];
		var urlParameters;
		if (sEmployeeNumber && sTPNumber && sFormCountry && sFromlocation && sToCountry && sToLocation && sBeginDate && sEndDate) {		
			urlParameters = {
					Empno: sEmployeeNumber,
					TPno: sTPNumber,
					FromCntry: sFormCountry,
					FromLoc: sFromlocation,
					ToCntry: sToCountry,
					ToLoc: sToLocation,
					Begda: sBeginDate,
					Endda: sEndDate
			...
			sap.m.MessageToast.show("Data is missing");
			sap.ui.project.e2etm.util.StaticUtility.setBusy(false, this);
			return;
		}
		
		sUrl = "SendMailInfoAccom?Empno='$EmployeeNumber$'&TPno='$TPNumber$'&ToCntry='$ToCountry$'&ToLoc='$ToLocation$'" +
		"&FromCntry='$FormCountry$'&FromLoc='$FormLocation$'&Begda='$BeginDate$'&Endda='$EndDate$'";
		sUrl = sUrl.replace("$EmployeeNumber$",sEmployeeNumber);
		sUrl = sUrl.replace("$TPNumber$", sTPNumber);
		sUrl = sUrl.replace("$ToCountry$", sToCountry);
		sUrl = sUrl.replace("$ToLocation$", sToLocation);
		sUrl = sUrl.replace("$FormCountry$", sToCountry);
		sUrl = sUrl.replace("$FormLocation$", sFromlocation);
		sUrl = sUrl.replace("$BeginDate$", sBeginDate);
		sUrl = sUrl.replace("$EndDate$", sEndDate);
		////// ucd1kor replaced with below code 22/05/2021
		var oDataModel = new sap.ui.model.odata.ODataModel(resServiceUrl);
		oDataModel.read(sUrl, null, null, true, jQuery.proxy(function(oData, response) {
			    sap.m.MessageToast.show("No Rooms Available - Email has been sent to Accommodation team");
				sap.ui.project.e2etm.util.StaticUtility.setBusy(false, that);
				if(sap.ui.getCore().getModel("global").getData().reservation == true){
					sap.ui.getCore().getModel("global").getData().reservation = false;
					sap.ui.getCore().getModel("global").refresh();
					sap.ui.core.routing.Router.getRouter("MyRouter").myNavBack(); 
				}
			},this), function(error) {
				sap.m.MessageToast.show("Email could not be sent to Accommodation team");
				sap.ui.project.e2etm.util.StaticUtility.setBusy(false, that);
			});
		
	},
	//Thanh 20.5.2021 If rooms are not available, turn on "inform accommodation" button
	checkAvailableRooms: function() {
		var oResModel = this.getView().byId("resView").getModel("resStatusModel").getProperty("/");
		var aRoomListToMonth = [];

		oResModel.forEach(function(item) {
			var oHotelToRoomList = item.HotelToRoomListNav.results;
			for (var i = 0; i < oHotelToRoomList.length; i++ ) {
				aRoomListToMonth.push(oHotelToRoomList[i].RoomListToMonthNav.results);
			}
		});
		
		aRoomAvailable = [];
		aRoomListToMonth.forEach(function(item) {
			var oRoomDetailsByDate = item[0].RoomDetlsByDateNav.results;
			var isAvailable = true;
			for(var j = 0; j < oRoomDetailsByDate.length; j++) {
				isAvailable = oRoomDetailsByDate[j].Status === "A";
			}
			aRoomAvailable.push(isAvailable);
		});
		
		var isAvailableRooms = false;
		if(aRoomAvailable.indexOf(true) > -1) {
			isAvailableRooms = true;
		}
		///UCD1KOR added below if condition
		var role = sap.ui.getCore().getModel("profile").getData().currentRole;
		if(role == "EMP" && aRoomAvailable.length == 0){
			//this.onInformAccommodation();
		}
		
		/*this.getView().byId("btnInformAccom").setVisible(!isAvailableRooms);
		this.getView().byId("txtInformAccom").setVisible(isAvailableRooms);*/
	},
	
	//Thanh 20.05.2021 declare new validate function for reusing
	validateViewRoomsForm: function() {
		var oView = this.getView();
		sap.ui.project.e2etm.util.StaticUtility.setBusy(true, this);
		var aInputs = [
			oView.byId("ipHcnty"),
