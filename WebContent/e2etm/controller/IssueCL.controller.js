...
//  if(tocnty == "DE" && )
  ...
  onFilterLetterValues:function(){
	    ...
	    //dye5kor
	     ...
	     if(sap.ui.project.e2etm.util.StaticUtility.checkCggs(tocnty,asgtype,trvcat)){
	        	...
	        	LetterData.push({text:"Both (CL and Calc. sheet)" , key:"CC"}); //changed from "Both" to "Both (CL and Calc. sheet)" uml6kor_3/8/2020 SWIFT: INC000018414449
         	...
         	if(depType == "INTL" || depType == "DOME"){
         		LetterData.push({text:"Extension Contract Letter", key:"EL"}); //added uml6kor_29/7/2020 INC000018414449 CL generation - include the extension letters 	
         	}
	        LetterDataModel.setData(LetterData);
	    	this.getView().byId("ipLtype").setModel(LetterDataModel,"LetterModelData");
  ...
  onLetterSelect:function(){
	...
	if((lType == "CL" || lType == "CC")&&this.getView().byId("ipTocnty").getSelectedKey()=='DE'){
      ...
      }//start of code uml6kor_29/7/2020 INC000018414449 CL generation - include the extension letters 	
      else if(lType == "EL"){
    	  this.getView().byId("lbFormat").setVisible(false);
          this.getView().byId("ipFormat").setVisible(false);
         
      }
	//end of code uml6kor_29/7/2020 INC000018414449 CL generation - include the extension letters 	
...
//	this.getView().byId("ipCurrency").setSelectedKey("");
  ...
  onGenerateCL:function(){
    ...
    var oCs = [];
    var oCa = [];
    var oEl = [];//addition of code uml6kor_29/7/2020 INC000018414449 CL generation - include the extension letters 	
    ...
    for(var i=0;i<items.length;i++){
      ...
      if(lType == "CL"){
        ...
        // batch.push(oDataModel.createBatchOperation("ZE2E_DOCS_DETAILSSet", "POST", oData));
         ...
         oCl.push(oDef1);
          ...
          oData.ZZ_DEP_PERNR = rowData.Pernr;
          oData.ZZ_ASG_TYP = rowData.AsgType;
          oData.ZZ_AUTOMATIC = 'E';
          oData.ZZ_HOSTENTITY = '';
          oData.ZZ_JOB = '';
          oData.ZZ_MENTOR = '';
          oData.ZZ_STADECL_MODE = format;
         // batch.push(oDataModel.createBatchOperation("ZE2E_DOCS_DETAILSSet", "POST", oData));
          var oDef1 = this.postEL(oData);
          oEl.push(oDef1);
        //End of code uml6kor_29/7/2020 INC000018414449 CL generation - include the extension letters         
       } else if(lType == "CS"){
        var oData = {};
     ...
     //   batch.push(oDataModel.createBatchOperation("ZE2E_DOCS_DETAILSSet", "POST", oData));
          ...
          for(var j=0;j<aAmounts.length;j++){ //changed variable i-> j uml6kor 26//9/2019 mail issue.
        	  if(aAmounts[j].AllowanceTyp1){
        		  oData.LetterToAllwNav.push({ZzEmpno: oData.ZzEmpno,ZzDepReq:oData.ZzDepReq,ZzVersion:oData.ZzVersion,
        			                          ZzTrvReq:oData.ZzTrvReq,ZzAllwcTyp:aAmounts[j].AllowanceTyp1,ZzAmount:aAmounts[j].AllowanceAmount1,
        			                          ZzCurr:aAmounts[j].Currency1});
        		  
        	  }
        	  if(aAmounts[j].AllowanceTyp2){
        		  oData.LetterToAllwNav.push({ZzEmpno: oData.ZzEmpno,ZzDepReq:oData.ZzDepReq,ZzVersion:oData.ZzVersion,
        			                          ZzTrvReq:oData.ZzTrvReq,ZzAllwcTyp:aAmounts[j].AllowanceTyp2,ZzAmount:aAmounts[j].AllowanceAmount2,
        			                          ZzCurr:aAmounts[j].Currency2});
...
//			     sap.m.MessageToast.show("Transaction complete.Please check the log for error details(if any)");
		...
		$.when.apply($,oEl).done(jQuery.proxy(function(elData) {  
  ...
  postEL:function(oData){
      ...
      oComponent.getModel().create("ZE2E_DOCS_DETAILSSet", oData, {success:function(oData, response) {
         //oCl[defPos].resolve();      
    	   ...
    	   var title = "";
    	   var message = "";
    	   title = "Error in CL Extension Letter generation for Dep Req No:"+JSON.parse(error.request.body).ZZ_DEP_REQ;
    	   if(error.responseText){
				message = JSON.parse(error.responseText).error.message.value;					
			}else{
				message = JSON.parse(error.response.body).error.message.value;
