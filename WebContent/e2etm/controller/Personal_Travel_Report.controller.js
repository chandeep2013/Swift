...
sap.ui.controller("sap.ui.project.e2etm.controller.Personal_Travel_Report", {
	/**
	...
	onInit : function() {
		...
		this.table = new sap.m.TablePersoController({
			table : this.getView().byId("PersonalTripReport"),
	...
	getReport:function(){
		...
		var StartTo = that.getView().byId("idFromDateRange1S");
		var EndFrom = that.getView().byId("idToDateRangeE");
		var EndTo = that.getView().byId("idToDateRange1E");
		var countryTo =that.getView().byId("idCountry").getSelectedKey();
		try{
...
//		}
		...
		if(Error == ""){
			...
			var AccomDigitized_url = sServiceUrl + "PersonalTravelReport/?FromStartDate='"+dateValue1+"'&ToStartDate='"+dateValue2+"'&FromEndDate='"+dateValue3+"'&ToEndDate='"+dateValue4+"'&ToCnty='"+countryTo+"'";
			var get = $.ajax({
				cache: false,
	...
	onChange : function(evt) {
		...
		this.getView().byId("PersonalTripReport").getBinding("items").aFilters = [];
		this.getView().byId("PersonalTripReport").getBinding("items").filter(filter);
	},
	 excelDownload: function() {
	        var table = this.getView().byId("PersonalTripReport");
	        ...
	        saveAs(blob, "Personal Trip Report.xlsx");
	    },
