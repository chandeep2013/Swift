...
sap.ui.controller("sap.ui.project.e2etm.controller.Cargo", {
	...
	onRouteMatched: function(evt){
		...
		if(sap.ui.getCore().getModel("profile").getData().currentRole == "GRM"){
			this.getView().byId("btnSubmit").setText("Approve");
		}
...
//					this.typeCargo(sCargoType,this, globalData);
			...
			this.getView().byId("idStaticEligibility").bindElement("/CommonSet(MODULE='CARO',SUB_MODULE='',TEXT_NAME='ZE2E_CARGO_ELIGIBILITY')");
			globalData = sap.ui.getCore().getModel("global").getData();
			if(sap.ui.getCore().getModel("profile").getData().currentRole === "CARG"){
				var sRole = "02";
				var sAction ="03";
			}
			else{
			...
			this.getListSearch(sRole,"","CARO",sAction);
		} 
	},
...
//	change Cargo
		...
		if (canContinue){
			...
			oModel.create("/ZE2E_CARGOSet", newCargo, {
				success : jQuery.proxy(function(mResponse) {
					sap.m.MessageToast.show("Your request has been saved.");
					this.dialog.close();
				}, this),
				error : jQuery.proxy(function() {
					sap.m.MessageToast.show("Your request has failed.");
					this.dialog.close();
				}, this)
			});
	...
	typeCargo: function(sCargoType,oThis,globalData){
		if(globalData.ZZ_DEP_TYPE_TXT === "DEPU"){
			sType = "DEPU";
		}
		switch(sCargoType){
		case "O":
...
//		this.oBusyDialog.open();
		...
		var loggedinRole = sap.ui.getCore().getModel("profile").getData().currentRole;
		try{
		...
		if(canContinue){
			...
			oModel.create("/ZE2E_CARGOSet", m2e2_Cargo, {
				success : jQuery.proxy(function(mResponse) {
					...
					if(loggedinRole == "GRM" && sMDRole =="02"){
						sap.m.MessageToast.show("The request has been Approved.");
					}
					else if(loggedinRole == "CARG" && sMDRole =="03"){
						sap.m.MessageToast.show("The request has been Approved.");
					}
					
					if(loggedinRole == "EMP" && sMDAction == "00" ){	//Save
						sap.m.MessageToast.show("The request has been saved.");
					}
					else if(loggedinRole == "EMP" && sMDAction == "01") {
			...
			try{
				this.getView().byId("idIconTabBarMulti").setSelectedKey("detail");
			}catch(err){}
			          ...
			          beginButton: new sap.m.Button({
			        	  ...
			        	  press: function () {
			        		  ...
			        		  get.done(function(result){
			        			  sap.ui.project.e2etm.util.StaticUtility.setBusy(false,oThis);
			        			  sap.m.MessageToast.show("Request is successfully rejected !");
	...
	onSendBackPress : function(oEvent){
		...
		var oRole="";
		var oAction ="";
		//#################### UCD1KOR below if condition added 08 Jan 2021################////////////////
		var loggedinRole = sap.ui.getCore().getModel("profile").getData().currentRole;
		if(loggedinRole == "GRM"){
			oRole="02";
			oAction ="02";
...
//			}
			...
			var oRole = "";
			var oAction = "";
			//################### UCD1KOR below if condition added ################///////////////
			var asgtyp =sap.ui.getCore().getModel("global").getData().ZZ_ASG_TYP;
			var loggedinRole = sap.ui.getCore().getModel("profile").getData().currentRole;
			
				if((asgtyp == "STX" || asgtyp == "LTX" || asgtyp == "NCX" || asgtyp == "STVA"|| asgtyp == "VA"|| asgtyp == "VN") && loggedinRole =="GRM"){
					oRole = "02";
					oAction = "03";
...
//					      new sap.m.Text({text:'Kindly download the excel document and submit later directly to the cargo vendor'}),
					content
				          ...
				          beginButton: new sap.m.Button({
				        	  ...
				        	  press: function () {
				        		  if(loggedinRole =="GRM"){
				        			  oThis.sendPostRequest(oRole,oAction);
				        		  }else{
				        			  var sText = sap.ui.getCore().byId('submitDialogTextarea').getValue();
					        		  var oData = oThis.getView().getModel("new").getData();
				          ...
				          beforeOpen:function(oEvent){
					          	if(loggedinRole =="GRM"){
					          		oEvent.getSource().getBeginButton().setEnabled(true);
					          	}
					          },
				          afterClose: function() {
				        	  dialog.destroy();
				...
				content: [
				          ...
				          beginButton: new sap.m.Button({
				        	  ...
				        	  press: function () {
				        		  ...
				        		  get.done(function(result){
				        			  sap.ui.project.e2etm.util.StaticUtility.setBusy(false,oThis);
				        			  sap.m.MessageToast.show("Request is successfully cancelled !");
				        			  oComponent.getModel().refresh();
				        		  });
				        		  get.fail(function(err) {
				        			  sap.ui.project.e2etm.util.StaticUtility.setBusy(false,oThis);
...
//					info:"Type: {path : 'ZZ_CAR_TYP', formatter: 'sap.ui.project.e2etm.util.Formatter.getTextCargoType'}",
	...
	doFilter:function(sEmpNo){
		...
		case 0:
			...
			this.getListSearch(sRole,sEmpNo,"CARO",sAction);
			this.setEnableButons(true);
			return;
		case 1:
			if(currentrole === "CARG"){
			...
			this.getListSearch(sRole,sEmpNo,"CARR",sAction);
			this.setEnableButons(true);
			return;
		case 2:
...
//		oModel.setHeaders({CARGOTYPE:CARGOTYPE});
	...
	/*UCD1KOR 17 Dec 2019 on press cargo report button press*/
	CargoReportPress: function(){
	...
	//	oGlobal.CARGOTYPE = CARGOTYPE;
		...
		sap.ui.core.routing.Router.getRouter("MyRouter").navTo("Cargo_Reports");
	},
