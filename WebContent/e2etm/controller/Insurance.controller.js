	...
	sap.ui.controller("sap.ui.project.e2etm.controller.Insurance", {
		...
		onRouteMatched:function(oEvent){
			...
			///########### UCD1KOR Nov 18, 2020 #############///////////
			setTimeout(function(){
				var html = that.getView().byId("Insurancehtml");
				html.setContent("");
				html.setContent("<style>li {font-size: 14.5px !important;}</style><div><ul><li>Insurance will be processed for family members only if travelling on “Dependent Visa”.</li>" +
						"<li>New Born kid should be included within 90 days from Date of Birth for issuing insurance</li>" +
						"<li>Insurance policy will be issued as per the dates mentioned in the request here. If there are any changes to the dates (shortening/extension), make sure you update the same here to avoid any recovery during settlement.</li></ul></div>");
			},100);
	...
	//			oEvent.getSource().setValueState('None');
		...
		///// UCD1KOR 17 Nov 2020 hide Dependent For BUSR travel
			if(sType === "BUSR"){
				this.getView().byId("hideDependentForBUSR").setVisible(false);
				this.getView().byId("hideDependentForBUSRToolbar").setVisible(false);
			}
			else{
				this.getView().byId("hideDependentForBUSR").setVisible(true);
				this.getView().byId("hideDependentForBUSRToolbar").setVisible(true);
			}
			...
			var oBatch7 = oComponent.getModel().createBatchOperation("PersonalTripSet(TrvReq='"+sRequest+"',EmpNo='"+sPernr+"',Version='"+sVersion+"')", "GET");
			oComponent.getModel().addBatchReadOperations([oBatch0,oBatch1,oBatch2,oBatch3,oBatch4,oBatch5,oBatch6,oBatch7]);//added batch7 uml6kor_15/9/2020_Personal trip during official travels_RBEIITAPPPIPELINE-176   
			oComponent.getModel().submitBatch(
					function(oResult){
												
						var oData = $.extend({}, oResult.__batchResponses[0].data, oResult.__batchResponses[1].data, 
								oResult.__batchResponses[2].data,oResult.__batchResponses[3].data,oResult.__batchResponses[7].data);
						...
						oData.PersTripFlag = false;
							if(oResult.__batchResponses[7].data.PersFlag == "X"){
				            		oData.PersTripFlag = true;
								}else{
									oData.PersTripFlag = false;
								}
						...
						/*sap.ui.getCore().setModel(oThis.traveller, "traveller");
						sap.ui.getCore().setModel(oThis.dependents, "dependents");*/
						sap.ui.getCore().setModel(new sap.ui.model.json.JSONModel(oThis.traveller), "traveller");
						sap.ui.getCore().setModel(new sap.ui.model.json.JSONModel(oThis.dependents), "dependents");
						oThis.oldTraveller = new sap.ui.model.json.JSONModel();
						oThis.oldTraveller.setData([]);
						oThis.getView().setModel(oThis.oldTraveller,"oldTraveller");
						//############## 17 March 2020 UCD1KOR ################/////// end
						oThis.getView().setModel(oModel, "new");
						oThis.displayNext();
						oThis.getView().byId("iconTabBarId").setSelectedKey(oThis.getView().getId() + "--idIconTabFilter0");
						//TGG1hC
						if(oData.ZZ_TRV_REQ == undefined){
							var trvrvq = oData.TrvReq;

						}
						else{
							trvrvq = oData.ZZ_TRV_REQ;
						}
						sap.ui.project.e2etm.util.StaticUtility.getDocumentList(oThis,trvrvq, oData.ZZ_DEP_PERNR,'INS','INSR',oData.ZZ_LAND1);
						/// Passport UCD1KOR 16 Apr 2021 - to get Passport and Insurance
						//sap.ui.project.e2etm.util.StaticUtility.getDocumentList(oThis,oData.ZZ_TRV_REQ, oData.ZZ_DEP_PERNR,'PS','PassPort',oData.ZZ_LAND1);
						//TGG1HC
		...
		onPressAddRow:function(){
			var that= this;
			var aTable = this.getView().byId("idDependentsTable");
			var length = aTable.getItems().length;
			var paymentDetails = this.getView().getModel("new").getData().ZE2E_INS_HDR.ZZ_SPONSOR;
		...
		if(length <  6){
			...
			//this.dependents[0].ZE2E_INS_ANS
			var trvreq = that.getView().getModel("new").getData().ZZ_TRV_REQ;
			
			if(trvreq === undefined){
				trvreq = that.getView().getModel("new").getData().TrvReq;
			}
			
			var ZE2E_INS_ANS = [{
		        "ZZ_TRV_REQ": trvreq,
		        "ZZ_OWNER": that.getView().getModel("new").getData().ZE2E_INS_HDR.ZZ_OWNER,
		        "ZZ_TRV_KEY": that.getView().getModel("new").getData().ZZ_TRV_TYP,
		        "ZZ_VERSION": that.getView().getModel("new").getData().ZE2E_INS_HDR.ZZ_VERSION,
		        "ZZ_DEPNDT_TYP": "",
		        "ZZ_QA_KEY": "01",
		        "ZZ_ANS": undefined,
		        "ZZ_COMMENTS": ""
			...
			var pushObj = {"ZZ_TRV_REQ":trvreq,
					...
					"ZZ_DOB":"",
					"ZZ_PASSPORT":"",
					"ZZ_BEGDA":that.getView().getModel("new").getData().ZZ_DATV1,
					"ZZ_INS_NO":"",
					"ZZ_PAY_TYP":paymentDetails,
					"ZZ_ENDDA":that.getView().getModel("new").getData().ZZ_DATB1,
					"ZE2E_INS_ANS":ZE2E_INS_ANS
			}
			this.dependents.push(pushObj);
			sap.ui.getCore().setModel(new sap.ui.model.json.JSONModel(this.dependents), "dependents");
			
			this.getView().getModel("new").getData().ZE2E_INS_HDR.ZE2E_INS_DETAILSet = this.traveller.concat(this.dependents);
			this.getView().getModel("new").refresh();
			}
			else{
				sap.m.MessageToast.show("Max Dependents are 6");
			}
		},
		onPressRemove:function(){
            ...
            if(sItems.length == 0){
            	...
            	for (var i = sItems.length - 1; i >= 0; i--) {
                    ...
                    sap.ui.getCore().getModel("dependents").setData(this.dependents);
                    sap.m.MessageToast.show("Successfully removed");
                }
                sap.ui.getCore().setModel(new sap.ui.model.json.JSONModel(this.dependents), "dependents");
		...
		doModify:function(oRequest){
			...
			var oBatch8 = oComponent.getModel().createBatchOperation("PersonalTripSet(TrvReq='"+sRequest+"',EmpNo='"+sPernr+"',Version='"+sVersion+"')", "GET");//added batch8 uml6kor_15/9/2020_Personal trip during official travels_RBEIITAPPPIPELINE-176   
			
			oComponent.getModel().addBatchReadOperations([oBatch0,oBatch1,oBatch2,oBatch3,oBatch4,oBatch5,oBatch6,oBatch7,oBatch8]);//added batch8 uml6kor_15/9/2020_Personal trip during official travels_RBEIITAPPPIPELINE-176   
			oComponent.getModel().submitBatch(
					function(oResult){
						var oData = $.extend({}, oResult.__batchResponses[0].data, oResult.__batchResponses[1].data, 
								oResult.__batchResponses[2].data,oResult.__batchResponses[3].data,oResult.__batchResponses[6].data,oResult.__batchResponses[8].data);//added batch8 uml6kor_15/9/2020_Personal trip during official travels_RBEIITAPPPIPELINE-176
...
//						oData.ZE2E_INS_HDR.ZZ_RECOVERY_AMT = oResult.__batchResponses[7].data.InsAmount.ZZ_RECOVERY_AMT;
						...
						sap.ui.getCore().getModel("global").getData().ZE2E_REQ_STATUSSet = oData.ZE2E_INS_HDR.ZE2E_REQ_STATUS;
						...
						//start uml6kor_15/9/2020_Personal trip during official travels_RBEIITAPPPIPELINE-176
						oData.PersTripFlag = false;
						 if(oResult.__batchResponses[8].data.PersFlag == "X"){
				            		oData.PersTripFlag = true;
								}else{
									oData.PersTripFlag = false;
								}
						...
						//############## 17 March 2020 UCD1KOR ################/////// Start
						setTimeout(function(){ 
							oThis.MaxItem = Math.max.apply(Math, oData.ZE2E_INS_HDR.ZE2E_INS_DETAILSet.map(function(o) { return o.ZZ_ITEM_NO; }));
							//oThis.minItem = Math.min.apply(Math, oData.ZE2E_INS_HDR.ZE2E_INS_DETAILSet.map(function(o) { return o.ZZ_ITEM_NO; }));
							for(var i =0;i<oData.ZE2E_INS_HDR.ZE2E_INS_DETAILSet.length;i++){
                                if(parseInt(oData.ZE2E_INS_HDR.ZE2E_INS_DETAILSet[i].ZZ_ITEM_NO) == oThis.MaxItem){
                                	if(oData.ZE2E_INS_HDR.ZE2E_INS_DETAILSet[i].ZZ_DEPNDT_TYP.trim() == "00"){
                                		oThis.traveller.push(oData.ZE2E_INS_HDR.ZE2E_INS_DETAILSet[i]);
                                	}
                                	else{
                               ...
                               /* else{
                                	oData.ZE2E_INS_HDR.ZE2E_INS_DETAILSet[i].removeQuesnire = true;
                                }*/
							}
							traveller.setData(oThis.traveller);
							dependents.setData(oThis.dependents);
							
							////***** Getting Previous version date **************///
							if(oThis.MaxItem > 1){
								oThis.onLoadCauroselForPreviousRecords();
							}
							else{
								oThis.oldTraveller.setData([]);
								oThis.getView().setModel(oThis.oldTraveller,"oldTraveller");
						...
						////***** Getting Previous version date end**************///
							oModel.setData(oData);
							oThis.getView().setModel(oModel, "new");
						}, 500);
						oModel.setData(oData);
						...
						if( oData.ZE2E_REQ_STATUS.ZZ_REASON.trim() != ''  ){	//Compare version
							//oThis.getPreviousVersion(oThis); UCD1KOR Dec 15 commented this line
						}
	...
	//					Invisible button SendBack and questionare for VISA travel
		...
		//########### 18 March 2020 UCD1KOR ###########/// start
		validateRelation:function(){
			var relation = this.getView().getModel("new").getData().ZE2E_INS_HDR.ZE2E_INS_DETAILSet;
			for (var i=0;i<relation.length;i++){
				if(relation[i].ZZ_DEPNDT_TYP == "Select" || relation[i].ZZ_DEPNDT_TYP == ""){
					return "Select relation field";
				}
		...
		///////################## Nov 17 2020 Validation for Insurance start date and end Date ##########////////
			var tsdate = this.getView().getModel("traveller").getData()[0].ZZ_BEGDA;
			var tedate = this.getView().getModel("traveller").getData()[0].ZZ_ENDDA;
			var tsDateObj = new Date(tsdate.substr(0,4),tsdate.substr(4,2)-1,tsdate.substr(6,2));
			var teDateObj = new Date(tedate.substr(0,4),tedate.substr(4,2)-1,tedate.substr(6,2));
			var Dep = this.getView().getModel("dependents").getData();
			for(var i=0;i<Dep.length;i++){
				var depsDate = Dep[i].ZZ_BEGDA;
				var depsDateObj = new Date(depsDate.substr(0,4),depsDate.substr(4,2)-1,depsDate.substr(6,2));
				var depeDate = Dep[i].ZZ_ENDDA;
				var depeDateObj = new Date(depeDate.substr(0,4),depeDate.substr(4,2)-1,depeDate.substr(6,2));
				///////////############# conditions need to check ###############////////////////////
				if(depsDateObj >=tsDateObj && depsDateObj <=teDateObj && depeDateObj>=tsDateObj && depeDateObj<=teDateObj){
					console.log("condition true");
				}
				else{
					sError = "Kindly make sure that dependent dates are within employee start date and end date"
					sap.m.MessageToast.show(sError);
					return sError;
				}
			}
			...
			//######## 18 March 2020 UCD1KOR #####///
			sError = this.validateRelation();
			if(sError == undefined){
				sError ="";
			}
			if(sError != ""){
...
//		There are two lines in travel details so keep one line is enough
	...
	//		}
		...
		displaySubmit:function(){
			if( sap.ui.getCore().getModel("global").getData().action == "02" ){
			////################## UCD1KOR Start of changes Nov 25 , 2020 ####################///////
				///For open insurance request also user can submit insurannce request many times for DEPU
				...
				var modelData = sap.ui.getCore().getModel("global").getData().ZE2E_REQ_STATUSSet;
				if(modelData && modelData.ZZ_ACTION == "01" && modelData.ZZ_NROLE == "01"){
					var cond = false;
				}
				else{
					var cond = true;
				}
				if(sap.ui.getCore().getModel("global").getData().ZZ_REQ_TYP == "DEPU" && cond){
					 this.getView().byId("btnNext").setVisible(false);
		...
		displayNext:function(){
			if( sap.ui.getCore().getModel("global").getData().action == "02" ){	//Open
				...
				this.getView().byId("btnSubmit").setVisible(false);
				 /// For open insurance request also user can submit insurannce request many times
				 //#########################################################################//
				var modelData = sap.ui.getCore().getModel("global").getData().ZE2E_REQ_STATUSSet;
				/*Role - 01 and action - 00 -Saved
				Role - 01 and Actin is - 01 - Submitee
				Role - 03 and action is  - 02 - Sent back
				Role -03 and Action  -01 - Closed
				01 01 */
		...
		getListSearch: function(sRole,sAction,sOwner,sReason){
			...
			this.getView().byId("idList").bindItems({
				...
				template: new sap.m.StandardListItem({
					...
					info: "Date: {path : 'ZZ_RTIMESTAMP', formatter: 'sap.ui.project.e2etm.util.Formatter.sapDate'}",
					infoState: "Success",
...
//               }                    
	...
	//	},
		...
		onLoadCauroselForPreviousRecords:function(){
			var that =this;
			var oData = this.getView().getModel("new").getData();
			var sINS_HDR = "ZE2E_INS_DETAILSet?$filter=ZZ_TRV_REQ eq '{0}' and ZZ_OWNER eq '{1}' and ZZ_TRV_KEY eq '{2}' and ZZ_VERSION eq '{3}' ";
			var key = oData.ZZ_TRV_KEY +"-"+(this.MaxItem -1);
			sINS_HDR = sINS_HDR.replace("{0}", oData.ZZ_TRV_REQ);
			sINS_HDR = sINS_HDR.replace("{1}", oData.ZZ_DEP_PERNR);
			sINS_HDR = sINS_HDR.replace("{2}", key);
			sINS_HDR = sINS_HDR.replace("{3}", oData.ZZ_VERSION.trim());
			
	
			oDataModel.read(sINS_HDR,null,null,true,
					function (data, oRequest) {
				oData = data.results;
				that.oldTraveller.setData(oData);
				that.getView().setModel(that.oldTraveller,"oldTraveller");
			},
			function(error){
				sap.m.MessageToast.show("Cannot get previous version");
			});
		...
		sendPostRequest:function(sAction){
			...
			///############ UCD1KOR Added below For loop #############//////////
			/*for(var i=0;i<oData.ZE2E_INS_DETAILSet.length;i++){
				if(oData.ZE2E_INS_DETAILSet[i].removeQuesnire = true){
					delete oData.ZE2E_INS_DETAILSet[i].removeQuesnire;
				}
			}*/
			var oThis = this;
			//start of change_dye5kor
