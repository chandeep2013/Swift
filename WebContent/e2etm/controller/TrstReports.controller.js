...
sap.ui.controller("sap.ui.project.e2etm.controller.TrstReports", {
	...
	onItemPress : function(evt) {
		...
		switch (id) {
		case "setlUsetlRep":
			// this.getView().byId("pgTitle").setText("Settled/Unsettled
			// Reports");
			page = spApp.getPage("setlUsetlView", false);
			if (!(page)) {
				page = sap.ui.view({
		...
		case "secoAdvRep":
			// this.getView().byId("pgTitle").setText("Secondary Advance");
			sap.ui.core.routing.Router.getRouter("MyRouter").navTo(
					"setlSecoAdvRep");
			// page = spApp.getPage("secoAdvView", false);
			// if (!(page)) {
			// page = sap.ui.view({
		...
		case "itmClDownload":
			...
			break;
		/*
		 * ################ UCD1KOR 10 Jan 2020 added below section
		 * ########################
		 */
		case "itmSettleUnSettledDownload":
			sap.ui.core.routing.Router.getRouter("MyRouter").navTo(
					"setlusetlSplitreport", {
						"role" : Base64.encode(this.LoggedInRole)
					});
			break;
		/*
		 * ################ UCD1KOR 18 May 2020 added below section
		 * ########################
		 */
		case "itmInlandTravel_DigitaldDownload":
			sap.ui.core.routing.Router.getRouter("MyRouter").navTo(
					"InlandTravel_Digital");
			break;
		case "itmPersonalTripReport": // uea6kor-uml6kor_24.9.2020 personal
										// trip report
			sap.ui.core.routing.Router.getRouter("MyRouter").navTo(
					"personaltripreport", {
						"role" : Base64.encode(this.LoggedInRole)
			...
			break;
		case "itmMISInsuranceReport":
			sap.ui.core.routing.Router.getRouter("MyRouter").navTo(
					"MISInsuranceReportTR");
			break;	
		}
		// if (!page) {
		// page = spApp.getPage("defaultPage", false);
