...
sap.ui.define([
	...
	return sap.ui.controller("sap.ui.project.e2etm.controller.travelrequest.TVRDetail", {
		...
		//Task 48: Linh add param sTvrType for this function
		copPrevPassData: function(oFRow, oRow, sTab, sTvrType) {
			...
			switch (sTab) {
				...
				//	var aPDetail = Array.from(Object.create(aFDetail));
					var aPDetail = aFDetail;
					var aTemps = [];
					var bFRow = sTvrType === "DOME"; // Task 48 Add this line
					for (var i = 0; i < aPDetail.length; i++) {
						var oDRow = this.copPrevPassDtlRow(oRow.ZZ_ZSLFDPD, aFDetail[i], bFRow); // Task 48 Update this line
						aTemps.push(oDRow);
					}
				...
				case oTrip.oneway:
					...
					oRow.ZZ_TOTIME = oFRow.ZZ_TOTIME;
					oRow.ZZ_ZMODE = oFRow.ZZ_ZMODE;
					oRow.ZZ_F_ROW = sTvrType === "DOME"; // Task 48 Update this line
				...
				//	aPDetail = Array.from(Object.create(aFDetail));
					aPDetail = aFDetail;
					aTemps = [];
					bFRow = sTvrType === "DOME"; //Task 48 Add this line
					for (var r = 0; r < aPDetail.length; r++) {
						var oRDRow = this.copPrevPassDtlOneRoundRow(oRow.ZZ_ZSLFDPD, aFDetail[r], bFRow); // Task 48 Update this line
						aTemps.push(oRDRow);
					}
		...
		onAddNewPassenger: function() {
			...
			if (oTravelData !== undefined) {
				...
				if (oData.ZZ_TRIP_TAB === oTrip.multicity) {
					var nPassDtLen = oTravelData.aPassenger[0].aPassengerDtl.length;
				...
				if (bValid) {
					if (nPass <= 5) {
						...
						if (oData.TRV_REQ_TYPE === "DOME") {
							oFirstRow.ZZ_VISIBLE = true; //task 49
							oFirstRow.ZZ_VISIBLE_REMOVE_BTN = false; //task 52
						}
						var nPsg = nPass + 1;
						...
						oRow.ZZ_VISIBLE = false; //visible field for intl or dom //task 49
						oRow.ZZ_ENABLED = true; //enable field for intl or dom
						oRow.ZZ_EXPANDED = false;
						// oData.ZZ_REMOVE_PSG = true; //task 52
						oRow.ZZ_VISIBLE_REMOVE_BTN = true; //task 2
						//Task 48 add param oData.TRV_REQ_TYPE for function copPrevPassData
						var oFnRow = this.copPrevPassData(oFirstRow, oRow, oData.ZZ_TRIP_TAB, oData.TRV_REQ_TYPE); //copy data first row

						oTravelData.aPassenger.push(oFnRow);
					} else {
						sMsgCode = "E01";
						TCommonSrv._showErrMsgByCode(sMsgCode, "E");
					}
				} else {
			...
			oData.travelingDtl = oTravelData;
			oData.typeOfTravelVisible = false;
			oVModel.setData(oData);
		},
		onRemovePassenger: function(oEvent) {
			...
			oData.ZZ_REMOVE_PSG = oTravelData.aPassenger.length !== 1;
			oData.travelingDtl = oTravelData;
			oVModel.setData(oData);
		}
		...
		_setHeaderTitle: function(sType, oFirstRow, oLastRow, nLastRowIdx) {
			...
			//fix bug V8-01 - get min and max date in list items
			if (oData.ZZ_TRIP_TAB === oTrip.multicity) {
				var aPsgDtl = oTravelingData.aPassenger[0].aPassengerDtl;
				var aTravelDate = [];
				aPsgDtl.forEach(function(oPassDtl) {
					if (oPassDtl.ZZ_DATE_VALUE !== "" && oPassDtl.ZZ_DATE_VALUE) {
						var oCurrDate = TCommonSrv.convertStringToDate(Formatter.keepDateAlways(oPassDtl.ZZ_DATE_VALUE));
						aTravelDate.push(oCurrDate);
					}

				});
				var aTravelDateSorted = TCommonSrv.sortDateArr(aTravelDate);
				var maxDate = aTravelDateSorted[aTravelDateSorted.length - 1];
				if (maxDate) {
					var sLastDate = TCommonSrv.convertDateToString(maxDate, "d/m/y");
					oLastRowInfo.toDate = sLastDate;
				}
			...
			//start of code changes (if condition) by uml6kor 4/9/2019
				if(oLastRowInfo.fromKey != "" && oLastRowInfo.fromKey != undefined)
				{
					oPassRow.ZZ_FMCNTRY = oLastRowInfo.fromKey.split("-")[1].trim();
				}
				if(oLastRowInfo.toKey != "" && oLastRowInfo.toKey != undefined)
				{
					oPassRow.ZZ_TOCNTRY = oLastRowInfo.toKey.split("-")[1].trim();
				}				
				if(oLastRowInfo.fromText != "" && oLastRowInfo.fromText != undefined)
				{
					oPassRow.ZZ_FMCITY = oLastRowInfo.fromText.split("-")[0].trim();
				}
				if(oLastRowInfo.toText != "" && oLastRowInfo.toText != undefined)
				{
					oPassRow.ZZ_TOCITY = oLastRowInfo.toText.split("-")[0].trim();
				}
				if(oLastRowInfo.fromDate != "" && oLastRowInfo.fromDate != undefined)
				{
					oPassRow.ZZ_FMDATE = oLastRowInfo.fromDate;
				}
		   //end of code changes (if condition) by uml6kor 4/9/2019
					//fix bug V8 - 01
				if (oLastRowInfo.toDate.length > 8) {
					oPassRow.ZZ_TODATE = TCommonSrv.convertDateToString(oLastRowInfo.toDate, "ymd");
				} else {
				...
				//fix bug V8 - 01
				var sToDate = oLastRow.ZZ_DATE_VALUE;
				if (oData.ZZ_TRIP_TAB === oTrip.multicity) {
					sToDate = oLastRowInfo.toDate;
				}

				oEmpData.travelDateTo = TCommonSrv.buildDateHeader(TCommonSrv.convertStringToDate(Formatter.keepDateAlways(sToDate)));
				...
				//handle for multi traveller
				var oTravelData = oData.travelingDtl;
				//Task 48 Remove this code
				// if (oData.TRV_REQ_TYPE === "DOME" && oTravelData.aPassenger.length > 1) {
				// 	oTravelData = this.updateSubPassengerDtl(oTravelData);

				// } else
				oTravelData.aPassenger[0].ZZ_VISIBLE = true; //task 49 BUSR V5
				if (oData.TRV_REQ_TYPE === "DOME" && oTravelData.aPassenger.length === 0) {
					...
					oTravelData.aPassenger[0].ZZ_VISIBLE_ADD_BTN = oData.TRV_REQ_TYPE === "DOME"; //task 52
				}
				oVModel.setData(oData);
			}
		},
		...
		setBaseData: function(sBType) {
			...
			if (sBType === "INTL") {
				...
				oData.ZZ_REMOVE_PSG = false; // invisible add new psg task 52
				oData.TRV_REQ_TYPE = "INTL";
				//oData.ZZ_SIM_REQ_KEY = "Y"; //commented by uml6kor 4/9/2019
				...
				oData.ZZ_REMOVE_PSG = oData.travelingDtl.aPassenger.length !== 1; //visible add new psg task 52
		...
		_prepareDataForSubScr: function() {
			...
			//dye5kor_to_to considet the INTL/DOME
			if(oData.ZZ_TRIP_TAB==2){
				var allTravelData = oData.travelingDtl.aPassenger[0].aPassengerDtl;
				for(i=0;i<allTravelData.length;i++){
					if(allTravelData[i].ZZ_TRV_TO_LOC.split("-")[1] != 'IN'){
						oData.TRV_REQ_TYPE = 'INTL';
					}
				}
			}
			...
			$.when(oFundDeferred).done(
				function() {
					...
					//if (oData.ZZ_SIM_REQ_KEY === "Y" && sMode === "003") { //fix bug V8 - 07
		...
		_getFinalRowRnd: function(oPass) {
			...
			oPassRow.ZZ_TOCNTRY = oPst.ZZ_TOCNTRY; //Task 38
			oPassRow.ZZ_FMCITY = oPst.ZZ_FMCITY;
			oPassRow.ZZ_TOCITY = oPst.ZZ_TOCITY;
		...
		_updateTravelMode: function(oData, sTripTab, sMode,sIndex) {
			switch (sTripTab) {
				case oTrip.oneway:
					oData.travelingDtl.aPassenger[0].ZZ_ZMODE = sMode;
					break;
				case oTrip.roundtrip:
				//dye5kor	//oData.travelingDtl.aPassenger[0].aPassengerDtl[0].ZZ_ZMODE = sMode;
				//dye5kor	//oData.travelingDtl.aPassenger[0].aPassengerDtl[1].ZZ_ZMODE = sMode;	
					if(sIndex==0){
					...
					//oData.travelingDtl.aPassenger[0].aPassengerDtl[sIndex].ZZ_ZMODE = sMode;
					break;
				case oTrip.multicity:
		...
		handleOneRoundTrip: function(oEvent, sField, sValue) {
				...
				switch (sField) {
					...
					case "fromtime":
						...
						// if (oData.ZZ_ROUND_TRIP === false) { temp disable task 38
					...
					case "header":
						//get general information
						// oPassRow.ZZ_ZSLFDPD = oPassFrow.ZZ_ZSLFDPD; //Task 49 BUSR V5
						oPassRow.ZZ_ZSEAT = oPassFrow.ZZ_ZSEAT;
						oPassRow.ZZ_ZMEAL = oPassFrow.ZZ_ZMEAL;
						//oPassRow.ZZ_MOBILE = oPassFrow.ZZ_MOBILE; //Task 49 BUSR V5
						break;
					case "trvmode":
						bCallBack = true;
				...
				if (oEmpData.travelFrom !== undefined && oEmpData.travelTo !== undefined) {
					...
					if (oPassFrow.ZZ_FMCITY && oPassFrow.ZZ_TOCITY) {
						oEmpData.travelLoc = oPassFrow.ZZ_FMCITY.replace(",", " -") + " to " + oPassFrow.ZZ_TOCITY.replace(",", " -");
					}
				...
				if (oEmpData.travelDateFrom !== undefined && oEmpData.travelDateTo !== undefined) {
					...
					if (oPassRow.ZZ_FMDATE_VALUE && oPassRow.ZZ_TODATE_VALUE) {
						var sFrom = TCommonSrv.convertStringToDate(Formatter.keepDateAlways(oPassRow.ZZ_FMDATE_VALUE));
						var sTo = TCommonSrv.convertStringToDate(Formatter.keepDateAlways(oPassRow.ZZ_TODATE_VALUE));
						oEmpData.travelDate = TCommonSrv.buildDateHeader(sFrom) + " to " + TCommonSrv.buildDateHeader(sTo);
					}
				...
				oData.empDetail = oEmpData;
				oVModel.setData(oData);
				if (oPassFrow.ZZ_FMCNTRY === undefined) {
					oPassFrow.ZZ_FMCNTRY = "IN";
				}
				if(oPassFrow.ZZ_TOCNTRY === undefined){
					oPassFrow.ZZ_TOCNTRY = oFRow.ZZ_TOCNTRY;
				}
				...
				//dye5kor	this._updateTravelMode(oData, oData.ZZ_TRIP_TAB, "A");
					var selectedModeIndex=0;
					if(oEvent.getParameter("id").indexOf("0-0")== -1){
						selectedModeIndex = 1;
					}
					this._updateTravelMode(oData, oData.ZZ_TRIP_TAB, sValue,selectedModeIndex);
				}

				//update data
				//Task 48 Remove block code below
				...
				// } else 
				oData.travelingDtl.aPassenger[0].ZZ_VISIBLE = true; //Task 49 BUSR V5
				if (oData.TRV_REQ_TYPE === "DOME" && oData.travelingDtl.aPassenger.length === 1) {
					// oData.travelingDtl.aPassenger[0].ZZ_VISIBLE = false; //Task 49 BUSR V5
					oData.travelingDtl.aPassenger[0].ZZ_ENABLED = true;
					oData.travelingDtl.aPassenger[0].ZZ_VISIBLE_ADD_BTN = true;
				} else {
					// oData.travelingDtl.aPassenger[0].ZZ_VISIBLE = true; //Task 49 BUSR V5
					oData.travelingDtl.aPassenger[0].ZZ_ENABLED = false;
					//Task 48 Update below code
					if (oData.TRV_REQ_TYPE !== "DOME") {
						oData.travelingDtl.aPassenger[0].ZZ_VISIBLE_ADD_BTN = false;
					}
				}
		...
		onFromCountryChange: function(oEvent) {
			...
			if (oData.ZZ_TRIP_TAB === oTrip.multicity) {
				...
				if (oData.ZZ_TRIP_TAB === oTrip.roundtrip) {
					var nIdx = parseInt(TCommonSrv.getTravelPsgIdx(oEvent, this)); //Task 48 Add this line
					var oPass = oData.travelingDtl.aPassenger[nIdx]; //Task 48 Update this line
					if (oPass.aPassengerDtl[0].ZZ_LOC_FROM_KEY !== oPass.aPassengerDtl[0].ZZ_LOC_TO_KEY) {
						...
						//this.updateSubTravellerRnd(oData.travelingDtl, oData.ZZ_TRIP_TAB); // Task 48 Remove this
					} else {
			...
			if (oData.ZZ_TRIP_TAB === oTrip.multicity) {
				...
				if (oData.ZZ_TRIP_TAB === oTrip.roundtrip) {
					...
					if (oPass.aPassengerDtl[0].ZZ_LOC_FROM_KEY !== oPass.aPassengerDtl[0].ZZ_LOC_TO_KEY) {
						...
						//this.updateSubTravellerRnd(oData.travelingDtl, oData.ZZ_TRIP_TAB); // Task 48 Remove this line
					} else {
						// TCommonSrv._showErrMsgByCode("E03", "V");
		...
		onChangeTravelDateFrom: function(oEvent) {
			...
			if (oData.ZZ_TRIP_TAB === oTrip.roundtrip) {
				if (sIdx === 1) {
					...
					oData.travelingDtl.aPassenger[0].aPassengerDtl[sIdx].ZZ_TODATE = Formatter.dateToYM(oEvent.getSource().getValue()); //task 38
					//this.updateSubTravellerRnd(oData.travelingDtl, oData.ZZ_TRIP_TAB); //Task 48 remove this line
				} else {
		...
		onDependentChange: function(oEvent) {
			...
			if (oExistDep === undefined) {
				...
				sDep = sNewDep;
				// TCommonSrv._showErrMsgByCode("E15", "E"); //tempdisable for investigate

			}
			//update dep
			if (oData.ZZ_TRIP_TAB === oTrip.roundtrip || oData.ZZ_TRIP_TAB === oTrip.multicity) {
				//round trp
				this._updateDependentDetailRndMul(sDep, aPass.aPassengerDtl);
			} else {
				//one way
				oTravelData.aPassenger[sIdx].ZZ_ZSLFDPD = sDep;
			}
		...
		onHandleTaxiMode: function(sTrvModeKey) {
			//dye5kor_enable link for taxi 
			var TaxiModeData = oVModel.getData().travelingDtl.aPassenger[0].aPassengerDtl;
			var oData = oVModel.getData();
			oData.ZZ_TAXI_LINK_ENABLED = false;
			if(TaxiModeData==0){
				TaxiModeData = oVModel.getData().travelingDtl.aPassenger[0];
				if(TaxiModeData.ZZ_ZMODE=='X'){
					oData.ZZ_TAXI_LINK_ENABLED = true;		
				}
			}else{
			
			/*if(oData.ZZ_TRIP_TAB !== oTrip.oneway){*/
			for(var i=0;i<TaxiModeData.length;i++){
			if(TaxiModeData[i].ZZ_ZMODE=='X'){
				oData.ZZ_TAXI_LINK_ENABLED = true;			
				}	
