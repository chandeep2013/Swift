...
sap.ui.define([
	...
	return Controller.extend("sap.ui.project.e2etm.controller.travelrequest.PVDetail", {
		...
		getExistVisaDepen: function(sVisaPlan) {
			...
			oDataModel.read(sPath, {
				success: function(result) {
					...
					oVD.selfVisa.ZZ_VISA_VALID = true;
		...
		onFileChange: function(evt) {
			var oThis = TravelRequestThis; //eslint-disable-line
			//dye5kor_busyindicator for visa upload
			var oControl = sap.ui.getCore().byId("dialogUploadVisaCopy");
			sap.ui.project.e2etm.util.StaticUtility.setBusy(true,oControl);
			//dye5kor_busyindicator for visa upload
			var aData = sap.ui.getCore().byId("dialogUploadVisaCopy").getModel().getData();
			// console.log(aData);
		...
		//	sap.ui.project.e2etm.util.StaticUtility.setBusy(false,oControl);
			var sFileName;
			try {
			...
			get.done(function(result, response, header) {
				try {
					...
					post.success(function(data) {
						...
						// Close the dialog
						//sap.ui.project.e2etm.util.StaticUtility.setBusy(false,sap.ui.getCore().byId("dialogUploadVisaCopy"));
						sap.ui.project.e2etm.util.StaticUtility.setBusy(false,oControl);
						sap.m.MessageToast.show("Upload Succesfully");
		...
		saveVisaPlan: function(aData, iSelectedVisaIndex, sStatus, sMessage, sRequestType) {
			...
			//TinhTD Temp
			aData.selfVisa.ZZ_MULT_ENTRY = aData.visaExistingDependent[iSelectedVisaIndex].ZZ_MULT_ENTRY;
			aData.selfVisa.ZZ_VISA_SDATE = Formatter.dateToYM(aData.visaExistingDependent[iSelectedVisaIndex].ZZ_VISA_SDATE);
			aData.selfVisa.ZZ_VISA_EDATE = Formatter.dateToYM(aData.visaExistingDependent[iSelectedVisaIndex].ZZ_VISA_EDATE);
			aData.selfVisa.ZZ_VISA_NO = aData.visaExistingDependent[iSelectedVisaIndex].ZZ_VISA_NO;
			//End TinhTD
			// Call service to create visa plan
			var token = "";
			var get = $.ajax({
		...
		loadExistVisa: function() {
			var globalData = sap.ui.getCore().getModel("global").getData(); //added by uml6kor 10/9/2019
			var oPModel = PassportVisaThis.getView().getModel();
			var oVPD = oPModel.getData();
			...
			var sToCountry = "";
			var empDetail = sap.ui.getCore().getModel("profile").getData().employeeDetail;
			// console.log("ZZ_DEP_REQ",empDetail);
			...
			oDataModel.callFunction(
				"/GetVisaDetails", {
					...
					urlParameters: {
						ZZ_CURR_VISA_TYP: "BUSR",
						ZZ_DEP_PERNR:  globalData.ZZ_DEP_PERNR//empDetail.ZZ_DEP_PERNR //added by uml6kor 10/9/2019
					},
					success: function(oCData) {
						var aData = oCData.results;
