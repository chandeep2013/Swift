				...
				oDataModel.read(sPath, {
					...
					success: function(oRData) {
						if (sType === "reload") {
							...
							TravelRequestThis.advanceDialog = sap.ui.xmlfragment("sap.ui.project.e2etm.fragment.travel.AdvanceDetails",
								TravelRequestThis); //eslint-disable-line
		...
		onAdvanceMoneySwitch: function(oEvent) {
			...
			if (bValid) {
				...
				if (bState === true) {
					...
					oData.aAdvMoney = [];
					var oAdvTemp = TCommonSrv.buildAdvMoneyEmpty("EUR");
					oData.aAdvMoney.push(oAdvTemp);
					oData.aAdvMoneyLenght = 1;
					TCommonSrv._handleShowHideObj(aAdvComps, bState, this, TravelRequestThis); //eslint-disable-line
				}
				oVModel.setData(oData);
