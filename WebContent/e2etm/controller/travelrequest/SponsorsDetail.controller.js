...
sap.ui.define([
	...
	return sap.ui.controller("sap.ui.project.e2etm.controller.travelrequest.SponsorsDetail", {
		...
		/*	var mcrModel = new sap.ui.model.json.JSONModel({"mcrVisible":false,"mcrNonVisible":false,"mcrFlag":""});
			this.getView().setModel(mcrModel,"mcrModel")
*/			
			//dye5kor
		...
		getCostAssignment: function(oData, oVData, oF4Source) {
			...
			if (oData.wbsElement === undefined) {
				...
				if (oTravelHeader) {
					...
					// "GET");
					var batchOperation1 = oDataModel.createBatchOperation("CostCenterF4Help?" + "ZZ_REINR='" + sRequest + "'&ZZ_TTYPE='" +
						oTravelHeader
						.ZZ_TRV_TYP +
				...
				//dye5kor_mcr and non mcr changes
					var batchOperation6 = oDataModel.createBatchOperation("MCRData?&$format=json", "GET");
				//dye5kor_mcr and non mcr changes
					oDataModel.addBatchReadOperations([batchOperation0, batchOperation1, batchOperation2, batchOperation3, batchOperation4,
						batchOperation5,batchOperation6
					]);
					oDataModel.submitBatch(function(oResult) {
						...
						oData.mcr = oResult.__batchResponses[6].data.results;
				...
				$.when(oCostDeferred).done(
					function() {
						...
						// fix bug V8 - 14
						if (oVData.ZZ_STATUS === undefined) {
							oThis._updateInitTextField();
						} else {
							oThis._updateSponsorTextField(oVData);
						}
		...
		_setFieldText: function(nIdx, sFieldId, sText) {
			...
			switch (sFieldId) {
				case "sponsoredId":
					oData.aSponsorshipDtl[nIdx].ZZ_FISTL_TEXT = sText.toLowerCase(); // Budget Center
					break;
				case "costCenterId":
					oData.aSponsorshipDtl[nIdx].ZZ_KOSTL_TEXT = sText.toLowerCase(); //cost center
					break;
				case "projectId":
					oData.aSponsorshipDtl[nIdx].ZZ_FIPOS_TEXT = sText.toLowerCase(); //wbs element
					break;
				case "expenseId":
					oData.aSponsorshipDtl[nIdx].ZZ_FIPEX_TEXT = sText.toLowerCase(); //budget code
					break;
				case "taskId":
					oData.aSponsorshipDtl[nIdx].ZZ_TASKDESC = sText.toLowerCase(); 		//TaskID
					break;
				case "resourceGrp":
					oData.aSponsorshipDtl[nIdx].ZZ_RESOUCESDESC = sText.toLowerCase();       //ResourceGroup
			}
		...
		handleValueHelpData: function(oControl) {
			...
			var oValueHelpDialog = new sap.ui.comp.valuehelpdialog.ValueHelpDialog({
				...
				ok: function(oControlEvent) {
					...
					//dye5kor_setting resource type by default
					if(oControl.getId().indexOf("resourceGrp") != -1){
						var mcrF4Data = this.oRows.getModel().getData();
						var resourcegrpId = oControl.getId();
						var resourceTypId = resourcegrpId.replace("resourceGrp","resourceType")
						if(mcrF4Data.length >= 0){
							for(var i=0;i<mcrF4Data.length;i++){
							if(mcrF4Data[i].ResourceId == sKey ){
								oThis.getView().byId(resourceTypId).setValue(mcrF4Data[i].ResourceTyp);
								oThis.getView().byId(resourceTypId).setEditable(true);
								}
							}
						}
		...
		setColumnF4Table: function(sControlId, oRowsModel, oValueHelpDialog, oControl) {
			...
			switch (sControlId) {
				...
				case 'col5': // Budget cost
					...
					break;
			//Mcr and non mcr changes
				case 'col6' || 'col7': // Task ID
					
				case 'col7': // Resource Group
		...
		_handleBeforeOpenDlg: function(oValueHelpDialog) {
			...
			$("<span class='sp_hint_fade'>Please click on the column name to filter the value</span>").insertBefore("#" + sTableId);
			//add search text beside icon
			oValueHelpDialog.addStyleClass("customDialog");
		},
		_GetControlIdNew: function(sId) {
			...
			switch (sId) {
				...
				case "taskId":		//Task ID
					sCid = "col6";
				case "resourceGrp": //Resource Group
					sCid = "col7";
			}
			return sCid;
		},
		_updateSummary: function(oData) {
			...
			oComponent.aFunTotal = nTotal; //eslint-disable-line
		...
		onChangeFundCenter: function(oEvent) {
			...
			//dye5kor_For Mcr  non Mcr Changes
			//var mcrFundData = this.getView().getModel("mcrModel").getData();
			var mcrFundData = TravelRequestThis.getView().getModel("mcrModel").getData();
			if(oEvent.getSource().getSelectedKey() != 'F03'){
				mcrFundData.mcrVisible = false;
				mcrFundData.mcrNonVisible = false;
			}else{
				if(TravelRequestThis.getView().getModel("mcrModel").getData().mcrFlag=='X'){
					mcrFundData.mcrVisible = true;
					mcrFundData.mcrNonVisible = false;	
				}else{
					mcrFundData.mcrVisible = false;
					mcrFundData.mcrNonVisible = true;
				}
			}
			//this.getView().getModel("mcrModel").setData(mcrFundData);
			//dye5kor_For Mcr  non Mcr Changes
			if (oData.ZZ_SP_VALID === true) {
				//dye5kor do not validate this if request has been created
				
				if (nTotal === 100 && (oData.aFundSelect.indexOf(oFund.getSelectedKey()) === -1)) {
					if(oData.ZZ_TRV_REQ_NUM == "0000000000"){
					...
					if (nTotal > 100) {
						...
						oVModel.setData(oData);
						TravelRequestThis.getView().getModel("mcrModel").setData(mcrFundData);
					}
				}
			} else {
		...
		addSponsorshipDtl: function() {
			...
			for (var i = 0; i < oData.aSponsorshipDtl.length; i++) {
				...
				oData.aSponsorshipDtl[i].ZZ_EXPANDED = true;
				//link task 19 V7
				oData.aSponsorshipDtl[i].ZZ_VISIBLE = true;
			}
			oData.aSponsorshipDtl.push(oSpRow);
			oData.aSponsorshipLen++;
			oVModel.setData(oData);
		},
		remSponsorshipDtl: function(oEvent) {
			var oData = oVModel.getData();
			var oRowObject = oEvent.getSource().getBindingContext("trvRequestDtl").getObject();
			if (oData.aSponsorshipDtl.length > 1) {
				var iIndex = oData.aSponsorshipDtl.indexOf(oRowObject);
				oData.aSponsorshipDtl.splice(iIndex, 1);
			} else {
				...
				switch (sCId) {
					...
					case "projectId":
						...
						if (oCurrEdit.ZZ_FIPOS.toString() !== "") {
							...
							//dye5kor_Mcr and non mcr changes
							//The selected PIF is MCR/Non Mcr
								if(oData.aSponsorshipDtl[nIdx].ZZ_GEBER=='F03'){
								oDataModel.read("MCRFlagSet(PIFID='" + oCurrEdit.ZZ_FIPOS +"')",null,null,true,
									jQuery.proxy(function(mcrData, response){ 
										//Read the main modle Data and set MCR Flag
										//var mcrModel = new sap.ui.model.json.JSONModel({"mcrVisible":false});
										//var mcrDataVisible = this.getView().getModel("mcrModel").getData();
										var mcrDataVisible = TravelRequestThis.getView().getModel("mcrModel").getData();
										if(mcrData.MCR_FLAG == 'X'){
											mcrDataVisible.mcrVisible = true;
											mcrDataVisible.mcrNonVisible = false;
											mcrDataVisible.mcrFlag='X';
										}else{
											mcrDataVisible.mcrVisible = false;
											mcrDataVisible.mcrNonVisible = true;
											mcrDataVisible.mcrFlag='';
										}
										//this.getView().getModel("mcrModel").setData(mcrDataVisible);
										TravelRequestThis.getView().getModel("mcrModel").setData(mcrDataVisible);
									},this),function(error){
							});
								}
							//dye5kor_Mcr and non mcr changes
		...
		//Upadte text for field fixbug V8 - 14
		_updateSponsorTextField: function(oData) {
			var oThis = this,
				oSVM = this.getView().getModel("sponsorDetail"),
				oSD = oSVM.getData(),
				aBudgets = oSD.F4Value.budgetCenter,
				aWbs = oSD.F4Value.wbsElement,
				aBudgetCost = oSD.F4Value.budgetCost,
				aSponsorDtl = oData.aSponsorshipDtl;
			aSponsorDtl.forEach(function(oSponsor, nIdx) {
				...
				var oExistFipos = TCommonSrv.checkExistingArray(aWbs, "ZZ_POSKI", oSponsor.ZZ_FIPOS.toString().toLocaleUpperCase());
				var oExistFipex = TCommonSrv.checkExistingArray(aBudgetCost, "FIPEX", oSponsor.ZZ_FIPEX.toString().toLocaleUpperCase());
				if (oExistFistl) {
					oThis._setFieldText(nIdx, "sponsoredId", oExistFistl.ZzFundDes);
				}
				if (oExistKostl) {
					oThis._setFieldText(nIdx, "costCenterId", oExistKostl.KTEXT);
				}
				if (oExistFipos) {
					oThis._setFieldText(nIdx, "projectId", oExistFipos.ZZ_POST1);
				}
				if (oExistFipex) {
					oThis._setFieldText(nIdx, "expenseId", oExistFipex.ZZ_BUD_DESC);
				}
			});
