...
sap.ui.controller("sap.ui.project.e2etm.controller.PaymentVoucher", {
	...
	fetchDetails : function() {
		...
		var rcvdata = oDataModel.createBatchOperation("RCVData/?ZZ_TRV_REQ='"+global.ZZ_TRV_REQ+",PYVR'&$format=json", "GET");//added by ucd1kor on 08/06/2021
		
		oDataModel.addBatchReadOperations([ getCurr, getWorklist, getDocs, getComments, getExpenses,rcvdata ]);
		oDataModel.submitBatch(function(oResult) {
			try {
				...
				//Added by ucd1kor on 08/06/2021
				var rcvModel = new sap.ui.model.json.JSONModel();
				rcvModel.setData({
					typeMode : oResult.__batchResponses[5].data.results
				});
				...
				oThis.setFooter();
				///##################### UCD1KOR 24/06/2021 added below logic for close and accounting #############//////////
				var data = 	oThis.getView().getModel("travelsettlement").getData();
				for(var i=0;i<data.payment.expenses.length;i++){ 
					if(data.payment.expenses[i].Frmpl ==""){
						oThis.hideAccounting = true;
						break;
					}
					else{
						oThis.hideAccounting = false;
					}
				}
				if(oThis.hideAccounting == true){
					oThis.getView().byId("idPYVRAccountingButton").setText("Close");
				}
				else{
					oThis.getView().byId("idPYVRAccountingButton").setText("Accounting");
				}
				/////////////################################### end changes ###################################////////////////
	...
	setTravelDetails : function(odata) {
		...
		oDataModel.read("TrvStGenDataSet(EmpNo='" + global.ZZ_DEP_PERNR + "',TravelPlan='" + global.ZZ_TRV_REQ + "',Version='" + version + "',TravelType='" + global.ZZ_REQ_TYP + "',LoginRole='" + currentRole + "',Module='PYVR',Item='" + odata.Item + "')", null, null, false, function(oData, response) {
			...
			///################### UCD1KOR 24/06/2021 accouting/closed button text change#################################////////
			/*var recdt = data.traveldetails.ReceivedDt;
			var autoPostDate = "20210629";
			if(recdt !="" && autoPostDate != ""){
				var creationDt = new Date(recdt.substr(0, 4), recdt.substr(4, 2) - 1, recdt.substr(6, 2)); 
				var autoPostDt = new Date(autoPostDate.substr(0, 4), autoPostDate.substr(4, 2) - 1, autoPostDate.substr(6, 2));
				if(creationDt >= autoPostDt){
					oThis.getView().byId("idPYVRAccountingButton").setText("Accounting");
				}
				else{
					oThis.getView().byId("idPYVRAccountingButton").setText("Close");
	...
	setFieldProperties : function(details) {
		...
		if (loginrole == "EMP") {
			...
			oThis.getView().byId("comments").setEditable(true);
			//############################# UCD1KOR 24/06/2021 changes ##################//
			if(loginrole == "TRST" && ((action =="03" && role == "12") || (action =="16" && role == "11"))){
				data.fieldproperties.caterogyEditable = true;
			}
			else if(data.fieldproperties.caterogyEditable == true){
				data.fieldproperties.caterogyEditable = true;
			}
			else{
				data.fieldproperties.caterogyEditable = false;
			}
		} 
	...
	onChangeExpType:function(evt){
		...
		switch (id[1]) {
		case "paymentexp":
			data.payment.expenses[index].Descr = evt.getSource().mProperties.value;
			break;
		}
		oThis.getView().getModel("travelsettlement").setData(data);
	...
	onClose : function(evt) {
		sap.ui.project.e2etm.util.StaticUtility.setBusy(true, oThis);
		if(evt.getSource().getText() == "Close"){ // for closed
			jQuery.sap.delayedCall(500, oThis, function() {
			...
			jQuery.sap.delayedCall(500, oThis, function() {
				//oThis.saveDetails("11", "14", "CLS");
				// UCD1KOR 08/06/2021 Action and role changes NXACT="00012399"
	...
	saveDetails : function(role, action, btype) {
		...
		if (!declaration && btype != "SAV" && loginrole == "EMP") {
			...
			//############# UCD1KOR 08/06/2021 Added below condition for accounting ##################//
			if(action =="28" && btype == "CLS"){
				traveldetails.NextAction = "00012399";
				//traveldetails.ModId = "PYVR";
			}
			...
			try {
				...
				/////################ UCD1KOR 24/06/2021 validation for category ##########//////////////
				
				if(btype =="REV" || btype =="APP"){
					for(var i=0;i<oData.TrvStDetailsSet.length;i++){
						if(oData.TrvStDetailsSet[i].Frmpl == "" || oData.TrvStDetailsSet[i].Frmpl == undefined){
							sap.ui.project.e2etm.util.StaticUtility.setBusy(false, oThis);
							sap.m.MessageBox.error("Please correct the Category Value..!!");
							return;
						}
					}
				}
				
				/////################## UCD1KOR 24/06/2021 changes end ########################////////
	...
	appendRows : function(exptype, traveldetails, trvdetset, etype, btype, prop) {
		var flag = false;
		var role = sap.ui.getCore().getModel("profile").getData().currentRole;
		//########## UCD1KOR added Frmpl field 14/06/2021 ############/////
		var recv = this.getView().getModel("rcvModel").getData().typeMode;
		for(var k=0;k<exptype.expenses.length;k++){
			for(var j=0;j<recv.length;j++){
				try{
					var val = recv[j].ZZ_EXP_TXT.split("-");
					if(val.length == 2){
						var splitvalue1 = val[0].trim();
						var splitvalue2 = val[1].trim();
						recv[j].ZZ_EXP_TXT = splitvalue1+"-"+splitvalue2;
					}
				}catch(err){}
				if(recv[j].ZZ_EXP_TXT.toUpperCase() == exptype.expenses[k].Descr.toUpperCase()){
					exptype.expenses[k].Frmpl = recv[j].ZZ_EXP_TYP;
				}
			}
		}
...
//				EType : etype,
				...
				Module : traveldetails.Module,
				Item : traveldetails.Item,
				Frmpl : exptype.expenses[i].Frmpl
