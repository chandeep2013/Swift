...
//		UploadCollection.setModel(uploadproperties,"uploadcollection");
	...
	onChangeTotalFare1:function(){
		if(oThis.Tfare1 == "" || oThis.Tfare1 == "0.00"){
			oThis.Tfare1 = this.getView().getModel("itenary1").getData().collection1.Tfare;
		}
		else{
			oThis.Tfare1 = this.getView().getModel("itenary1").getData().collection1.Tfare;
		}
	},
	onChangeTotalFare2:function(){
		if(oThis.Tfare2 == "" || oThis.Tfare2 == "0.00"){
			oThis.Tfare2 = this.getView().getModel("itenary2").getData().collection2.Tfare;
		}
		else{
			oThis.Tfare2 = this.getView().getModel("itenary2").getData().collection2.Tfare;
		}
	},
	onChangeTotalFare3:function(){
		if(oThis.Tfare3 == "" || oThis.Tfare3 == "0.00"){
			oThis.Tfare3 = this.getView().getModel("itenary3").getData().collection3.Tfare;
		}
		else{
			oThis.Tfare3 = this.getView().getModel("itenary3").getData().collection3.Tfare;
		}
	},
	onChangeTotalFare4:function(){
		if(oThis.Tfare4 == "" || oThis.Tfare4 == "0.00"){
			oThis.Tfare4 = this.getView().getModel("itenary4").getData().collection4.Tfare;
		}
		else{
			oThis.Tfare4 = this.getView().getModel("itenary4").getData().collection4.Tfare;
		}
	...
	onChangeCharge1:function(oEvent){
		var totalFare = parseFloat(oThis.Tfare1) - parseFloat(oThis.AChargeAmt1);
		var value = oEvent.getSource().getValue();
		var SelectedKey = this.getView().byId("ch1").getSelectedKey();
		if(SelectedKey != ""){
			if(value == "" || value == undefined){
				value = "0";
			}
			if(totalFare == "" || totalFare == "NaN"){
				totalFare = "0"
			}
			
			var add = parseFloat(totalFare) + parseFloat(value);
			this.getView().getModel("itenary1").getData().collection1.Tfare = add.toString();
			this.getView().getModel("itenary1").refresh();
		}
		else{
			
			this.getView().byId("ch1Value").setValue(oThis.AChargeAmt1);
			sap.m.MessageToast.show("Please select value from dropdown First");
		}

		
	},
onChangeCharge2:function(oEvent){
	var totalFare = parseFloat(oThis.Tfare2) - parseFloat(oThis.AChargeAmt2);
	var value = oEvent.getSource().getValue();
	var SelectedKey = this.getView().byId("ch2").getSelectedKey();
	if(SelectedKey != ""){
		var value = oEvent.getSource().getValue();
		if(value == "" || value == undefined){
			value = "0";
		...
		var add = parseFloat(totalFare) + parseFloat(value);
		this.getView().getModel("itenary2").getData().collection2.Tfare = add.toString();
		this.getView().getModel("itenary2").refresh();
	}
	else{
		this.getView().byId("ch2Value").setValue(oThis.AChargeAmt2);
		sap.m.MessageToast.show("Please select the type of charge from dropdown");
	}
		
		
	},
onChangeCharge3:function(oEvent){
	var totalFare = parseFloat(oThis.Tfare3) - parseFloat(oThis.AChargeAmt3);
	var value = oEvent.getSource().getValue();
	var SelectedKey = this.getView().byId("ch3").getSelectedKey();
	if(SelectedKey != ""){
		var value = oEvent.getSource().getValue();
		if(value == "" || value == undefined){
			value = "0";
		...
		var add = parseFloat(totalFare) + parseFloat(value);
		this.getView().getModel("itenary3").getData().collection3.Tfare = add.toString();
		this.getView().getModel("itenary3").refresh();
	}else{
		this.getView().byId("ch3Value").setValue(oThis.AChargeAmt3);
		sap.m.MessageToast.show("Please select the type of charge from dropdown");
	}
		
	},
onChangeCharge4:function(oEvent){
	var totalFare = parseFloat(oThis.Tfare4) - parseFloat(oThis.AChargeAmt4);
	var value = oEvent.getSource().getValue();
	var SelectedKey = this.getView().byId("ch4").getSelectedKey();
	if(SelectedKey != ""){
		var value = oEvent.getSource().getValue();
		if(value == "" || value == undefined){
			value = "0";
		...
		var add = parseFloat(totalFare) + parseFloat(value);
		this.getView().getModel("itenary4").getData().collection4.Tfare = add.toString();
		this.getView().getModel("itenary4").refresh();
	}else{
		this.getView().byId("ch4Value").setValue(oThis.AChargeAmt4);
		sap.m.MessageToast.show("Please select the type of charge from dropdown");
	}
...
//		Accept						
	...
	fetchGeneralData : function() {
		...
		// addeb by UIK1KOR on 16/09/2020 for Personal trip during official travels_RBEIITAPPPIPELINE-176    
		var oPsnlTripSet = oDataModel.createBatchOperation("PersonalTripSet(TrvReq='"+ticket.TravelPlan+"',EmpNo='"+ticket.EmpNo+"',Version='')","GET")
		// end of change on 16/09/2020
		//start of code uml6kor for assignment Model visible 29/8/2019
		if(ticket.TravelType == "DEPU")
		{
			oThis.getView().byId("asglabel").setVisible(true);
			oThis.getView().byId("mdtext").setVisible(true);
		}
		else
		{
			oThis.getView().byId("asglabel").setVisible(false);
			oThis.getView().byId("mdtext").setVisible(false);
		}
		//end of code uml6kor  29/8/2019
		...
		var batchOperation6 = oDataModel.createBatchOperation(
				...
				+ ticket.TravelType + "'", "GET");
		/////################################### 12 Apr 2021 ####################################///
		/// Passport copy added into attachments
		try{
			var EmpNo = ticket.EmpNo;
		}catch(err){
		...
		//// INS, TCK URL added 
		var batchOperation7 = oDataModel.createBatchOperation("DmsDocsSet?$filter=DepReq+eq+'999999999'+and+EmpNo+eq+'" + EmpNo + "'+and+DocType+eq+'PS'", "GET");
		var InsUrl = "DmsDocsSet?$filter=DepReq+eq+'{0}'+and+EmpNo+eq+'{1}'+and+DocType+eq+'{2}'";
		InsUrl = InsUrl.replace("{0}", ticket.TravelPlan);
		InsUrl = InsUrl.replace("{1}", EmpNo);
		InsUrl = InsUrl.replace("{2}", 'INS');
		var TckUrl = "DmsDocsSet?$filter=DepReq+eq+'{0}'+and+EmpNo+eq+'{1}'+and+DocType+eq+'{2}'";
		var sRequestNoZero =  ticket.TravelPlan.replace(/^0+/, '');
		TckUrl = TckUrl.replace("{0}", sRequestNoZero);
		TckUrl = TckUrl.replace("{1}", EmpNo);
		TckUrl = TckUrl.replace("{2}", "TCK");
		
		var batchOperation8 = oDataModel.createBatchOperation(InsUrl, "GET");
		var batchOperation9 = oDataModel.createBatchOperation(TckUrl, "GET");
		oDataModel.addBatchReadOperations([batchOperation3, batchOperation4, batchOperation5, batchOperation6,batchOperation7,oPsnlTripSet,batchOperation8,batchOperation9 ]);

		oDataModel.submitBatch(
			function(oResult) {
				try {
					//attachments 
					// INS added UCD1KOR 12 Apr, 2021
					oThis.getView().byId("UploadCollection").aItems=[];
					filesAll = $.merge(oResult.__batchResponses[0].data.results,
									   oResult.__batchResponses[1].data.results,oResult.__batchResponses[6].data.results,oResult.__batchResponses[7].data.results);
					//#######UCD1KOR 18 May 2020 Passport Copy added########////////
					var passportCopy = oResult.__batchResponses[4].data.results;
					if(passportCopy[passportCopy.length-1] !== undefined){
						filesAll.unshift(passportCopy[passportCopy.length-1]);
					}
					var uploadData = oThis.getView().getModel("new").getData();
					if(oResult.__batchResponses[6].data.results[0] !== undefined){
						filesAll.unshift(oResult.__batchResponses[6].data.results[0]);
					}
					uploadData.Files = filesAll;					
					oThis.getView().getModel("new").setData(uploadData);					
					oThis.getView().getModel("new").refresh(true);
					
					var HistoryTableModel = new sap.ui.model.json.JSONModel();

					HistoryTableModel.setData(oResult.__batchResponses[2].data.results);
					oThis.getView().setModel(HistoryTableModel, "HistoryTable");
					
					var profileData = sap.ui.getCore().getModel("profile").getData();
					if(oResult.__batchResponses[2].data.results.length >0 && (profileData.currentRole == "TKAD" ||profileData.currentRole == "TKM")){
						oThis.getView().byId("idHistoryLink").setVisible(true);
					}
					else{
						oThis.getView().byId("idHistoryLink").setVisible(false);
					}
				...
				//	oThis.getView().byId("icontabbar").getItems()[1].setCount(filesAll.length);
					...
					// addeb by UIK1KOR on 16/09/2020 for Personal trip during official travels_RBEIITAPPPIPELINE-176    
					var prsnltrip = new sap.ui.model.json.JSONModel();
				    prsnltrip.setData(oResult.__batchResponses[5].data);
				    
				    if(prsnltrip.oData.PersFlag == "")
				    	{
				    	prsnltrip.oData.prsflg = false;
				    	}
				    else if(oThis.getView().getModel("pt").getData().PersFlag == "X")
			    	{
				    	prsnltrip.oData.prsflg = true;
			    	}
				    oThis.getView().setModel(prsnltrip,"pt");
					// end of change on 16/09/2020
...
//					}));
	...
	onSaveAccept:function(source){
		...
		var ch1 = oThis.getView().byId("ch1").getSelectedKey();
		var ch1Value = oThis.getView().byId("ch1Value").getValue();
		
		var ch2 = oThis.getView().byId("ch2").getSelectedKey();
		var ch2Value = oThis.getView().byId("ch2Value").getValue();
		
		var ch3 = oThis.getView().byId("ch3").getSelectedKey();
		var ch3Value = oThis.getView().byId("ch3Value").getValue();
		
		var ch4 = oThis.getView().byId("ch4").getSelectedKey();
		var ch4Value = oThis.getView().byId("ch4Value").getValue();
	...
	onChangeCommens:function(){
		oThis.getView().byId("txtarea5").setValueState("None");
	},
...
//		//###################### UCD1KOR 11 May 2020 ################// commented below line
...
//			});
	...
	bindAdminTable : function(itemSelected, bindingItem, index) {
		...
		//Start of code uml6kor 23/8/2019 
		if (ticket.DepTrvCat == 'WRKP')
        { ticket.AssgnModel = ticket.AssgnModel + ' (Work Permit)';}
		else if(ticket.DepTrvCat == 'TRNG')
        { ticket.AssgnModel = ticket.AssgnModel + ' (Training)';}
		else if(ticket.DepTrvCat == 'TRFR')
        { ticket.AssgnModel = ticket.AssgnModel + ' (Transfer)';}
		//END of code uml6kor 23/8/2019
...
//		var v4 = oThis.getView().byId("v4");
		...
		if (length <= 2)
			addVer.addContent(new sap.m.FlexBox({
				...
				items : [ new sap.m.Label({
					...
					width : "200px"
				}).addStyleClass("align_ticket"), //uml6kor_upgrade 7/11/2019 added class
	...
	onPressHistoryLink:function(){
		if (!this._oDialogFund) {
			this._oDialogFund = sap.ui.xmlfragment("sap.ui.project.e2etm.fragment.ticketing.TcktHistory", this);
			this.getView().addDependent(this._oDialogFund);
		}
		this._oDialogFund.open();
	},
	onCloseTicketHistory:function(){
