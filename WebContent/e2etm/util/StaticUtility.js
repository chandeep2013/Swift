...
sap.ui.project.e2etm.util.StaticUtility = {
	...
	setBusy: function(bBoolean, oController) {
		...
		try {
			if (oRepatriation["BusyDialog"] != null) {
				oRepatriation["BusyDialog"].close();
			}
		} catch (exc) {}
				try {
			if (oRepdashboard["BusyDialog"] != null) {
				oRepdashboard["BusyDialog"].close();
			}
		} catch (exc) {}
		/// UCD1KOR Reports controllers added
		try {
			if (oPEReport["BusyDialog"] != null) {
				oPEReport["BusyDialog"].close();
			}
		} catch (exc) {}
		
		try {
			if (IMSReport["BusyDialog"] != null) {
				IMSReport["BusyDialog"].close();
			}
		} catch (exc) {}
		
		try {
			if (oInlandTravel["BusyDialog"] != null) {
				oInlandTravel["BusyDialog"].close();
			}
		} catch (exc) {}
		
		//fix bug V8 - 02
		if (oController.bTravelRequest) {
			oController.getView().setBusy(bBoolean);
			return;
		}
		...
		if (selectedProfileData.dependentDetail != null) {
			for (var i = 0; i < selectedProfileData.dependentDetail.length; i++) {
				if (selectedProfileData.dependentDetail[i].ZZ_DATE_EXPIRY !== '00000000' && selectedProfileData.dependentDetail[i].ZZ_DATE_EXPIRY !==
					'') {
					selectedProfileData.dependentDetail[i].ZZ_DATE_EXPIRY_VALUE = new Date(selectedProfileData.dependentDetail[i].ZZ_DATE_EXPIRY.substr(
							0, 4), selectedProfileData.dependentDetail[i].ZZ_DATE_EXPIRY.substr(4, 2) - 1, selectedProfileData.dependentDetail[i].ZZ_DATE_EXPIRY
						.substr(6, 2));
				}
				if (selectedProfileData.dependentDetail[i].ZZ_DATE_ISSUE !== '00000000' && selectedProfileData.dependentDetail[i].ZZ_DATE_ISSUE !==
					'') {
					selectedProfileData.dependentDetail[i].ZZ_DATE_ISSUE_VALUE = new Date(selectedProfileData.dependentDetail[i].ZZ_DATE_ISSUE.substr(0,
						4), selectedProfileData.dependentDetail[i].ZZ_DATE_ISSUE.substr(4, 2) - 1, selectedProfileData.dependentDetail[i].ZZ_DATE_ISSUE.substr(
						6, 2));
				}
			}
		}
	...
	// UCD1KOR COmmented 
	// Validation removed for duration of days
	// future uncomment
	...
	checkDateOverlapping: function(startDate, endDate, curRequest, type, version, priRequest) {
		...
		if (aList != null) {
			for (var i = 0; i < aList.length; i++) {
				if (aList[i].ZZ_DEP_REQ.replace(/^0+/, '') == curRequest.replace(/^0+/, '') || aList[i].ZZ_TRV_REQ.replace(/^0+/, '') == curRequest.replace(
						/^0+/, '') || aList[i].ZZ_TRV_REQ.replace(/^0+/, '') == priRequest.replace(/^0+/, '')) {
					if (type == "DEPU" || type == "BUSR" || type == "INFO") { // DEPU,
						// BUSR,
						// INFO
						continue;
					} else if (type == "SECO" && ((aList[i].ZZ_REQ_TYP == "DEPU" && aList[i].ZZ_TRV_REQ.replace(/^0+/, '') == priRequest.replace(/^0+/,
							'')) || aList[i].ZZ_DEP_REQ.replace(/^0+/, '') == curRequest.replace(/^0+/, ''))) { // SECO
						continue;
				...
				if ((aList[i].ZZ_DEP_STDATE <= startDate && aList[i].ZZ_DEP_ENDATE >= startDate) || (aList[i].ZZ_DEP_STDATE <= endDate && aList[i].ZZ_DEP_ENDATE >=
						endDate) || (aList[i].ZZ_DEP_STDATE >= startDate && aList[i].ZZ_DEP_ENDATE <= endDate)) {
					if ((startDate != aList[i].ZZ_DEP_STDATE && endDate != aList[i].ZZ_DEP_ENDATE && aList[i].ZZ_REQ_TYP == "DEPU" && (type == "BUSR" ||
							type == "INFO")) || aList[i].ZZ_REQ_TYP == "VISA" || (aList[i].ZZ_TRV_CAT == "TRFR" && aList[i].ZZ_DEP_TYPE == "DOME")) {
						continue;
					}
					//dye5kor
					if (aList[i].ZZ_TRV_REQ != priRequest) {
						var sMsg = "Request Number" + " '" + aList[i].ZZ_DEP_REQ + "' " + "has already opened from " + sap.ui.project.e2etm.util.Formatter.sapDate(
								aList[i].ZZ_DEP_STDATE) + " to " + sap.ui.project.e2etm.util.Formatter.sapDate(aList[i].ZZ_DEP_ENDATE) +
	...
	saveDependent: function(aData, oController) {
		...
		delete aDataCopy.employeeDetail.RetirementDate;
	...
	openDeputationExtFileDialog:function(controller, model){

		if (!controller[model.getData().FileUpload.length.toString()]) {
			controller[model.getData().FileUpload.length.toString()] = sap.ui.xmlfragment(
				"sap.ui.project.e2etm.fragment.common.DeputationEXTFileDialog", controller);
			...
			sap.ui.project.e2etm.util.StaticUtility.filterExtDocument(controller, model);
		}
		controller[model.getData().FileUpload.length.toString()].open();
	},
	filterExtDocument:function(controller, model){
		...
		sap.ui.project.e2etm.util.StaticUtility.setExtFilter(controller, afilter);
	...
	openDeputationFileDialog: function(controller, model) {
		if (!controller[model.getData().FileUpload.length.toString()]) {
			controller[model.getData().FileUpload.length.toString()] = sap.ui.xmlfragment(
				"sap.ui.project.e2etm.fragment.common.DeputationFileDialog", controller);
			controller[model.getData().FileUpload.length.toString()].setModel(model);
			controller.getView().addDependent(controller[model.getData().FileUpload.length.toString()]);
	...
	filterDocument: function(controller, model, sDocumentType) {
		...
		var oDocumentType = controller[model.getData().FileUpload.length.toString()].getContent()[0].getItems()[0].getItems()[0].getItems()[1];
		var sDocType = "P";
		if (sap.ui.getCore().getModel("global").getData().currentStage == "2" && (sap.ui.getCore().getModel("global").getData().currentSubSet ==
				"1_2_1" || sap.ui.getCore().getModel("global").getData().currentSubSet == "1_3_1")) {
			sDocType = "S";
		}
		if (sap.ui.getCore().getModel("global").getData().currentStage == "2" && sap.ui.getCore().getModel("global").getData().currentSubSet ==
			"1_4_1") {
	...
	uploadDocument: function(controller, model , ext) {
		...
		if (oFileUpload.getProperty("value").trim() == "") {
			...
			if(ext == "ExtFile"){
				var Type= "EX";
				 ...
				 Type= oDocumentType.getProperty("selectedKey");
			}
			var oData = {
			...
			if(ext == "ExtFile"){
				try{
					sap.ui.project.e2etm.util.StaticUtility.uploadFile(oDeputationThis, sap.ui.getCore().byId("changeEndDateExt--List_ExtDocument_Id").getModel(), aData.screenData
							.ZZ_DEP_REQ, aData.screenData.ZZ_DEP_PERNR);
				}catch(err){
					sap.ui.project.e2etm.util.StaticUtility.uploadFile(oDeputationThis, sap.ui.getCore().byId("extdoclistdepuDetail--List_ExtDocument_Id").getModel(), aData.screenData
							.ZZ_DEP_REQ, aData.screenData.ZZ_DEP_PERNR);
				}
				
			}else{
			sap.ui.project.e2etm.util.StaticUtility.uploadFile(oDeputationThis, sap.ui.getCore().byId("List_Document_Id").getModel(), aData.screenData
				.ZZ_DEP_REQ, aData.screenData.ZZ_DEP_PERNR);
			}
		}
	},
	...
	////////######################### UCD1KOR Aug 18th 2020 ################################////////
	////////########################### To get Extension documents ##########################///////
	getExtDocument:function(sCountry, sVisaType, controller, sDepReq, sEmpNo){
		...
		try {
			...
			var batchOperation0 = oDataModel.createBatchOperation("VISA_DOCSSet?$filter=Country+eq+'" + sCountry + "'+and+VisaType+eq+'" +
				sVisaType + "'", "GET");
			var batchOperation1 = oDataModel.createBatchOperation("VISA_DOCSSet?$filter=Country+eq+'" + sCountry + "'+and+VisaType+eq+'DPND'",
				"GET");
			var batchOperation2 = oDataModel.createBatchOperation("DmsDocsSet?$filter=DepReq+eq+'" + sDepReq + "'+and+EmpNo+eq+'" + sEmpNo + "'",
				"GET");
			...
			if (sap.ui.getCore().getModel("global").getData().currentStage == "2" &&
				...
				batch.push(oDataModel.createBatchOperation("DmsDocsSet?$filter=DepReq+eq+'" + sRequestNoZero + "'+and+EmpNo+eq+'" + sEmpNo +
			...
			//start of code uea6kor-uml6kor 26/8/2020 RBEIITAPPPIPELINE-217 date change extension
				if (controller.getView().getModel().getData().screenData.ZZ_VREASON_EX == "EX") {
						batch.push(oDataModel.createBatchOperation("DmsDocsSet?$filter=DepReq+eq+'" + sDepReq + "'+and+EmpNo+eq+'" + sEmpNo +
							"'+and+DocType+eq+'EX'", "GET"));
						cggsFormsFlag = true;
					}
//				end of uea6kor-uml6kor 26/8/2020 RBEIITAPPPIPELINE-217 date change extension
				else if (controller.getView().getModel().getData().screenData.ZZ_DEP_TOCNTRY == "DE" &&
					controller.getView().getModel().getData().screenData.ZZ_ASG_TYP == "STA") {
					batch.push(oDataModel.createBatchOperation("DmsDocsSet?$filter=DepReq+eq+'" + sDepReq + "'+and+EmpNo+eq+'" + sEmpNo +
						"'+and+DocType+eq+'CGF'", "GET"));
					cggsFormsFlag = true;
			...
			oDataModel.submitBatch(function(oResult) {
				if (sDeputationNo == '') {
					sap.ui.getCore().byId(controller.currentViewId+"--FileUploadExtButtonsId").setVisible(false);
				} else {
					...
					if (oData.DocumentName, oResult.__batchResponses[2].data) {
						...
						for (var i = 0; i < oResult.__batchResponses[2].data.results.length; i++) {
							...
							var bVisible = false;
							try {
								bVisible = sap.ui.getCore().byId(controller.currentViewId+"--FileUploadExtButtonsId").getVisible();
							} catch (exp) {
								oDeputationThis.visaCopyPanelDone = $.Deferred();
							}
							var oItem = {
								...
								DocType: sap.ui.project.e2etm.util.StaticUtility.getFileTye(sFileName.substr(0, sFileName.indexOf(".")), oResult.__batchResponses[
									0].data.results),
							...
							if (oResult.__batchResponses[4].data.results.length != 0) {
								for (var pp_i = 0; pp_i < oResult.__batchResponses[4].data.results.length; pp_i++) {
									...
									var oItem = {
										...
										Checked: false,
										Deleted: false
										//Visible: sap.ui.getCore().byId("FileUploadExtButtonsId") ? sap.ui.getCore().byId("FileUploadButtonsId").getVisible() : false
									};
									oData.FileUpload.push(oItem);
								}
						...
						var afilter = new Array();
						try {
							sap.ui.getCore().byId(oDeputationThis.onExtListId).setModel(oModel);
							sap.ui.getCore().byId(oDeputationThis.onExtRepId).setModel(oModel);
							sap.ui.project.e2etm.util.StaticUtility.setBusy(false, oDeputationThis);
							var afilter = new Array();
							ofilter = sap.ui.project.e2etm.util.StaticUtility.setExtFilter(controller, afilter);
							sap.ui.getCore().byId(oDeputationThis.onExtRepId).getBinding("rows").filter(ofilter, sap.ui.model.FilterType.Application);
							sap.ui.getCore().byId(oDeputationThis.onExtListId).getBinding("items").filter(afilter, sap.ui.model.FilterType.Application);
						} catch (exc) {
							sap.ui.project.e2etm.util.StaticUtility.setBusy(false, oDeputationThis);
							oDeputationThis.visaCopyPanelDone = $.Deferred();
						}
				...
				if (sDeputationNo == '') {
					...
					try {
						if (sap.ui.getCore().getModel("profile").getData().currentRole == "DEPU") {
							if (sap.ui.getCore().getModel("global").getData().currentStage == "2" && ((sap.ui.getCore().getModel("global").getData().currentSubSet ==
									"1_1_1" && oDeputationThis.getView().getModel().getData().screenData.ZZ_STAT_FLAG == "AA005") || (sap.ui.getCore().getModel(
										"global").getData().currentSubSet == "1_3_1" && oDeputationThis.getView().getModel().getData().screenData.ZZ_STAT_FLAG ==
									"AA007") || (sap.ui.getCore().getModel("global").getData().currentSubSet == "1_4_1" && oDeputationThis.getView().getModel().getData()
									.screenData.ZZ_STAT_FLAG == "AA008"))) {
							...
							if (controller.getView().getModel().getData().screenData.ZZ_DEP_TYPE == "INTL") {
								if (sap.ui.getCore().getModel("global").getData().currentStage == "1") {
									if (sap.ui.getCore().getModel("global").getData().currentSubSet == "1_2_1" && sap.ui.getCore().getModel("profile").getData().currentRole ==
										"EMP" && (controller.getView().getModel().getData().screenData.ZZ_STAT_FLAG == "AA004" || controller.getView().getModel().getData()
											.screenData.ZZ_STAT_FLAG == "JJ003")) {
										sap.ui.getCore().byId("FileUploadButtonsId").setVisible(true);
									...
									if ((sap.ui.getCore().getModel("global").getData().currentSubSet == "1_2_1" && sap.ui.getCore().getModel("profile").getData().currentRole ==
											"EMP" && (controller.getView().getModel().getData().screenData.ZZ_STAT_FLAG == "JJ007" || controller.getView().getModel().getData()
												.screenData.ZZ_STAT_FLAG == "JJ003")) || (sap.ui.getCore().getModel("global").getData().currentSubSet == "1_4_1" && (
											controller.getView().getModel().getData().screenData.ZZ_STAT_FLAG == "JJ010" || controller.getView().getModel().getData().screenData
											.ZZ_STAT_FLAG == "JJ003" || controller.getView().getModel().getData().screenData.ZZ_STAT_FLAG == "JJ002"))) {
					...
					$.when(oDeputationThis.visaCopyPanelDone).then(function() {
						...
						if (oData.DocumentName, oResult.__batchResponses[2].data) {
							...
							for (var i = 0; i < oResult.__batchResponses[2].data.results.length; i++) {
								var sFileName = oResult.__batchResponses[2].data.results[i].FileName;
								if (sFileName.substr(0, sFileName.indexOf(".")) == "CL_CONTRACT_LETTER" || sFileName.substr(0, sFileName.indexOf(".")) ==
									"CL_INVITATION_LETTER" || sFileName.substr(0, sFileName.indexOf(".")) == "CL_PRE_INVITATION_LETTER" || sFileName.substr(0,
										sFileName.indexOf(".")) == "CL_COC") {
									continue;
								}
								if (sFileName.substr(0, sFileName.indexOf(".")) == "CL_VISA_COPY_SELF_" + controller.getView().getModel().getData().screenData
									.ZZ_TRV_CAT) {
									try {
	...
	//////////################ UCD1KOR Aug 18th 2020 #######################/////////////
	setExtFilter:function(controller, aFilterArray){
		var sDocType = "EX";
		var oFilterSingle = new sap.ui.model.Filter("DocType", sap.ui.model.FilterOperator.EQ, sDocType);
	...
	setFilter: function(controller, aFilterArray) {
		...
		var oFilterSingle;
		if ((sap.ui.getCore().getModel("global").getData().currentStage == "2" && sap.ui.getCore().getModel("global").getData().currentSubSet ==
				"1_1_1") || (sap.ui.getCore().getModel("global").getData().currentStage == "1" && sap.ui.getCore().getModel("global").getData().currentSubSet ==
				"1_2_1")) {
			sDocType = "P";
			////###################### UCD1KOR Nov 2nd DocType PS Added#################///////////
			oFilterSingle = new sap.ui.model.Filter("DocType", sap.ui.model.FilterOperator.EQ, sDocType);
			sDocTypeV = "PS";
			oFilterSingle1 = new sap.ui.model.Filter("DocType", sap.ui.model.FilterOperator.EQ, sDocTypeV);
			aFilterArray.push(oFilterSingle,oFilterSingle1);
	...
	removeFile: function(oController, oModel,ext) {
		...
		sap.m.MessageBox.confirm("Do you want to delete selected file(s) ?", function(oAction) {
			if (oAction == "OK") {
				...
				if(ext == "ExtFile"){
					sap.ui.getCore().byId(oController.currentViewId+"--ExtViewRepeatId").getBinding("rows").filter([ofilterDelete, ofilterDocType], sap.ui.model.FilterType.Application);
				}
				else{
					sap.ui.getCore().byId("ViewRepeatId").getBinding("rows").filter([ofilterDelete, ofilterDocType], sap.ui.model.FilterType.Application);
				}
				sap.ui.project.e2etm.util.StaticUtility.deleteFile(oController, oModel, oController.getView().getModel().getData().screenData.ZZ_DEP_REQ,
					oController.getView().getModel().getData().screenData.ZZ_DEP_PERNR);
			}
		});
	},
	...
	uploadFileDeputation: function(oController, oControl, sDepReq, sEmpNo, sMessage) {
		...
		get.done(function(result, response, header) {
			...
			if (oControl.getId() == "UploadVisaSelf") {
				sFileName = "CL_VISA_COPY_SELF_" + oController.getView().getModel().getData().screenData.ZZ_TRV_CAT + oControl.getProperty("value")
			...
			} else if (oControl.getId().substr(0, oControl.getId().lastIndexOf("-")) == "UploadVisaDependent-panelDependentVisaCopy" || oControl
				.getId().substr(0, oControl.getId().lastIndexOf("-")) == "UploadVisaDependent-panelExistingDependentVisaCopy") {
			...
			post.success(function(data) {
				...
				if (oControl.getId() == "UploadVisaSelf") {
					...
					oData.screenData.ZZ_DOC_PATH = data.d.FileUrl;
					oController.getView().getModel().setData(oData);
				} else if (oControl.getId().substr(0, oControl.getId().lastIndexOf("-")) == "UploadVisaDependent-panelDependentVisaCopy" ||
					oControl.getId().substr(0, oControl.getId().lastIndexOf("-")) == "UploadVisaDependent-panelExistingDependentVisaCopy") {
			...
			for (var i = oData.FileUpload.length - 1; i >= 0; i--) {
				if (oData.FileUpload[i].Checked) {
					if (oData.FileUpload[i].Checked.Url != "") { // Document
						...
						iRequest++;
						var sUrl = sServiceUrl + "DmsDocsSet(DepReq='" + sDepReq + "',FileName='" + FileName + "',DocType='" + oData.FileUpload[i].DocType +
							"',EmpNo='" + sEmpNo + "',Index=" + iRequest + ",Module='',Country='')";
						...
						oItem.success(function(data, response, header) {
							for (var j = 0; j < oData.FileUpload.length; j++) {
								if (oData.FileUpload[j].FileName + oData.FileUpload[j].Extension == header.getResponseHeader('FileName')) {
									sap.ui.project.e2etm.util.StaticUtility.setCheckMark(false, oData.FileUpload[j].FileName, oData.FileUpload[j].DocType,
										oModel);
	...
	calculateTotalCost: function(aArray) {
		...
		try {
			for (var i = 0; i < aArray.length; i++) {
				nTotal += parseFloat(aArray[i].bag_cost) + parseFloat(aArray[i].board_cost) + parseFloat(aArray[i].con_cost) + parseFloat(aArray[i].dep_cost) +
					parseFloat(aArray[i].inl_cost) + parseFloat(aArray[i].ins_cost) + parseFloat(aArray[i].lod_cost) + parseFloat(aArray[i].otr_costs) +
					parseFloat(aArray[i].tkt_cost) + parseFloat(aArray[i].visa_cost);
			}
		} catch (exc) {}
		return nTotal;
	},
	...
	donwloadSelectedFile: function(ext) {
		try{
			var currentViewId = ext.split("#")[1];
			var ext = ext.split("#")[0];
		}catch(err){}
		
		//var currentViewId = currentView.split("--")[0];
		if(ext == "ExtFile"){
			if (!sap.ui.project.e2etm.util.StaticUtility.validateRemoveFile(sap.ui.getCore().byId(currentViewId+"--List_ExtDocument_Id").getModel())) {
				sap.m.MessageToast.show("No document is selected");
				return;
			}
		}
		...
		setTimeout(function() {
			...
			if(ext == "ExtFile"){
				var oData = sap.ui.getCore().byId(currentViewId+"--List_ExtDocument_Id").getModel().getData();
			}
			else{
			...
			for (var i = oData.FileUpload.length - 1; i >= 0; i--) {
				if (oData.FileUpload[i].Checked) {
					var url = "DmsDocsSet(DepReq='" + oDeputationThis.getView().getModel().getData().screenData.ZZ_DEP_REQ + "',EmpNo='" +
						oDeputationThis.getView().getModel().getData().screenData.ZZ_DEP_PERNR + "',DocType='" + oData.FileUpload[i].DocType +
						"',FileName='" + oData.FileUpload[i].FileName + oData.FileUpload[i].Extension + "',Index=1,Module='',Country='')?$format=json";
	...
	deleteUploadCollectionFile: function(oController, oEvent, sDepReq, sFileName, sDocType, sEmpNo, sIndex) {
		...
		get.done(function(result, response, header) {
			...
			var sUrl = sServiceUrl + "DmsDocsSet(DepReq='" + sDepReq + "',FileName='" + sFileName + "',DocType='" + sDocType + "',EmpNo='" +
	...
	dynamicBindingApprove: function(ZZ_REINR, ZZ_TRV_KEY, ZZ_MODID, ZZ_SMODID, oModel) {
		var oThis = this;
		oComponent.getModel().read("/ZE2E_REQ_LOGSet?$filter=ZZ_REINR+eq+'" + ZZ_REINR + "'+and+ZZ_TRV_KEY+eq+'" + ZZ_TRV_KEY +
			"'+and+ZZ_MODID+eq+'" + ZZ_MODID + "'+and+ZZ_SMODID+eq+'" + ZZ_SMODID + "'", null, null, false,
			function(oDataComment, oRespone) {
	...
	getDocumentList: function(oController, sRequest, sEmployee, sDocType, sModule, sCountry) {
		...
		try {
			if (sModule == "INSR" || sModule == "ACOM") {
				...
				/////// Passport copy added for INSR Dashboard UCD1KOR 12 Apr 2021
				///////////////////////////////////////////////////////////////////
				var batchOperation7 = oDataModel.createBatchOperation("DmsDocsSet?$filter=DepReq+eq+'999999999'+and+EmpNo+eq+'" + sEmployee + "'+and+DocType+eq+'PS'", "GET");
				oComponent.getModel().addBatchReadOperations([oBatch0, oBatch1, oBatch2,batchOperation7]);
		...
		oComponent.getModel().submitBatch(function(oResult) {
			...
			try {
				if (sModule == "INSR" || sModule == "ACOM" || (sModule == "CARG" && aaVendorFlag == true)) {
					...
					////////////// Passport copy added for INSR Dashboard UCD1KOR 12 Apr 2021
					if(sModule == "INSR"){
						var passportCopy = oResult.__batchResponses[3].data.results;
						aData.Files.unshift(passportCopy[passportCopy.length-1]);
					}
					if (sModule == "CARG") {
	...
	viewPdf: function(oController) {
		...
		if (window.location.hostname == "localhost")
			fileUrl = "proxy/sap/opu/odata/sap/ZE2E_DEP_NWGS_SRV/TravelPdfSet(EmpNo='" + empno + "',TrNo='" + tpno + "',TrvKey='" + ttype +
			"',Module='REQ')/$value";
		else
			fileUrl = "/sap/opu/odata/sap/ZE2E_DEP_NWGS_SRV/TravelPdfSet(EmpNo='" + empno + "',TrNo='" + tpno + "',TrvKey='" + ttype +
			"',Module='REQ')/$value";
	...
	onCloseSearchForex: function(evt, oForex, tabKey) {
		...
		if (module != "") {
			oDataModel.read("ForexWorklistSet?$filter=LoginRole eq '" + role + "' and TravelPlan eq '" + tpno + "' and Module eq '" + module +
				"' and Tab eq 'CL'", null, null, true,
				// success
	...
	getListAdmin: function(sRole, ZZ_OWNER, sModid, sAction, oThis, sStatus, SpecificDB) {
		...
		var sAccUrl =
			"/ZE2E_REI_HDRSet?$filter=ZZ_TRV_REQ+eq+'{0}'+and+ZZ_OWNER+eq+'{1}'+and+ZZ_TRV_KEY+eq+'{2}'+and+ZZ_VERSION+eq+'{3}'+and+WAERS+eq+'{4}'+and+ZZ_VISA_PLAN+eq+'{5}'&$expand=TRV_HDR,ZE2E_REI_DETAIL ";
	...
	checkActionEnable: function(sDOMEorINTL, sType, sStatus, sTravelStartDate, sTravelEnddate) {
		...
		} else if (sDOMEorINTL == "DOME") {} // end check DOME or INTL
	},
	...
	getExcelColumns: function(table) {
		...
		for (var i = 0; i < cols.length; i++) {
			if (cells[i].getBindingInfo("text")) {
				...
				if (text == "E.Date" || text == "DOB" || text == "Start Date" || text == "End Date" || text == "Submitted Date" || text ==
					"Date of Payment" || text == "Request Recd. Date" || text == "Request received Date" || text == "Cargo Movement" || text ==
					"Invoice Date") {
					columns.push({
	...
	checkValidCargoTp: function(asgType, trvreq, toCountry, toLoc) {
		...
		// UCd1KOR condition removed || asgType == "STX" 
		if (((asgType == "STVA" ) && Number(trvreq) > Number(cargoStvaTp)) || (asgType == "NC")) { //added uml6kor 14/6/2019 _New Policy_
	...
	modifyCggsData: function(data) {
		...
		var oData = {
			...
			Asgtyp: data.Asgtyp,
			Depreq: data.Depreq,
			Depsubtype: data.Depsubtype,
			Deptype: data.Deptype,
			Frmcntry: data.Frmcntry,
			Frmloc: data.Frmloc,
			Jobtitle: data.Jobtitle,
			Pernr: data.Pernr,
			Tocntry: data.Tocntry,
			Toloc: data.Toloc,
	...
	getCggsData: function(pernr, depreq, version, controller) {
		...
		oComponent.getModel().read("CggsDataSet(Pernr='" + pernr + "',Depreq='" + depreq + "',Version='" + version + "')", null, null, false,
			function(oData, response) {
...
//	//end of code exflag uea6kor-uml6kor 24/8/2020 RBEIITAPPPIPELINE-217 date change extension
	...
	fetchExtCheckListForms: function(aData, ExtDisplay) {
		try {
			var smodule = "EX";
			var oModel = new sap.ui.model.json.JSONModel();
			var oDataModel = new sap.ui.model.odata.ODataModel(sServiceUrl);
			var batchOperation0 = oDataModel.createBatchOperation("VISA_DOCSSet?$filter=Country+eq+'" + aData.ZZ_DEP_TOCNTRY + "'+and+VisaType+eq+'" +
					aData.ZZ_TRV_CAT + "'", "GET");
				
			var batchOperation1 = oDataModel.createBatchOperation("DmsDocsSet?$filter=DepReq+eq+'" + aData.ZZ_DEP_REQ + "'+and+EmpNo+eq+'" + aData.ZZ_DEP_PERNR + "'+and+DocType+eq+'" + smodule + "'",
				"GET");
			var batchOperation2 = oDataModel.createBatchOperation("GetDomain?DomainName='ZZ_DOC_TYPE'&$format=json", "GET");
			var batch = [ batchOperation0, batchOperation1,batchOperation2];

			oDataModel.addBatchReadOperations(batch);
			oDataModel.submitBatch(function(oResult) {
				if (aData.ZZ_DEP_REQ == '') {
					sap.ui.getCore().byId("FileUploadExtButtonsId").setVisible(false);
				} else {
					// Visible and invisible three buttons on top
					// Get and filter documents
					...
					// For each dependent, append document for them
					//if (viewData.selectedDependents.length != 0) {
						oData.DocumentName = jQuery.merge(oData.DocumentName, oResult.__batchResponses[2].data.results);
					//}
					// Set CSS for every required document to identify
					// template or not
					for (var i = 0; i < oData.DocumentName.length; i++) {
					...
					oData.DocumentType = oResult.__batchResponses[2].data.results;
					oModel.setData(oData);
						oData.FileUpload = new Array();
						if(oResult.__batchResponses[0]){
							for (var i = 0; i < oResult.__batchResponses[1].data.results.length; i++) {
							var sFileName = oResult.__batchResponses[1].data.results[i].FileName;
							if(sFileName !== undefined && sFileName !== "" && sFileName !== "undefined"){
								var oItem = {
										FileName: sFileName.substr(0, sFileName.indexOf(".")),
										FileContent: oResult.__batchResponses[1].data.results[i].FileContent,
										Extension: sFileName.substr(sFileName.lastIndexOf(".")),
										DocType:sap.ui.project.e2etm.util.StaticUtility.getFileTye(sFileName.substr(0, sFileName.indexOf(".")), oResult.__batchResponses[
											0].data.results) ,
										Image: sap.ui.project.e2etm.util.StaticUtility.getIconFileType(sFileName.substr(sFileName.lastIndexOf("."))),
										URL: oResult.__batchResponses[1].data.results[i].FileUrl,
										Content: null,
										Checked: false,
										Deleted: false,
										Visible: true
							...
							var file = [];
							for(var i=0;i<oData.FileUpload.length;i++){

								if(oData.FileUpload[i].FileName !== ""){
								    file.push(oData.FileUpload[i])
								}
							}
							oData.FileUpload = file;
...
//						controller.getView().getModel().setData(viewData);
						...
						var afilter = new Array();
						try {
//							sap.ui.getCore().byId("List_ExtDocument_Id").setModel(oModel);
							sap.ui.getCore().byId(ExtDisplay.sId).setModel(oModel);
//							sap.ui.project.e2etm.util.StaticUtility.setBusy(false, oDeputationThis);
							var afilter = new Array();
							ofilter = sap.ui.project.e2etm.util.StaticUtility.setExtFilter(aData, afilter);
							sap.ui.getCore().byId(ExtDisplay.sId).getBinding("rows").filter(ofilter, sap.ui.model.FilterType.Application);
							sap.ui.getCore().byId("List_ExtDocument_Id").getBinding("items");//.filter(afilter, sap.ui.model.FilterType.Application);
						} catch (exc) {
//							sap.ui.project.e2etm.util.StaticUtility.setBusy(false, oDeputationThis);
//							oDeputationThis.visaCopyPanelDone = $.Deferred();
						}
...
//	//end of code exflag uea6kor-uml6kor 24/8/2020 RBEIITAPPPIPELINE-217 date change extension
	...
	checkStvaAsg: function(asgType) {
		if (asgType == "STVA" || asgType == "VA" || asgType == "VN" || asgType == "NC"|| asgType == "STX" || asgType == "LTX" || asgType == "NCX" ) {//added uml6kor _new policy_ 13/6/2019
			return true;
		}
		return false;
	},
...
//				
//				var files_ex = cggsDisplay.getModel().getData();
//				var found_ex = false;
//				for (var i = 0; i < files_ex.length; i++) {
//					if (files_ex[i].DocType == "EX") {
//						found_ex = true;
//						break;
//					}
//				}
...
//			else 
			...
			} else if (sap.ui.project.e2etm.util.StaticUtility.checkStvaAsg(aData.ZZ_ASG_TYP) && aData.ZZ_TRV_CAT == "TRFR") {
	...
	uploadDocsRead: function(vTravelPlan, vEmpNo, sModule) {
		...
		oComponent.getModel().read(
			"DmsDocsSet?$filter=DepReq eq '" + vTravelPlan + "' and EmpNo eq '" + vEmpNo + "' and DocType eq '" + sModule + "'", null, null, true,
			function(oData, response) {
	...
	///#################### UCD1KOR File upload on manager dashboard for requisition forms ####################///////////
	validateUploadCggsFileChange:function(oEvent, vTravelPlan, vEmpNo, cggsDisplay, stvaFlag, version, exflag){
		 //added exflag uea6kor-uml6kor 24/8/2020 RBEIITAPPPIPELINE-217 date change extension
		...
		if (stvaFlag)
			sModule = "SRQ";
		if (exflag)   ////added exflag uea6kor-uml6kor 24/8/2020 RBEIITAPPPIPELINE-217 date change extension
			sModule = "EX";
	...
	validateUploadCggsFile: function(oEvent, vTravelPlan, vEmpNo, cggsDisplay, stvaFlag, version, exflag,model,controller) { //added exflag uea6kor-uml6kor 24/8/2020 RBEIITAPPPIPELINE-217 date change extension
	var inputFile = oEvent.getParameters("files");
		//var fileToLoad = inputFile.files[0];
		var fileToLoad = controller[model.getData().FileUpload.length.toString()].getContent()[0].getItems()[0].getItems()[2].getItems()[1];
		var oDocumentName = controller[model.getData().FileUpload.length.toString()].getContent()[0].getItems()[0].getItems()[1].getItems()[1];
		
		if (fileToLoad.getProperty("value").trim() == "") {
			sap.m.MessageToast.show("Please select an upload file", {
				my: "center center",
				at: "center center"
			});
		...
		var sFileType = fileToLoad.oFileUpload.files[0].type;
		var sImage = sap.ui.project.e2etm.util.StaticUtility.getIconMIMEType(sFileType);
		var sExt = fileToLoad.getProperty("value");
		sExt = sExt.substr(sExt.lastIndexOf("."));
		var Type= sModule;
		var oData = {
			FileName: oDocumentName.getProperty("selectedKey"),
		...
		upload = this.uploadFileToDms(fileToLoad, vTravelPlan, vEmpNo, sModule, version,oDocumentName,model);
		//		}
     if(sModule != "EX"){//added code uea6kor-uml6kor 24/8/2020 RBEIITAPPPIPELINE-217 date change extension
...
//		controller.getView().getModel().setData(viewData);
		// Filter 1st, 2nd, 3rd documents
		try {
			sap.ui.getCore().byId("extdoclistgrm--List_ExtDocument_Id").setModel(fileModel);
			sap.ui.getCore().byId("extdoclistgrm--ExtViewRepeatId").setModel(fileModel);
		}
		catch(err){}
	...
	uploadFileToDms: function(file, vTravelPlan, vEmpNo, sModule, version,oDocumentName,oModel) {
		//	fileUploader.setUploadUrl(sServiceUrl + "DmsDocsSet");
		var fileUpload = $.Deferred();
		//var file = evt.getParameters("files").files[0];
		var fileN = "";
		sap.ui.project.e2etm.util.StaticUtility.setBusy(true, oHomeThis);
		if (sModule == "SRQ" || sModule == "CGF") {
			var name = file.name.split(".");
			fileN = name[0] + "_" + version + "." + name[1];
			FileName = name[0] + "_" + version + "." + name[1];
		}
		////############## UCD1KOR 10/09/2020 #################///////////////////
		try{
			for (var i = 0; i < oModel.getData().FileUpload.length; i++) {
		...
		if (sModule == "EX") {
			fileN = oDocumentName.mProperties.selectedKey;
		}
		var sSlung = vTravelPlan + "," + vEmpNo + "," + FileName + "," + sModule;
	...
	deleteFileFromDms: function(oModel, sDepReq, sEmpNo) {
		oComponent.getModel().refreshSecurityToken(jQuery.proxy(function() {
			...
			var exData = oData;
			var exflag = false;
			if(oData.FileUpload != undefined){
				oData = oModel.getData().FileUpload;
				exflag = true;
			}
			...
			for (var i = oData.length - 1; i >= 0; i--) {
				if (oData[i].Checked) {
					if (oData[i].URL != "") { // Document															// has
						...
						iRequest++;
						var sUrl = sServiceUrl + "DmsDocsSet(DepReq='" + sDepReq + "',FileName='" + FileName + "',DocType='" + oData[i].DocType +
							"',EmpNo='" + sEmpNo + "',Index=" + iRequest + ",Module='',Country='')";
						var oItem = jQuery.ajax({
							cache: false,
							type: 'DELETE',
		...
		setTimeout(function() {
			...
			for (var i = fileData.length - 1; i >= 0; i--) {
				if (fileData[i].Checked) {
					var url = "DmsDocsSet(DepReq='" + vDepReq + "',EmpNo='" + vPernr + "',DocType='" + vDocType + "',FileName='" + fileData[i].FileName +
						fileData[i].Extension + "',Index=1,Module='',Country='')?$format=json";
					...
					for (var prop in data[i]) {
						...
						if (nav == "EmpSalDetlsNav") {
							if (column[1] == "ZzRevFrm") {
									...
									if(revDate[2] == undefined){
										revDate = oData[nav][i][column[1]].split("/");
									}
						...
						if (nav == "EmpMgrToMgrNav" || nav == "OhaAllowanceNav" || nav == "Rent") {
							if (column[1] == "Begda" || column[1] == "Endda") {
								...
								///############### 30 JUN 2020 Bug fix for begda ##############////
								if(aDate.length == 1){
									var aDate = aDate.toString();
									var aDate = aDate.split("/");
									aDate[0] = aDate[0] <10 ? ("0"+(aDate[0])):  aDate[0];
									aDate[1] = aDate[1] <10 ? ("0"+(aDate[1])):  aDate[1];
									aDate[2] = "20"+aDate[2];
								}
	...
	onOccCostReportSearch: function(oController, entitySet) {
		...
		var filterString = "/" + entitySet + "?$filter=Country eq '" + country + "' and City eq '" + city + "' and Month eq '" + month +
			"' and Year eq '" + year + "'";
	...
	fetchBookingDetails: function(oController, repType, table) {
		...
		var filterString = "RepType eq '" + repType + "' and Pernr eq '" + pernr + "' and Reinr eq '" + reinr + "' and Begda eq '" + begda +
			"' and Endda eq '" + endda + "'";
		...
		oComponent.getModel("reservation").read("/BookingDetailsSet?$filter=" + filterString, null, null, true,
			jQuery.proxy(function(oData, response) {
				...
				if (repType == "RPDP") {
					oModel.setData({
						BookingDetailsNav: [oData.results]
					});
				} else {
	...
	checkTravelCreation: function(aData, link) {
		var odata = {
			Cntry: aData.ZZ_DEP_TOCNTRY,
			Asgtyp: aData.ZZ_ASG_TYP,
			Pernr: aData.ZZ_DEP_PERNR,
			Trvky: aData.ZZ_TRV_CAT,
			DepReq: aData.ZZ_DEP_REQ,
			OrgUnit: ""
		};
	...
	checkTrvlDatesOverlapping: function() {
		...
		if (aList != null) {
			for (var i = 0; i < aList.length; i++) {
				...
				//changes added for check overlapping travels during travel settlement by UEA6KOR_18.06.2019
				var existDR = aList[i].ZZ_DEP_REQ;  
				var existTR = aList[i].ZZ_TRV_REQ;							
				var newDR = aData.ZZ_DEP_REQ;     
				var newTR = aData.ZZ_TRV_REQ;     
			
				if( existTR != newTR && existDR != newDR ){ 
					// Validation is ok
